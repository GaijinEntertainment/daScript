...
| `per_call_overhead.das` | Fixed costs paid per call or per loop entry rather than per element: a helper that makes a block (a `peek_data` string helper, a block-form table `get`), entering a `for` over a 4-element array and over a 4-entry table's keys, and comparing strings that share a long prefix |
