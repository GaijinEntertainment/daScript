...
def release() {
    ...
    release_program("my-bench", "tools/bench.das")   // a companion exe baked beside the main one under the same class, sharing the bundle's runtime libraries and modules
...
**wasm64 ONLY, lowered at link.** Every compiled web artifact is cross-compiled as memory64 (8-byte pointers). The wasm32 *cross-compile* target is unsupported - on a 64-bit host it bakes 8-byte-pointer layouts the wasm32 runtime reads as 4 (garbage); pure scripts only ever "worked" at wasm32 by dodging that bug class. wasm64 makes host-ptr-size == target-ptr-size, so the bug class vanishes with zero codegen changes. The link then runs emscripten's `-sMEMORY64=2`: the objects and archives keep their 8-byte pointers (the JS boundary still passes them as BigInt), and Binaryen lowers the memory itself to 32-bit, so the artifact runs on every engine - WebKit included, which has no memory64. The price is a 4 GB address space (2 GB by emscripten's default `MAXIMUM_MEMORY`). The feature string carries no `+relaxed-simd` for the same reason: WebKit refuses a module that uses any of it.
