...
For the corresponding tutorial, see [tutorial_linq_decs](https://daslang.io/doc/reference/tutorials/55_linq_decs.html#tutorial-linq-decs). For
nanosecond-level cost comparisons across the SQL / Array / Decs lanes,
see `benchmarks/sql/results.md` in the daslang source tree (repo-only).
...
* - `._where(P)._select(F).min_by(K)` / `.max_by(K)` / `.min_max()` / `.min_max_by(K)` / `.min_max_average()` / `.min_max_average_by(K)`
  - `plan_loop_or_count` -> `emit_min_max_family_lane`
  - Multi-output reduce mirroring the daslib impls exactly (first-element latch, strict `less`, average seeded from the first element and divided in the element type); one state var per output. Non-copyable element results cascade.
...
* - `arrA |> _join(unsafe(each_kv(tab)), <on a == d.key>, ...)` (or `keys(set)` with a bare-element key; any terminator/where/select form above)
  ...
  - **Table-srcB probe**: the b-key selector IS the table key, so no
    ...
    lead, table lead - tablextable probes both sides).
...
* [tutorial_linq_decs](https://daslang.io/doc/reference/tutorials/55_linq_decs.html#tutorial-linq-decs) - using `from_decs_template` and
  `_fold` over decs entities.
* [tutorial_linq](https://daslang.io/doc/reference/tutorials/28_linq.html#tutorial-linq) - the underlying linq surface (`where`,
  `select`, `order_by`, etc.) and the `_fold` macro.
* [linq_fold](https://daslang.io/doc/stdlib/generated/linq_fold.html) - the `linq_fold` module
  reference (the two public macros `_fold` and `_old_fold`).
* [linq](https://daslang.io/doc/stdlib/generated/linq.html) - the full linq surface.
* [linq_boost](https://daslang.io/doc/stdlib/generated/linq_boost.html) - the shorthand macros
