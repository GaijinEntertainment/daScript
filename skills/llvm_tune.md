...
`DAS_TUNE_MANIFEST` env overrides the location — **and its presence also disables the policy
rail entirely** (`tune_policy_gated`: an explicit manifest means manual). Sectioned:
`"kernels"` (perm winners, flat fname→suffix), `"runtime"` (library knobs — dasLLAMA's),
`"provenance"` (refreshed each write), `"race"` (per-family race results behind each
multi-variant crown). **Staleness:** a sidecar older (mtime) than the running
binary reads as ABSENT everywhere and `tune_manifest_set` resets it — measurements never
outlive the binary (kills the worktree hand-copy trap). So does one whose box identity differs
from this box's — the cross-box-copy trap. The identity is the five-field
`platform|arch|model_id|os_build|cpu` string `tune_box_identity()` builds (non-empty cpu on
every platform, full `os_build` incl. the Windows UBR), but the COMPARISON goes through
`box_match_key`, which folds the volatile Windows UBR (the 4th `os_build` component) — so a
monthly cumulative update does NOT re-tune, while a real OS-version change still does. A sidecar
is fresh anyway when `provenance.applied_box` matches this box (via the same key): that is how a
scope resolver adopts a sibling box's mint, while `box` keeps naming the box that measured it.
Several libraries share one file:
...
- `[tune_scope(name="lib", tuner="rel/path.das", covers="modA;modB")]` — on a dummy struct in the **library** module owning the kernels; names the tuner unit that regenerates this library's keys in the app sidecar. `tuner=` resolves against the declaring file. `covers=` extends the DEMAND to other modules' kernels: completeness = a sidecar entry for every `[tune]` + non-`perm=`-pinned `[tuned]` fn across declaring+covered modules (AST-derived — `module_kernel_fnames`); a tuner missing a demanded kernel re-tunes every start and the startup refusal NAMES it (`tune_sidecar_verdict`'s missing arm; `tune_sidecar_missing_entries` stays as the bare-list API). `version_of="module/CONST"` (optional) pins the scope to a library version: the sidecar's provenance must record the int const's folded value under the lowercased const name (`version_key=` overrides) or the scope reads incomplete with a named reason — the declaring module must `require` the const's module (the annotation names it only by string; suppress the unused-require lint), and the tuner must stamp the key via `tune_provenance_note` or it re-tunes every start (dasLLAMA: `DASLLAMA_VERSION`, stamped by `tuner_noise`; that module's checklist owns when the counter bumps). Reading winners needs no scope — every `[tune]`/`[tuned]` reads the app sidecar. **Visibility trap:** the default-auto pass fires only for roots that SEE llvm_tune — the scope-owning library must `require llvm/daslib/llvm_tune public` (re-export llvm_tune ALONE — a blanket public that also re-exports jobque_boost floods requirers with ambiguities like `Stream`).
- `[tune_policy(missing = fallback|warn|error|auto|restart)]` — on the app's `main`. `error` = dev mode (fail compile, print tuner cmd). `auto` = tune at runtime + re-exec — **and the DEFAULT ("untuned does not start")**: any root-with-`main` app whose libs declare scopes gets it with NO annotation (the `TuneDefaultPolicy` infer pass injects it; root detection = the anonymous module name; the appended `tune_policy` marker annotation is the once-only/dedup). Escapes: `DAS_TUNE_POLICY=fallback` (CI kill switch), `--jit-opt-level=0` (DEFAULT only — flips the injected default to `fallback` because winners are raced under O3 codegen; a declared `[tune_policy]` is unaffected and `DAS_TUNE_POLICY` still overrides in both directions; pass it after `--` in a script run; `jit_cli_opt_level()` is the compile-time query), `tune_suppress_mint(knob)` (RUNTIME, from a library `[init]` — suppresses the `auto`/`restart` MINT process-wide, so untuned scopes keep fallback stamps and the process runs; `knob` is the override name the caller acts for and prints verbatim in the suppression line; an explicit `--tune` still wins; dasLLAMA wires `DASLLAMA_ALLOW_UNTUNED=1` to it), the host's `-documentation` policy for documentation/reflection roots (all `[tune]` transforms and policy are inert; query through `is_building_documentation()`), the standing gates, and structurally any root without `main` — dastest test files never tune. `DAS_TUNE_POLICY` env overrides any declared flavor.
...
- **Scope resolver** — `tune_set_scope_resolver(fn)`, registered from an `[init]` (it must land before the policy guard at the top of `main`): the auto and restart guards consult it before spawning a scope's tuner — dasLLAMA's exchange client (`modules/dasLLAMA/performance/exchange_client.das`, repo-only) is the worked consumer, downloading a matching sidecar instead of racing ~20 minutes. True = "the scope may now be satisfied"; completeness is re-checked, never trusted; false (or still-incomplete) falls through to the local tuner, and `--tune` never consults it. A resolver adopting a sibling box's sidecar stamps `provenance.applied_box` (see Staleness above); the resolver prints its own restart marker (`exchange: sidecar applied, restart to apply the winners` — supervisors treat it exactly like the tuner's).
- **Supervisor interrupt** — `tune_interrupt_requested()` is true while the file named by `DAS_TUNE_CONTROL` exists. The dasLLAMA tuner halves poll it between kernel families (`interrupt_gate` in `modules/dasLLAMA/harness/tuner_noise.das`, repo-only) and hard-stop without minting; minting is end-gated, so the measurement in flight always completes. Caveat: the dasLLAMA tuner runs its two halves (`gen_tune_probe`, then `tune_kernels`) as SEPARATE processes, each end-gating its OWN write — so an interrupt during the second half leaves the first half's mint on disk. That merges cleanly (both halves measure THIS box), but "writes nothing" holds only within one process. The watchdog's stop-the-tune tray actions ride this (during a tune there is no HTTP to ask through).
...
- **Cross-module restamp is broken.** Mutating a *required* library's already-compiled `[tune]` function from the root apply (strip+restamp `[llvm_code]`) trips the re-infer pass (`Function::visit` AV). Library kernels take winners at `[tune]` time from the app sidecar, NOT via a post-hoc restamp; the re-exec model exists precisely to avoid this.
...
band (`max(1.5×floor, 1%)`) in BOTH windows — a one-window flip or differing challengers is
state scramble among twins, not a false crown. The band bounds twin scramble between QUIET
mints (measured ≤1.2%), and a burst-minted winner is caught by the same-challenger
consistency test (a burst crown loses both quiet re-races). Absolute median drift between race and re-race is stamped
