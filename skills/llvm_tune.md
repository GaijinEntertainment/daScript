...
outlive the binary (kills the worktree hand-copy trap). So does one whose `provenance.box`
differs from this box's `tune_box_identity()` (the five-field `platform|arch|model_id|
os_build|cpu` string, all platforms) — the cross-box-copy trap — UNLESS
`provenance.applied_box` names this box: that is how a scope resolver adopts a sibling box's
mint while `box` keeps naming the box that measured it. Several libraries share one file:
...
- **Supervisor interrupt** — `tune_interrupt_requested()` is true while the file named by `DAS_TUNE_CONTROL` exists. The dasLLAMA tuner halves poll it between kernel families (`interrupt_gate` in `modules/dasLLAMA/harness/tuner_noise.das`, repo-only) and hard-stop without minting — minting is end-gated, so the measurement in flight always completes and an interrupted run writes nothing. The watchdog's stop-the-tune tray actions ride this (during a tune there is no HTTP to ask through).
