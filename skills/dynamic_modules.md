...
build, `.das_module.emscripten.manifest` from a wasm `--jit-target` run, so the binary kinds and
the targets sharing a tree keep their own), and later starts replay those rows
