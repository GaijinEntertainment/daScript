...
When daslang is built as a dynamic/DLL binary (`daslang_dyn` / `daslang`), it cannot use compile-time `NATIVE_MODULE` macros or static linking to resolve modules under `modules/`. Instead, each module directory needs a `.das_module` descriptor — a small daslang script that tells the runtime where to find the module's components.
...
|--------|-------------|---------------------|
| `daslang_static` | `daslang` | `NATIVE_MODULE` macros (compiled from `external_resolve.inc`) + static linking of C++ module libs |
| `daslang` (default) | `daslang_dyn` | `.das_module` descriptor scripts + `.shared_module` DLLs |
