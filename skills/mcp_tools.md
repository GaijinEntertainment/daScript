...
| `format_file` | Running the formatter script manually; supports comma-separated list or glob, returns JSON array of results. Comment policy is the folder's: the nearest `.lint_config` `[format]` table decides — `force_clean_comments = true` also strips comments file-wide (keeps the leading header block, `//!` docs, `//fmt:` directives, `nolint:` suppressions; fail-closed — a strip must pass compile_check or the file is restored), `format_enabled = false` exempts the tree. No per-call override |
