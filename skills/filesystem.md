...
- **`dir_rec(path)` callback yields paths RELATIVE to the walked root** (`std::filesystem::relative(entry, root)` — source-verified `builtin_fs_dir_rec`; a direct child yields just its filename, a nested entry yields `sub/dir/file`), never absolute. `path_join(root, name)` before any `fread`/`stat` — a bare `fread(name)` silently returns `""` from the wrong cwd.
