...
**Before annotating, verify the function CLONES `p` internally — not MUTATES `p` in place.** `apply_template` and the entire `apply_qmacro_*` / `apply_qblock_*` family **mutate their input** via `TemplateVisitor`; pre-clones at their callsites are **load-bearing**. Marking them `[clone(...)]` would make PERF024 flag callers who are doing the right thing. Read the function body and confirm: every code path either calls `clone_*(p)` on the param and stores the result, OR forwards `p` to another `[clone(p)]`-annotated function. If `p` flows into `apply_template(rules, at, p)` without a clone wrap, it's a mutator, not a cloner.
