...
- **`get_const_expr(program, expr)`** — the compiler's own demand fold, for a macro or backend that needs the VALUE of a const whose initializer the policy left unfolded (a lint run, `options infer_time_folding = false`). Returns a fresh constant expression, or `null` when the expression is not one. It folds arithmetic and constant constructor calls (`A * 4`, `float4(1,2,3,4) / 2`, a const named off another const, a sum of any length), which an is-this-already-constant test of your own does not — that is the whole reason to ask rather than hand-roll. A cycle of consts defined off each other is broken by the in-flight set, which reads a revisited global as non-constant and leaves the standard init-loop diagnostic to report it; each const is folded at most once per ask, so a chain of them costs its length rather than the shape of its reads. It records no access-fold on what it reads, so asking never suppresses the unused-variable or dead-store lint on those globals. The answer lands on whichever gc root is in force: from application code wrap the ask in `ast_gc_guard()`, or the node dangles once the program dies and surfaces as a crash in the exit leak report, nowhere near the ask.
