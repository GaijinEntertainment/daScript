...
- Adding a new MCP parameter: extend the tool's `ToolDef` entry in `utils/mcp/registry_das.das` (the `make_tool(...)` schema, `arg_names`, and `required`/`any_required`), then thread the new arg through the handler lambda into `do_judge_duplicates` / `do_find_dupe` and into `run_find_dupe_judge`. Mind the 6-arg dispatch limit (`arg1`..`arg6`) — combine related options or drop low-value ones first.
