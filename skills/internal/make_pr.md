...
| 0 Commit order | `git log --reverse --oneline origin/master..HEAD`, then `git log -p` over each commit's files | Fixes first, features after: every bug fix the branch carries is its own commit, ahead of every feature commit, so it can land or be cherry-picked without the feature. No commit rewrites or removes lines an earlier commit on the branch introduced - that change folds into the earlier commit (soft-reset and rebuild the series), so each commit reads as final. A pin or version stamp (`LLVM_JIT_EMITTER_HASH`, `LLVM_JIT_CODEGEN_VERSION`) is the exception: every commit that changes what it covers sets it again |
...
| 0a REVIEW audit | preflight `review-md` gate (`make-pr --only review-md` reruns it alone) | Red = a discovered `REVIEW.das` gate failed - fail-fix, no agents until green. Then one `review-md-auditor` per checklist. Discovered rules bind on top of this file; checklist defects fixed in the same batch, then two dragon passes per rule document at most - one over the batch, one cold re-read whose serious findings close the round without a third read (`skills/review_md.md`, the damper) |
...
| 0a Dupe audit | one `dupe-auditor` over the whole diff (`skills/dupe_audit.md`) | DUPLICATE -> reuse the existing function, or state in the change why it cannot serve. TEMPLATABLE -> discuss the fold before any fix; a declined fold is named in the PR body's `### Not done` |
...
| 1.5 Dupes | `make-pr --only dupes` (scoping + report; modes: `skills/internal/detect_dupe.md`) - off the default chain, the rerun form when commits after row 0a add functions | The structural sweep alone; the verdicts are the row-0a `dupe-auditor`'s (`skills/dupe_audit.md`), which reads the code. Skip for tests/fixtures/generated-only PRs |
