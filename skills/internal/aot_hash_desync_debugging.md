...
3. **The function's module was never AOT'd at all.** The diagnostic tells the two apart: "no AOT stub links for any function of module X" is a file missing from the AOT build, "other functions of module X link" is a stale artifact. `tests/daslib` files are listed one by one in `AOT_DASLIB_FILES` (`tests/aot/CMakeLists.txt`), and configure fails on a file there that is neither listed nor `options no_aot` - or both; the option is only for a file whose required module keeps out of AOT, never a way to skip registration. New test dir not registered in `tests/aot/CMakeLists.txt`, or the module is in the daslib AOT *exclusion* regex there (modules whose builtins lack AOT header decls get excluded - if you gave such a module runtime-callable functions, add the missing decls to `include/daScript/simulate/aot_builtin_ast.h` and un-exclude it; precedent: `style_lint` and its five `for_each_*_macro` decls).
