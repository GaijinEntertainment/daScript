...
| ast-verify changed `.das` | preflight's `ast-verify` gate — `<daslang> -dry-run --ast-verify <file>` per changed `.das` (plus the `tests/linq/test_linq_fold.das` qmacro canary), parallel with a 300s per-file timeout, `cant_`/`failed_`/`invalid_` skipped; only an `AST verify` line fails, a compile error belongs to whoever owns the file | mirrors `extended_checks.yml` "Run ast-verify on changed .das files" exactly. Each item is a whole-engine compile under the verifier (30-80s for anything requiring dasLLAMA), so a sweep-shaped diff is minutes locally and longer on CI's 4 cores — width is physical cores halved, `-j` only lowers it |
