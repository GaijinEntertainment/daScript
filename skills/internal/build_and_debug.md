...
Dev-tier rails that cut the edit-compile-run loop. Never benchmark through `--jit-opt-level=0`; a `-jit` benchmark that must see one-unit codegen passes `--jit-split-modules=0`. An `-exe` is one codegen unit unless its build passed `--jit-split-modules=-1`, which partitions it per das module; `--jit-lto` on top of a split exe recovers the one-unit form's cross-module inlining, and its POSIX link needs a `clang++` on PATH whose LLVM is not older than the one daslang bundles (an older driver reads no newer bitcode; the exe tests skip the LTO cell on such a box, Apple's clang included).
...
- **From a script:** `compile_file` reads and refreshes the same default cache when the compile's `CodeOfPolicies` sets `module_cache = true` (the LSP subtools do); the key folds the compiled file, the binary, this process's arguments up to `--` and the policies, so a lint-profile compile and a plain one never share a record. Off by default: a nested compile parses from source.
