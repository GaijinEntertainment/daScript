...
- **Invalidation** is mtime+size on the sources and content-hash on the compile-time inputs a macro pins (`add_module_cache_dependency` - a tune sidecar, a config), positional like the obj cache: an edited module recompiles itself and everything after it, and the cache rewrites itself. Content-hash rather than mtime is why an app that rewrites its sidecar byte-identically on exit does not churn the cache.
