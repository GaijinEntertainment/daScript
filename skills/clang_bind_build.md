...
- **`find_package(Clang)` succeeds but links wrong** — the major.minor must match the pin exactly; a too-old SDK (e.g. 16.0.6 against a 22.1 pin) is silently rejected and the whole module is skipped (no error, just no `cbind`).
- **CRT match** — daslang compiles its modules with the static CRT (`/MT`); any C/C++ dep linked into a module must match (see the libhv `BUILD_FOR_MT` handling in [modules/dasHV/CMakeLists.txt](../modules/dasHV/CMakeLists.txt)). The official `clang+llvm-…-windows-msvc` `libclang.lib` already matches.
