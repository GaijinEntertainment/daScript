...
also crosses to C - a standalone context declares it in the C half of the header it generates, so a
C host needs no daslang API (`require daslib/export_c`; `[export_c(name = "...")]` picks the C
symbol, which is how two overloads both reach C); `[init]` / `[finalize]` run at context
