...
C++ binding is made optional), `module_exists(name)` (a module the compiling program required,
C++ or das - the guard for a path-guarded das target, same answer inside a tool's nested compile),
