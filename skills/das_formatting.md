...
> **Not the converter:** the installed `bin/gen1_to_gen2` binary (CMake target from `utils/dasFormatter/main.cpp`) is the **v1→v2 syntax converter** (flags `-i` / `--tests` / `--semicolon`), not a code formatter. The formatter is the daslang script `utils/das-fmt/dasfmt.das` (flags `--path` / `--verify`) — invoke it through `bin/daslang utils/das-fmt/dasfmt.das -- ...` or the MCP `format_file` tool.
