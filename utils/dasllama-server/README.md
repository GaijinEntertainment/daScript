...
| `POST` | `/v1/audio/phonemes` | The front end alone (needs `--tts`): `{"input", "voice"?}` -> `{"normalized", "lang", "chunks": [{"text", "phonemes"}]}` - the normalizer's spoken form of the text, the dialect the voice speaks (`lang`, the model's first language when no voice is named or the name is an alias), then one row per chunk a synthesis of it would take, each carrying that chunk beside its phoneme string in that dialect. Answered by the TTS worker on the same queue as a synthesis (the same 4096-character cap and the same 503 when no speech model is served), so the speech studio can show what the model will actually say |
