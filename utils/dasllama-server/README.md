...
| `--tts-voices-dir` | - | - | Directory of voice clips (wav / flac / mp3 / ogg, a few seconds of one speaker each) a cloning speech model adds to its voices at boot, each under its file's stem (`mine.wav` -> voice `mine`; a stem the model already carries replaces that voice). A clip that does not decode is logged and skipped; the key on a model that cannot clone is logged and ignored |
