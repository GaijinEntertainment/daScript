...
the server - pp512 and tg128, three timed reps each after an untimed warmup, and on a slot whose
device homes streams tg128@N: N device-home streams (up to four, as many as the device homes)
through the scheduler's device mode, the summed rate - and shows tokens per second beside the hardware line; every route that could contend holds for the run's
