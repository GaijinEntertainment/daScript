...
| `POST` | `/v1/audio/phonemes` | The front end alone (needs `--tts`): `{"input"}` -> `{"normalized", "chunks": [{"text", "phonemes"}]}` - the normalizer's spoken form of the text, then one row per chunk a synthesis of it would take, each carrying that chunk beside its 45-symbol phoneme string. Answered by the TTS worker on the same queue as a synthesis (the same 4096-character cap and the same 503 when no speech model is served), so the speech studio can show what the model will actually say |
