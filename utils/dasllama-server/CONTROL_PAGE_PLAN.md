...
stringified at poll/reap; (2) the 27-key config schema hand-listed 5x across main.das +
...
constant, `-r` per-stage requests; per-request status lines) - live run 15/15 x 200 against
...
- **daslib**: new `write_toml(JsonValue?) : string` in `daslib/toml.das` (parse-only today).
  Canonical emit, comments lost - accepted. Doc + tests (round-trip read_tomlowrite_toml).
...
- **/v1/stats grows**: `prefill_tokens`, memory footprint (weights bytes, KV pages x
