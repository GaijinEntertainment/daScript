...
| `bench_idle.json` / `bench_done.json` | `curl $B/bench` before / after a run of the in-process mode - the server started with no `lcpp_bin`, `curl -X POST $B/bench`, then the GET once `state` reads `done` (under a minute on `smol`) |
| `bench_failed.json` | the A/B mode: the server started with `lcpp_bin` set to any path, `curl -X POST $B/bench`, then the GET once it fails. An in-tree A/B run ALWAYS fails (lcpp_bench refuses `-jit` script runs - it measures released exes only), which is exactly the state `bench_failed` pins |
