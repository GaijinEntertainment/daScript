...
| `bench_done_batched.json` | HAND-COMPOSED, not captured: `bench_done.json`'s shape with the batched row's three keys (`ours_tgb`, `tgb_sd`, `npl`) and its log lines, in the numbers a 5060 Ti box read for Llama-3.2-1B Q8_0. The capture box (an M1 Max, Metal) homes no device streams, so its server never produces the row; this fixture pins what the page does with the keys when a box does |
