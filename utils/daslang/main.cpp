...
static bool libExportAll = false; // -lib-export-all: every public entry-module function is a C entry
static bool libNeedsOutput = false; // -lib: the host includes the header by name, so the path is required
