...
```
daslang utils/internal/flatten-fuzz/main.das -- [flags]
```
...
| `-B, --bool` | pure-bool domain: bool params/locals/return, **exhaustive** 2^3 truth-table oracle (ignores `-n`/`--inputs`) |
| `-?, --show-help` | help |
...
- **Bool** (`--bool`) - every value is `bool` (params, locals, return), the
  ...
  3-bool-input function has only 2^3 = 8 possible inputs, so each generated
...
- A deterministic compiler crash (the SIGSEGV/SIGBUS whose faulting address was
  ...
  shoe allocator (`MemoryModel::Deck`) - `total * size` (entry count x element
  ...
  per-`ExprVar` `"{name}"` string built inside the O(n^2) opt-pass tree walks;
