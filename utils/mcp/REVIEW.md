...
it, or it has its own `main` that something in the shipped SDK runs - also adds it to the
`install(FILES ...)` block that lists `utils/mcp/main.das` in `CMakeLists.txt` (repo root),
in the same change.** `tools/` and `subtools/` are globbed; a top-level file left out of the
