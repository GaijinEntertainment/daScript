...
**A new or changed test under a `utils/` tool whose assertions CI can run ships in the same
change with the CI row that runs them** — a row in `.github/workflows/extended_checks.yml`
(repo root), a `DAS_UTILS_TO_TEST` entry in `CMakeLists.txt` (beside this file) that the
`run_utils_tests` row builds, or the tool's own workflow for a browser suite. A row that only
compile-checks the test (`dastest --compile-only`) is not that row. A test whose assertions no
