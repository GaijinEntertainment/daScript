...
TEST_CASE("jit_lib_create_finish drops a context whose init raised, and keeps the message") {
    ...
    char * slot = nullptr;
    CHECK(jit_lib_create_finish(ctx, 0, &slot) == nullptr);
    REQUIRE(slot != nullptr);
    CHECK(std::strstr(slot, "boom in the body") != nullptr);

    das::Context * good = make_ctx();
    REQUIRE(good != nullptr);
    CHECK(jit_lib_create_finish(good, 1, &slot) == good);
...
TEST_CASE("jit_lib_run_once runs a library's init in a process that already carries a runtime") {
    ...
    int32_t guard = 0;
    void * env = nullptr;
    CHECK(jit_lib_run_once(&guard, &env, &bump_once) == 1);
    CHECK(g_once_calls == 1);
    CHECK(guard == 2);
    CHECK(env == (void *) das::daScriptEnvironment::getBound());
    CHECK(jit_lib_run_once(&guard, &env, &bump_once) == 1);
    CHECK(g_once_calls == 1);
