...
VECMATH_FINLINE void VECTORCALL v_sincos4(vec4f x, vec4f& s, vec4f& c)
{
  ...
  xl = v_mul(x, _mm_set1_ps(0.63661977236f));

  vec4i q = _mm_cvtps_epi32(xl);

  offsetSin = v_andi(q, (vec4i)V_CI_3);
  offsetCos = _mm_add_epi32(_mm_set1_epi32(1), offsetSin);

  vec4f qf = v_cvt_vec4f(q);
  xl = v_nmsub(qf, _mm_set1_ps(_SINCOS_KC2), v_nmsub(qf, _mm_set1_ps(_SINCOS_KC1), x));
  ...
  vec4f cx =
    v_madd(
      v_madd(
        v_madd(_mm_set1_ps(_SINCOS_CC0), xl2, _mm_set1_ps(_SINCOS_CC1)),
  ...
  vec4f sx =
    v_madd(
      v_madd(
        v_madd(_mm_set1_ps(_SINCOS_SC0), xl2, _mm_set1_ps(_SINCOS_SC1)),
        xl2, _mm_set1_ps(_SINCOS_SC2)),
      xl3, xl);

  vec4i izero = v_cast_vec4i(v_zero());
  vec4i sinMask = v_cmp_eqi(v_andi(offsetSin, (vec4i)V_CI_1), izero);
  vec4i cosMask = v_cmp_eqi(v_andi(offsetCos, (vec4i)V_CI_1), izero);
  s = v_sel(cx, sx, v_cast_vec4f(sinMask));
  c = v_sel(cx, sx, v_cast_vec4f(cosMask));

  sinMask = v_cmp_eqi(v_andi(offsetSin, (vec4i)V_CI_2), izero);
  cosMask = v_cmp_eqi(v_andi(offsetCos, (vec4i)V_CI_2), izero);

  s = v_sel(v_neg(s), s, v_cast_vec4f(sinMask));
  c = v_sel(v_neg(c), c, v_cast_vec4f(cosMask));
}

VECMATH_FINLINE void VECTORCALL v_sincos_x(vec4f va, vec4f& vs, vec4f& vc)
{
  float a, s, c;
  _mm_store_ss(&a, va);
