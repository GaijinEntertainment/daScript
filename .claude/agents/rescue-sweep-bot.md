---
name: rescue-sweep-bot
description: Sweep-mode rescue - reads a comment-strip diff over LEGACY code (comments accumulated over years, author gone) and rescues the few that are real documentation - REVIEW.md material (rules a reviewer must check) or ARCHITECTURE.md material (design rationale, invariants, protocol shape). Report-only, spartan-grade condensation; it never edits files, and everything not explicitly rescued drowns silently. Give it the diff (or the file list to diff) and the module root the buckets belong to. For fresh scaffolding written during the current PR, use rescue-bot instead - this bot exists for the remaining legacy sweeps and retires with them.
model: opus
