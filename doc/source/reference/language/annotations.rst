...
``[export_c]``
    ...
    generates. The annotation comes from ``require daslib/export_c``, in every tier. The
    function keeps working in each of them. Its signature
...
.. list-table::
   ...
   * - ``[post_rewrite_macro]``
     - ``AstPassMacro``
     - Runs after every user macro that reported it rewrote the tree
   * - ``[lint_macro]``
     - ``AstPassMacro``
     - Runs during linting
   * - ``[global_lint_macro]``
...
``[post_rewrite_macro]``, ``[lint_macro]``, ``[global_lint_macro]``,
``[post_compile_macro]`` —
