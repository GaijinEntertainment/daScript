...
The ``typedef`` form declares exactly the same type:

.. das-doc: alt
.. code-block:: das

    typedef U_F = variant<i_value:uint; f_value:float>
...
.. code-block:: das
    ...
    def read_float(u : U_F) : float {
        return u as f_value             // panic when the current case is not f_value
    }
...
.. code-block:: das
    ...
    def read_float_or(u : U_F) : float {
        return u ?as f_value ?? 1.0     // will return 1.0 if u is not f_value
    }
...
.. code-block:: das
    ...
    def reinterpret_case(var u : U_F) : float {
        unsafe {
            u.i_value = 0x3f800000
            return u.f_value            // returns the memory occupied by f_value - i.e. 1.0f
        }
    }
...
.. code-block:: das
    ...
    assert(typeinfo safe_variant_index<unknown_value>(t)==-1)   // -1, no error

``variant_index`` on an unknown case name is a compilation error:

.. das-doc: expect error[30839]
.. code-block:: das

    var u : U_F
    assert(typeinfo variant_index<unknown_value>(u)==-1) // error[30839]: variant unknown_value not found
