...
The rule ships **off**, like LINT029: a ``var`` parameter reads as a write
contract, so library callees that take ``var`` to hand out a pointer or
advance a builder carry the shape. Arm it on a file with
