...
One process runs one daslang library at a time: a second library's
``_create`` returns NULL and says so through its ``_last_error(NULL)``, and so
does a library's ``_create`` in a host that registered the daslang modules
itself. Several instances of one library are fine, on any thread.
