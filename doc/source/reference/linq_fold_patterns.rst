...
.. list-table::
   ...
   * - ``unsafe(from_json(jv, type<Row>))``
     ...
     - In-tree source (JSON is always present) — wired unconditionally, with no ``static_if`` gate: ``extract_json_source`` name-matches ``from_json``, so a JSON-less program returns null and the chain falls to the array tier. The adapter pulls in **no** json dependency — it emits ``from_json`` / ``from_JV`` / ``read_json_field`` by name (resolved at the user's splice site, like ``linq_fold_decs`` emits ``for_each_archetype``). Emits an inlined ``for (e in jv.value as _array)`` walk replacing the generator, and **field-prunes** the per-element materialization (pass 2b): only the keys the chain reads are pulled via ``read_json_field`` (``jv?[key] ?? default``) — unread keys (notably ``string`` fields whose materialization clones) are never touched, so a scalar-only chain skips ~all of ``from_JV``'s per-row work (3.6× over the full ``from_JV`` build — see ``benchmarks/micro/json_source_shapes.das``). A whole-row escape routes to the full ``from_JV(it, type<Row>)`` instead. ``unsafe`` is required (the source is ``[unsafe_outside_of_for]``). Deferred materialization mirrors XML: order/distinct/take buffer a cheap ``(orderKey, JsonValue?)`` surrogate and run ``from_JV`` only for the K survivors — and because ``from_JV`` builds the whole row in one call (no per-attribute walk like XML's ``build_xml_row``), the survivor materialize is a single ``outBind <- from_JV(handle, type<Row>)``.
