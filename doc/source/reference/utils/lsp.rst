...
hover, and document / workspace symbols.
...
Claude Code loads LSP servers from plugins.  The manifest ships in the
repository at ``utils/lsp/plugin/.claude-plugin/plugin.json``; wire it
in with::
...
references, hover, documentSymbol, and workspaceSymbol operations.
...
``$DASLANG_LSP_COMPILER`` → repo layouts
(``bin/Release/daslang[.exe]``, ``bin/daslang``, ``build/daslang``,
``build/bin/daslang``) under the workspace and the plugin's own
repository → ``daslang`` on ``PATH``.  Relative paths are absolutized
at discovery.
