...
directory management (``mkdir``, ``dir``), path manipulation (``path_join``,
``base_name``, ``dir_name``), and file metadata queries (``stat``, ``file_size``).
...
* ``tilde_home(text) : string`` — every home directory in ``text`` (``/Users/<name>/``, ``/home/<name>/``, ``<drive>:\Users\<name>\`` or ``<drive>:/Users/<name>/``) rewritten as ``~/`` wherever it starts a path: the path under the home survives, the user name does not; a bare home with no separator after the name stays.
