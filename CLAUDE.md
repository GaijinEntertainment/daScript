...
- **Newlines inside `(...)`, `[...]`, `{...}` are free** — long pipe chains, multi-arg calls, array/table literals can wrap freely. Statement-level (no surrounding bracket) still requires one statement per line, so wrap the RHS in `(...)` if a `let x = a |> b |> c` needs to break across lines. **DANGER — silent, no error:** without the parens, a continuation line starting with a binary operator (`+ b`, `|> f()`) parses as a *separate* expression statement; if it's pure (side-effect-free) the optimizer **silently deletes it**. So `let x = a` ⏎ `+ b` ⏎ `+ c` becomes `x = a` with `+ b`/`+ c` dropped — terms vanish, wrong result, no diagnostic. Always wrap a multi-line arithmetic RHS in `(...)`
