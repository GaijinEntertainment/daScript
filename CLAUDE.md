...
- `include/vecmath/` - a vendored copy of the standalone header repo `https://github.com/GaijinEntertainment/vecmath`, NOT of DagorEngine's `prog/1stPartyLibs/vecmath` (that tree consumes the same headers). A backend or fix written here goes upstream as a PR to the vecmath repo, with its `README.md`/`CLAUDE.md` rows and CRLF line endings; a sync pulls that repo's master back. `tests-cpp/big/vecmath_backend/` is the primitive-contract battery every arm must pass
