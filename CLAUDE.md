...
- **The finalize banner (E4.5): `erase`/`erase_if`/`clear`/shrinking `resize`/`pop` (arrays) and `erase`/`clear` (tables) FINALIZE the elements they drop** — gated on `typeinfo needs_container_finalize`, i.e. `default_init_containers && need_delete && is_safe_to_delete && !is_const`; the same policy governs both halves, so turning it off restores construct-and-finalize behavior together. `is_safe_to_delete` is false for raw pointers, lambdas, blocks, and any composite carrying one, so those keep the old drop-the-slot behavior: clear-then-delete on `array<T?>` stays the borrowed-pointer idiom, and slot-erase of pointer-carrying tuples (the linq-planner shape) never deletes pointees. **delete = ownership teardown, erase/clear = release** — the asymmetry is the design.
...
- **`label`, `expect`, `pass`, `explicit` and `capture` are reserved words** (lexer keywords `DAS_LABEL`/`DAS_EXPECT`/`DAS_PASS`/`DAS_EXPLICIT`/`DAS_CAPTURE` — `pass` is the no-op statement, `explicit` the generic-parameter modifier you see in `array<T> explicit` signatures) — using any as a parameter/variable name is a syntax error (`capture` reports `unexpected capture, expecting $i or name`); rename (`tag`, `want`, `cpass`, `declared`, `grab`)
...
- **`options` are MODULE-LOCAL for pass-macros** (`[lint_macro]` / `AstPassMacro`). The macro fires once per module in the require chain, reading `prog._options` from THAT module's options table — not the program-root's. So `options _my_lint_off = true` in `foo.das` suppresses YOUR lint in `foo`, but `require foo` from `bar.das` does not inherit the flag — `bar` gets linted unless it sets its own. Don't confuse with runtime options (`multiple_contexts`, `persistent_heap`, `rtti`) which DO unify across the program codegen and effectively cascade up to consumers. **`options gc` is the exception: it does NOT cascade from a required module** — `heap_collect` stays illegal unless the ENTRY program declares `options gc` itself (probe-verified 2026-07-30; a module-side `options gc`, e.g. in a harness, buys consumers nothing)
