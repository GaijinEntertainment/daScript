...
| `slice(s, i, j)` / `chop(s, i, n)` in a loop where `s` is defined outside the loop | walk bytes via `peek_data`, or hoist the slice | PERF031: each call re-strlens the WHOLE source (das strings cache no length) and leaks a temp string — O(n²) loops. Loop-element receivers stay silent. Repo `.lint_config` keeps it off until the ~127-hit in-tree sweep lands; ON for SDK users |
