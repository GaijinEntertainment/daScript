...
| `skills/internal/daslang_internals.md` | Reasoning about how the compiler implements something no user-facing doc covers — the container collect banner and `is_pod_delete` gating, the `/*option*/` policy marker, AST-versus-runtime fixed arrays, `addr<T?>` desugaring, `isWorkhorseType`, known mangling defects |
...
| `skills/environment_variables.md` | Reading or ADDING any environment variable — the full daslang set, the read-once/typed-default rules a new one must follow, the C++ `env_cfg` accessor rail; dasLLAMA's knobs are generated into its own `ENVIRONMENT.md` |
