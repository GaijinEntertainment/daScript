...
| `b == uint('(')` where `b : uint` (also `!=`, ranges, and Yoda forms) | make `b : int`, then write `b == '('` | STYLE035: character literals are `int`; changing the plain variable once removes every repeated `uint('<char>')` cast. The warning is deduplicated at the variable declaration. |
