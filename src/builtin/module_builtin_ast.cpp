...
namespace das {
    ...
    void builtin_structure_for_each_field ( const BasicStructureAnnotation & ann,
        const TBlock<void,char *,char*,smart_ptr<TypeDecl>,uint32_t> & block, Context * context, LineInfoArg * at ) {
        for ( auto & it : ann.fields ) {
            const auto & fld = it.second;
            das_invoke<void>::invoke<const char *,const char *,smart_ptr<TypeDecl>,uint32_t>(context,at,block,
