...
- **A plain-value bind added or changed under this folder uses `addExternInline` or
  ...
  `das_equ*`, `das_nequ*`, `das_handle_equ*`, `das_handle_nequ*`). Inline-module binds run
  inside interpreted inner loops, so each gets its own node the callee inlines into, while
  binds elsewhere share one node per signature to keep binaries small.
