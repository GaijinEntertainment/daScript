...
- [ ] **PR D2** — `plan_zip` + `plan_decs_join` migrations; SourceAdapter widening to Array | Decs | DecsJoin | Zip; full GroupBySourceAdapter unification
...
| **D2** | 4b — Special cases | `plan_zip` (1-2 rows, `SourceAdapter::Zip`). `plan_decs_join` (1 row, `SourceAdapter::DecsJoin`). Full GroupBySourceAdapter unification once DecsJoin variant exists. | not started |
...
- **`plan_zip` / `plan_decs_join` SourceAdapter shape** — defer to PR D2.
