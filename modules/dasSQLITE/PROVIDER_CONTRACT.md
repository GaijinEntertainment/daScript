...
| `readTableSchema(path, tbl)` | readonly open + `PRAGMA table_xinfo` (generated columns included, virtual-table hidden columns filtered) + affinity rules | `[sql_table(schema_from=...)]` compile-time introspection. Returns `Result<array<SchemaFromCol>, string>` with provider-normalized `data_type : SqlType`; `null` when the provider has no file-based schema source. Annotation time has no runner, so exactly one registered provider may supply the hook (ambiguity is a macro error until a `provider=` annotation arg exists) |
