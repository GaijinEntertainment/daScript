...
doc: `../ARCHITECTURE.md`. Planned work: `../ROADMAP.md`.

**A diff that adds a name to `skip_report.txt` names in its commit message the generated wrapper
that still exposes the newly skipped struct or command - or states that none does.**
...
**A diff that adds a name to `copyability_report.txt` states in its commit message that every
