...
`CLAUDE.md`. A dasImgui test file - a `.das` file that declares a `[test]` function or is named
`test_*.das`, `failed_imgui_*.das`, or `record_*.das` - wherever the diff puts it, also answers
to the `tests/` subfolder's checklist (`modules/dasImgui/tests/REVIEW.md`).
