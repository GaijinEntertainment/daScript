...
reaches — nothing runs the whole directory.** The suites `run.das` lists (decode, prefill, matrix,
...
registration ships a test for the bit itself** — feed the function, check the bytes; "the model still
...
**`performance/fetch_models.das` is the model-provenance manifest.** Per file: the HF repo + revision
pin, canonical bytes + sha256, or a conversion recipe. Verify is the default; `--fetch` downloads what
is absent and never converts, benches, or touches tune state (`tune_policy(missing="fallback")`). Its
gate (`BRINGUP.md` §2): `fetch_models.das --` ends `0 failed`.
...
- `ENVIRONMENT.md` — generated by `harness/gen_env_doc.das` from `dasllama_env.das`'s `[EnvConfig]`
  declarations AND `daslib/env_registry`'s, so a daslib renderer change regenerates it
  (`tests/test_env_registry.das` fails on drift). Hand-editing it is a defect, as is editing
...
**Peak memory wins ties against load cost.** A load-time change trading footprint for speed ships the
...
**Every new kernel or mid-runtime loop is COVERED by `[hot_path]`.** A region entry is a KERNEL
...
kernel-backend override or a batch donor, carries the annotation itself (`ARCHITECTURE.md` §2.11).
...
`env_config_*` elsewhere is a defect. Sanctioned forms are `ARCHITECTURE.md` §2.9; markers and
scanners live in `daslib/env_registry`, so a change THERE re-runs `tests/test_env_registry.das`.
