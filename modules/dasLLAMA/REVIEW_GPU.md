...
carries `[metal_dispatch]` / `[vk_dispatch]` with every annotation the generated builder
reads - per-field `@binding` / `@role` / `@off` / `@span` / `@default`, `@workgroup` state
with its `tgmem=` dispatch key. A NEW `enc_*` body is hand-written only as a wrapper - a
format or twin pick, a default-filling wrapper, or a composite over generated builders; any
other hand-written `enc_*` body is a defect.
...
**A change to CODE a served GPU decode or prefill path executes - a driver
...
by instances of one template, or single-codec with a sibling serving the other codec behind
an arming gate that keys on `kv16`. The rail serves both codecs, so a codec no kernel covers
silently drops that codec's GPU path.
