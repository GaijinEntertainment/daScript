...
1. **One vision carrier, the ASR shape**: `enum VisionKind { none uv v }` +
   ...
   (qwen3vl, v2+ ledger) adds an arm, not a seam. Rejected: per-seam overloads (x2 today, x3
...
tensors - vision 658 / 167 M params / 16 blocks + `v.patch_embd [16x16x3x768] F32` +
`v.position_embd [768x10240x2] F32` + `mm.input_projection [768x1536] BF16`; audio 751 (ported).
...
(`attn_q/k/v/out [768^2]`, `ffn_gate/up [768x3072]`, `ffn_down [3072x768]`) each with four F32
...
0. **Preprocess = v1's, verbatim**: align 48 (patch 16 x n_merge 3), min/max pixels
   40/280 x 2304, PAD_CEIL letterbox on black, align-corners bilinear (u8 truncating),
...
1. **Stem**: `x = px x 2 - 1` (in-graph; v1's normalize stays x/255) -> conv2d patch 16 stride
   16 NO bias = im2col (`j = c*256 + ky*16 + kx`, v1's `uv_im2col` with `p = 16`) x GEMM over
   `v.patch_embd` (F32 on disk) -> `[n_patches x 768]` -> `+ pos_x table row + pos_y table row`
   (two 10240-entry tables, no pos-norm).
2. **16 pre-norm RMS blocks**, eps 1e-6, RMS = `x / sqrt(mean(x^2)+eps)` x w (no mean
   ...
   -> **2D NEOX rope**: head dim 64 = first 32 by `pos_x`, second 32 by `pos_y`, theta = 100,
   `theta_j = p * 100^(-j/16)` over NEOX pairs (j, j+16) within each half -> **V = rms(V)
   weightless, per head** -> attention with **`kq_scale = 1.0`** (not 1/sqrtd), full bidirectional,
...
6. **Splice / decode**: identical to v1 - `<|image>` + N rows + `<image|>`, media-first,
   sequential positions, the non-causal span, **no decoder-side rescale** (gemma4's `xsqrtn_embd`
   applies to the token path only).

The three parity traps, named: `kq_scale = 1.0`; the rope halves (x first, y second) with
theta=100; V's weightless per-head RMS. The fourth is the oracle itself: CLIP's flash-attention
...
- `dasllama_gemma4v.das` (new): `Gemma4vTower` - hparams, planes (`PlaneF` f32: patch_embd,
  pos tables, norms, clamp scalars; `PlaneU16` bf16: the 7x16 block GEMMs + `mm.input_projection`
  ...
  contract as `gemma4uv_encode` (`[npos x proj_dim]`), a `register_gemma4v_gpu` hook slot.
- Shared tower helpers, hoisted or added (behavior-pinned by the tests of the families that
  ...
  1/sqrtd); a 2-axis rope table builder (`build_rope_tabs_rows` twice, one per half) + a
...
- **A. Oracle rig + pins**: E2B F32 twin, `-fa off` encode dumps at the sizes above, the
  per-node sum ladder for 96^2, meta/token facts pinned here; predictions below scored
...
- **C. The tower**: `dasllama_gemma4v.das` stage/load/forward; tier-1 against the 96^2 ladder
...
- P1 (tier-1 fidelity): against the f32-twin `-fa off` oracle, the 96^2 per-token mean/v0..v3
  ...
  **Scored at slice C:** measured 1.9e-4 (96^2 cb) ... 9.4e-4 (blue 336^2) across the eight
  fixtures - the band held except the 96^2 floor (1.9e-4 sits just under 3e-4; the uv 2e-4 would
  have held there and nowhere else). Bar set at 2e-4 + 4e-3*rms (~3.2e-3): 3.4x headroom.
- P2 (cost): CPU encode of 624x480 (1170 patches -> 130 tokens) on the M1 Max lands in
  ...
  **Scored at slice F:** the exact-plane CPU encode ~ 1.9 s on the `-jit` probe (just above
  the band - the bf16 per-row dot path runs ~92 GMAC/s, not the ~200 the band assumed),
  ~ 60 % of the turn. Yes, loudly: the Metal leg is on, AND the q8 CPU lane the out-of-scope
  list held back "unless slice F demands it" is demanded. Record-grade (the released
  `lcpp_bench` exe, after G0/G): CPU row (q8 lane) `img:enc` 419 ms, `--accel` row (exact
  planes on the +AMX tier) 459 ms - the PERF_LEDGER entry carries the protocol and the rest.
...
- P4 (first red): the first tier-1 red after the stem passes is in the rope halves or
  `kq_scale`, not in the norms - and the 96^2 node ladder localizes it to block 0 within one
  run.
...
- P7 (the q8 lane, registered before its first run - slice G0): with the 112 block GEMMs as
  ...
  maxdiff vs the f32-twin oracle lands in **5e-3 ... 3e-2** (16 layers of q8xq8 noise on
  ...
  **Scored at slice G0:** fidelity MISSED by 2x - maxdiff 3.1e-2 (96^2 cb) ... 6.8e-2 (green
  336^2), ~9 % of token rms on the worst sampled element (bar set at 1.2e-1*rms); the caption
...
- P8 (the Metal leg, registered before its first run - slice G): the block loop through the
  ...
  the 640x480 encode at **<= 120 ms** on the M1 Max (the 112 f16-tile GEMMs ~ 40 ms, the rest
  ...
  **Scored at slice G:** 89 ms (21x over the exact CPU lane, 4.8x over q8); tier-1 GPU
  8.9e-4 ... 3.7e-3 - the 4e-3*rms rung held on 4 of 5 (green 336^2 at 3.7e-3 breached it by
