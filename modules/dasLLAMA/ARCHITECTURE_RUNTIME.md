...
- **The fused add-rms+requant twin quantizes the consumer's block form per site** (`rq_kind`:
  Q8_0 or Q8_K, the normed row also stored where a consumer reads it as floats), picked once
  every layer is registered (`rd_fused_sets`); the profiler keys on the stamps' recorded names,
  so no count rides the gate.
