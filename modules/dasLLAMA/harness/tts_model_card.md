...
| `tts_g2p.bin` | the grapheme-to-phoneme pack: misaki's gold and silver US-English lexicons, CMUdict 0.7a rendered into the same inventory, the g2p_en GRU spelling model | 12719239 | `e537a9a4d7dc8441d7a2f396c7ef954d22f406f766c8181d1e55f0e27d38ceaf` |
...
names (`Bella`, `Jasper`, ...) are accepted for them; Kokoro's are its 54 packs (`af_heart`,
`bf_emma`, `am_adam`, ...). Every model speaks at 24 kHz. `GET /v1/stats` on the server lists
...
- `tts_g2p.bin`: `modules/dasLLAMA/harness/build_g2p_data.py` over misaki 0.9.4 (`us_gold.json`,
  `us_silver.json`), CMUdict 0.7a (the NLTK distribution) and g2p_en 2.1.0 (`checkpoint20.npz`),
  plus `harness/g2p_local_additions.json`.
