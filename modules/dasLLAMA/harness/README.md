...
| `oracle/simple_ids.cpp` | The reference oracle - a trimmed lcpp `examples/simple` that prints greedy token IDs (CPU, no EOG break) instead of text. |
...
from `simple_ids` (the same method the arc used); the prompt is tokenized once by lcpp and the
exact IDs are fed to both sides.

### One-time: build the oracle

`simple_ids` links against an existing lcpp build. From the lcpp checkout:
...
so the parity gate runs without lcpp/the oracle at test time. The GGUF models themselves are
...
- **It stays a manual harness check, not a CI fixture** - it needs a hand-built >4k prompt and
  the oracle binary beside a lcpp build.
