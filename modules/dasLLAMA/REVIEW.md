...
**Upstream code lifted into this module is described by mechanism, not attributed** - no
"lifted/ported verbatim from" and no upstream symbol, header, or constant names in a `.md`
file or a `.das` comment; state what the code does and why its shape wins. (The name half of
this - the banned literal - is `REVIEW.das`'s `check_banned_name`; this rule is the half no
gate can see.)
