...
rows, each naming its arm, in the PR body or in the change's dated `PERF_LEDGER.md` row.** A
reading taken across two processes or two commits says which way the wall clock moved, not which
implementation to adopt.

**A change to an allocation whose size scales with a model dimension, a row count or a region
count, added, grown or removed to move wall-clock, wherever the allocation sits, ships the
