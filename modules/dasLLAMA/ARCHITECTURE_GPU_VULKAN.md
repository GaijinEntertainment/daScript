...
`khr_cls_enc` in `dasllama_vulkan_prefill.das`, licensing `Q51Cm2T` alone - q51 is a routed-expert plane and the
resident MoE block is cm2-gated, while q8, the other per-32 format, rides `Q8KhrBatch` (`q8_batch_khr_cls`) on a KHR-mode card;
