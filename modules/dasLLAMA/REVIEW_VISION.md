...
(`eval_embd_span_` / `forward_prefill_embd`'s span bounds / `encode_image_`), a media-carrying
...
loop, or splitting command buffers per layer, is not a split; the fused turn (head + rows +
tail in one eval) and the splice fallback (the rows as their own call) both satisfy this rule.
