...
record stores and tune sidecars — and stays engine-free.** A second validator, or an engine
require added to it, is a defect (the macro-only `dasllama_lint` carrier is not one).

**A field added to what `write_bench_records` (`profile_common.das`) writes is added to
`dasllama_exchange_schema.das`'s run validation in the same change** — the validator ignores run keys
