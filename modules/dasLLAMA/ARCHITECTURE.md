...
- **A GPU backend is a FAMILY of role files — matching things in matching files across
  ...
  necessity: metal_common → dasllama_common → metal_gemm would cycle; renamed from the
  misleading `dasllama_math_metal`, M8). Backend-only capabilities (vulkan: weight arena, streamed mirrors, heat
