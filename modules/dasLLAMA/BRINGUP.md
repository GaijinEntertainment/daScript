...
Order: build -> mint -> sweep (the board bakes its own images; pre-bake is for sec.4 converter
workflows).
...
- **A verdict on a long board's tail cells is not evidence - re-run the cell solo.** Three
  ...
  exceed the fail bar by 4x; a tail FAIL is a re-run instruction, never a verdict.
  Discriminating an
