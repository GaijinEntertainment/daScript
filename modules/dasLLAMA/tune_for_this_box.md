...
| Pool width (total lanes) | `set_jobque_threads(n)` / profile `runtime.jobque_pool` - TOTAL compute lanes (n-1 pool workers + the computing main), the request the hybrid `[init]` policy raises on a box whose second tier is compute-grade. A profile declares the shape of the NEXT queue and is inert for one that already exists when the profile loads, so it belongs in the box profile, not in a running session. `DAS_JOBQUE_THREADS` and an app's own cap both outrank it | queue creation (`with_job_que`) | the whole jobque worker pool |
| Decode-phase worker limit | `set_phase_decode_workers(n)` (-1 = per-phase parking off) / profile `runtime.phase_decode_workers` - how many workers stay awake for a DECODE step; `dispatch_phase_batch` wakes the full pool again for a batch of 32 positions or more. The hybrid dial: chunk-starved slow-tier workers spinning beside the fast decode lanes cost ~15% of tg on an M5 Max, and the rank gate does not recover it - only parking does. **A value < 1 is refused** (0 would park every worker) - the setter warns and leaves parking disabled. Declared in the same profile block as `jobque_pool`, which shapes the NEXT queue, so the pair is a box-profile decision rather than a mid-session one. Inert while `dispatch_worker_limit` is set, that static limit winning | runtime (per dispatch phase) | the jobque worker limit around every decode step |
...
| Metal prefill raced constants | `set_metal_tall_floor` (default 64, the tall M-tile occupancy floor in 128x64 threadgroups; the MoE tall arms carry their own rows-per-expert >= 128 gate, a different quantity this knob does not reach) / `set_metal_devw_small_panel_mb` (default 32, the dev-W dequant-panel engage knee; the 48MB allocation cap stays compiled) / `set_metal_cvt_min_rows` (default 256, the half-X convert row floor - raising it changes which sites run f16 X, a numerics form) / profile `runtime.metal_tall_floor`, `runtime.metal_devw_small_panel_mb`, `runtime.metal_cvt_min_rows`. Setters refuse values < 1 with a warning. CACHED at `metal_prefill_init` like the tensor crowns - a change after the first prefill needs a shutdown/re-init | runtime (cached at Metal prefill bring-up) | the Metal prefill form picks (tall/dev-W/half-X) |
...
| Batch oversplit | `set_q8_batch_chunks_per_job` (default 4) / profile `runtime.q8_batch_chunks_per_job` - chunks = hw_jobs x this, so awake workers steal a straggler's remaining chunks (measured 3990X @32w: +9-14% on the fat shapes; the grp4 kernel floors at 4 groups/chunk - see the kernel note) | runtime | the x64 batch (prefill GEMM) kernels |
| Jobque spin | `set_jobque_spin_us` (default 30000 on desktop and browser; a box profile or application can override it; worker spin-before-park) / `set_jobque_spin_gpu_us` (default 500 - the window while the whole-model GPU driver serves the loaded model, where the pool carries only the sampler between steps and a spinning worker starves the driver's submit; the driver's arm and drop push it to a live queue) / `set_jobque_join_poll` (default 50, poll denomination: levelx1024x128 relax-rounds at join before parking) / profile `runtime.jobque_spin_us`, `runtime.jobque_spin_gpu_us`, `runtime.jobque_join_poll` - all applied by `setup_dasllama_jobque` inside the queue | runtime | every fork/join dispatch |
...
| Speculative depth per round kind | `set_mtp_depth_nextn(k)` / `set_mtp_depth_assistant(k)` (default 1 each) / profile `runtime.mtp_depth_nextn` / `runtime.mtp_depth_assistant`; `set_mtp_depth(k)` (the `--mtp-depth` flag) is the explicit override and `clear_mtp_depth()` hands the knobs back. The assistant knob is MINTED: the tuner races depth 1 vs 2 as serving tg on the SpecBench chat corpus with a gemma-4 vehicle and its `mtp-` head beside it (`DASLLAMA_CONFIRM_MTP_GEMMA` pins one); depth 2 must beat by 2% - its downside is asymmetric and a tie is not worth the longer round | runtime | `get_mtp_depth()` at every speculative round; `MtpRoundKind` is set by the assistant drafter's attach and detach |
| Metal twin crowns | profile `runtime.metal_tensor` - the comma list of families whose twin won this box's race (`metal_tensor_race` for the prefill mul_mm twins, `metal_tensor_race_decode` for the batch GEMM twins, the iquant f4 GEMV twins and `kq_rows_<fmt>`: the K-quant small-batch form where B single-row passes beat the two-row twin, an M4 Pro k4 verdict; `kq_mvb2_k4_r2`: the k4 twin is its two-row register tile, again the M4 Pro's). Runtime knobs never travel with the class defaults; a box without a mint serves no crown | runtime | `metal_tensor_crowned(family)` at PSO compile / dispatch (`enc_kq_site_b` reads `kq_rows_*` once in `metal_decode_init`) |

Axes are ~independent (unroll _|_ width _|_ TB _|_ threads) - sweep them separately
...
7. **Never benchmark interpreted - always `-jit`.** If a number looks ~100x off, check the flag
...
(`{plain, u2, u4, u8}  union  {vec4,vec8,vec16,vec32} x {-, u2, u4, u8}`). One protocol, margin-decided:

- **Windows of ~20 ms.** One unscored warm pass times each row at the base 2000 reps and scales
  its reps (up to 32x) so a timed window lands near 20 ms; every recorded sample is normalized
...
requant), reports each variant as %Delta vs that kernel's SHIPPED fallback perm (`vec8_u2` for
...
Pins the repack backend, repacks, sweeps `TB  in  {128..2048}` x output-width `n`, interleaved,
...
(`ntok x n` floats) spills the cache that serves re-streaming; the optimum is **not** a peak
at L2/n but a **cliff** - TB* sits at the small end (128) for every realistic `n`, and large
TB falls off hard once `TB x n` spills (+13% -> +46%); the knee moves down as `n` grows.
...
Production `matmul_q8q8_batch` on the real Llama shapes (qo 2048x2048, w13 2048x5632,
w2 5632x2048, cls 2048x32000) at ntok=512, threaded, GMAC/s aggregate + per-core, both
...
## Tool 4 - per-op profiling (ours and the reference's)
...
- **Reference side (the recipe that found the M1 gap, now shipped):** `harness/ggml_op_profile.patch` -
  apply to the reference checkout with `git apply --ignore-whitespace` and rebuild. It times
  thread-0 wall per graph node in the compute thread into buckets NAMED LIKE OURS
  ...
  ~1.17x on the repack-less quantized GEMVs + 1.66x on the small `mm_wo` - profile before optimizing.
...
1. Thread sweep first (`DAS_JOBQUE_THREADS` can't be swept in-process - the worker count is
   fixed at jobque creation, so it's one process per value: `DAS_JOBQUE_THREADS=N` x
...
**When to stop:** GEMM at isolated parity with upstream is the floor, not a lever; <=2-3%
candidates are noise; on M1 the campaign ended at ~100-108% of upstream prefill with the
