...
| Jobque spin | `set_jobque_spin_us` (default 30000 on a desktop, 0 in a browser - a spinning web worker costs the caller its core; worker spin-before-park) / `set_jobque_join_poll` (default 50, poll denomination: levelx1024x128 relax-rounds at join before parking) / profile `runtime.jobque_spin_us`, `runtime.jobque_join_poll` - both applied by `setup_dasllama_jobque` inside the queue | runtime | every fork/join dispatch |
