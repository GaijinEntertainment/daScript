...
| Metal twin crowns | profile `runtime.metal_tensor` - the comma list of families whose twin won this box's race (`metal_tensor_race` for the prefill mul_mm twins, `metal_tensor_race_decode` for the batch GEMM twins, the iquant f4 GEMV twins and `kq_rows_<fmt>`: the K-quant small-batch form where B single-row passes beat the two-row twin, an M4 Pro k4 verdict). Runtime knobs never travel with the class defaults; a box without a mint serves no crown | runtime | `metal_tensor_crowned(family)` at PSO compile / dispatch (`enc_kq_site_b` reads `kq_rows_*` once in `metal_decode_init`) |
