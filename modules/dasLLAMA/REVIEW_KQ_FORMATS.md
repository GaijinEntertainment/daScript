...
**A per-format byte or stride-unit count of a `KqFmt` under `modules/dasLLAMA/` is read off the
`kq_desc` row through a `dasllama/dasllama_kqformat.das` accessor of that row (`kq_qsb`, `kq_ssb`,
`kq_elems`, `kq_sb`, ...), through `kq_qsb` / `kq_ssb` on a format id
(`dasllama/dasllama_gemm_schema.das`), or through `kq_disk_bytes` (`dasllama/dasllama_gguf.das`) -
never written as a literal. The two homes a literal per-format count lives in are the `kq_desc` row
