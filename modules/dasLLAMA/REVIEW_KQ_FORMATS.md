...
column, that member's `ggml_type_bytes` entry or a named per-format constant defines for it - written
anywhere under `modules/dasLLAMA/` outside `dasllama/dasllama_kqformat.das` (the `kq_desc` rows
and the named stride and stream-tag constants they read), the `ggml_type_bytes` table
...
a format id, or through a named constant one of those files declares.** A hand-copied value
drifts from the definition it restates.
