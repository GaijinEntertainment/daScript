...
| `DASLLAMA_METAL_TOWER_F16` | flag | on | Serve the qwen3v tower's block/merger GEMMs from a once-converted f16 weight shadow on the crowned tensor mul_mm twins (the file's GEMM tensors are f16, so the shadow holds their exact values; needs a tensor crown - a box without one keeps the f32 route); 0 pins the f32 GEMM route - the A/B rail. |
