...
| `DASLLAMA_VK_FUSE_BISECT` | number | 0 | Keeps a fused form of the one-row token command on its split dispatches, a same-build A/B whose logits stay right - bits 1 = the wo GEMV's residual epilogue (`Q8GemvAr`), 2 = the down GEMV's, 4 = the fused gate-up twin, 8 = the prologue's add+rms+requant stamp, 16 = the post-attention one (and the wo epilogue, which would add the residual twice), 32 = the post-FFN one (and the down epilogue), 64 = the fused qk-norm+rope; `DASLLAMA_VK_FUSE=0` pins every site at once. |
| `DASLLAMA_VK_NROW_BISECT` | number | 0 | `benchmarks/lcpp_bench.das --npl N` only: drops a class of dispatch from the recorded N-row token command so a profile prices it - bits 1 = no attention, 2 = the one-column GEMVs (row 0 alone), 4 = no rope, 8 = no activation requant; the logits are garbage under any bit. |
