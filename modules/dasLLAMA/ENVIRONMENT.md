...
| `DASLLAMA_GPU_CTX_MAX` | number | the model's context | Cap the resident driver's KV mirror at this many positions (the card's room and the 4 GiB binding cap still bound it); a cap under the arming floor lowers the floor to the cap, since the cap is the shape the caller serves; 0 follows the model's context; a server keeps the model's. |
...
| `DASLLAMA_VK_NROW_BISECT` | number | 0 | `benchmarks/lcpp_bench.das --npl N` only: drops a class of dispatch from the recorded N-row token command so a profile prices it - bits 1 = no attention, 2 = the one-column GEMVs (row 0 alone), 4 = no q/k norm and no rope, 8 = no activation requant; the logits are garbage under any bit. |
