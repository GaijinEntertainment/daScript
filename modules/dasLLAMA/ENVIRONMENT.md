...
| `DASLLAMA_VK_FUSE_BISECT` | number | 0 | Keeps a fused form of the token command on its split dispatches (the N-row command shares the gate-up, the two epilogues and the fused q/k norm through the layer flags it reads at load; a change needs a reload), a same-build A/B whose logits stay right - bits 1 = the wo GEMV's residual epilogue (`Q8GemvAr`), 2 = the down GEMV's, 4 = the fused gate-up twin, 8 = the prologue's add+rms+requant stamp, 16 = the post-attention one (and the wo epilogue, which would add the residual twice), 32 = the post-FFN one (and the down epilogue), 64 = the fused qk-norm+rope; `DASLLAMA_VK_FUSE=0` pins every site at once. |
