...
| `DASLLAMA_VK_FUSE` | flag | on | Fused add+rms+requant on every site of the decode token command, every model (Q8_0 or Q8_K by the consumer, the normed row stored where a head or router reads it; plus qk-norm+rope) AND the prefill batch ar+rq pairs; 0 pins the split dispatches for a same-build A/B. |
