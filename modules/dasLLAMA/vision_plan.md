...
Oracle = lcpp mtmd: `llama-mtmd-cli` end-to-end (temp 0), `llama-mtmd-debug -p preproc /
...
2. **Resize**: ONE das implementation adopting mtmd's conventions - corner-aligned ratio
   ...
   Geometry policy ported exactly (it is the model contract, not lcpp parity).
...
2. **Letterbox resize** (`img_tool::resize`, PAD_CEIL default, pad color BLACK - a slice-A
   ...
   min(tw/sw, th/sh) as float; content size = min(ceil(sidexscale), target) per side;
   ...
   probe: gray 480x800->480x816 has BLACK rows top+bottom, mean exactly x800/816; white
   3000x2000->960x624 content is 936x624 with 12 black cols each side (mean 0.975 =
   936/960).
3. **Normalize**: u8/255 exactly - `image_mean = 0`, `image_std = 1` (identity, verified),
   NO x2-1 anywhere in the uv path (gray 128 -> 0.501960814 in the dump).
4. **Embedder** (the whole "tower" - ten tensors read, 52M, no attention/rope/clamps):
   im2col 48x48 stride 48 -> rows of 6912 (=48x48x3, interleaved RGB row-major) ->
   ...
   **weightless** rms_norm(eps 1e-6 from meta) -> `mm.input_projection` 3840x3840, no bias.
   Two eps values in one graph - parity trap. Timing datum (dasprof, M1 CPU bf16,
   130 tok): ~245 ms, 95% in the 3840x3840 proj.
5. **Splice**: head + `<|image>`(id 255999) + N embeddings + `<image|>`(id 258882) + text
   ...
   `tokenizer.ggml.suppress_tokens = [258883, 258882]` - lcpp never SAMPLES the end
...
6. **Eval**: image rows via `eval_embd_` WITH the non-causal span flag - the causal UPPER
   ...
   SWA-layer span query attends `[sliding-window floor, span_end)`. That matches lcpp's
...
output). The rig needed a LOCAL lcpp patch (committed in that tree, dasprof-style):
...
- **Tier 0, preproc** (minted: gray/white/black/cb 640, cb 800x480, gray 480x800, cb 100
  upscale, white 3000x2000 downscale): das letterbox output must match the dump's fnv1a64
...
- **Tier 1, embedder** (minted: `-p encode` on gray/rainbow 480, rainbow 672x336, cb/red/
...
- **Tier 2, pipeline** (minted: `cli.cats.log`): `llama-mtmd-cli --temp 0 --jinja` (the
  ...
  (`coco-val2017-000000039769.jpg`, sha-pinned, 640x480->130 tokens); greedy ids under the
  `DASLLAMA_IDS:` marker; prompt stream shape pinned in The-pipeline sec.5.
...
- **A. Oracle rig** (DONE 2026-08-14): mmproj fetched + pinned; sibling check resolved the
  inverted family map (this plan corrected in place); local lcpp debug patch; 16
...
- **G. Embedder planes** (re-measured 2026-08-14 at product shape; dlim rail STILL OWED):
  ...
  the image rail, 640x480 -> 130 soft tokens, 14 t/s decode). Encode alone ~54 ms @ 130 tok,
...
4. **`roundi` rounds ties AWAY**, matching `std::round` - `trunc(x + copysign(0.5, x))` on
   ...
   the workaround was the bug risk, not the fix. Only 120x960 distinguishes the two.
...
7. **A measurement states its serving format, or it measures a format nobody serves.**
   ...
   serving reads 14 - a 7x gap with nothing to do with the code under test. The same script
...
- **E-series `gemma4v` ViT** (the on-disk E2B mmproj carries it, paired with the E2B
  ...
  `models/gemma4v.cpp`, kept from the original plan: x2-1 scale_bias in-graph -> conv2d
  patch embed no-bias (patch 16, n_merge 3 pooling) -> learned x/y pos tables -> pre-norm
  RMS ViT blocks (16 x 768-wide, ffn 3072, 12 heads) with 2D NEOX rope theta=100 (first half
  of head dims by pos_x, second by pos_y), **V rms_norm every layer** (weightless - no
  attn_v_norm tensor), q/k weighted norms, **kq_scale = 1.0 NOT 1/sqrtd** - the two parity
  traps; avg_pool k3 s3 -> xsqrtn_embd pooler; optional standardize tensors; rms_norm ->
  ...
  **audio** 751 tensors / 305 M params / **12 blocks** (macaron FFN pair, conv module
  ...
  64-wide q/k norms, ffn gate/up/down, ln1/ln2, post-norms) plus `v.patch_embd` [16x16x3x768]
  and `v.position_embd` [768x10240x2]; **projections** `mm.input_projection` 768->1536 and
  ...
  **On "the E-series is small" - it is not, and the name is why it reads that way.** The `E`
  ...
  `per_layer_token_embd` [8960x262144] = 2.35 B ==> ~2.3 B compute-carrying ("E2B"); E4B = 7.52 B
  total, PLE [10752x262144] = 2.82 B ==> ~4.7 B ("E4B"). So E4B is a **7.5 B-parameter file**.
  The towers are not carved out of that budget either - the mmproj is a separate ~990 MB.
  For scale, the audio tower (12 blocks x d_model 1024, FFN 4096) is larger than Whisper-small's
  encoder (12 x 768): a serious ASR front end, not a token one.
...
- **Batched image serving** - the full rail: a prompt-segment type (tokens | embedding rows
  ...
  build this speculatively. Worth knowing if it ever does: lcpp toggles causality
