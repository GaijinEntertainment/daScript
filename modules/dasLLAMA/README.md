...
| **Qwen3.8-27B** | Q4_K_M GGUF (native K-quant planes) | Qwen3.5 (arch `qwen35` dense at scale: 48 Gated-DeltaNet recurrent + 16 gated full-attention layers — 24Q/4KV heads at 256, 48 v-heads x 128 DeltaNet, partial NEOX rope 64/256, QK-norm) | BPE (`qwen35` pre) | ✅ | `llama.cpp` `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the counting continuation, per-token and chunked-DeltaNet prefill (frozen large-tier fixture in `test_parity.das`) |
