...
dasLLAMA is an **LLM + speech stack written entirely in daslang** — no C++ kernels, no
hand-written assembly, on CPU *and* GPU. The daslang JIT compiles the whole engine, and a
per-box auto-tuner picks the kernel forms for *your* machine, so the same source ships tuned
...
scoreboard + methodology at
[daslang.io/dasllama.html](https://daslang.io/dasllama.html). Every supported model is
**verified token-for-token** against its reference implementation before it lands in the
support table.
...
- **Image-in chat**: Gemma-4 dense via its `gemma4uv` vision mmproj (12B is the tested
  ...
  dynamic resolution; Gemma-3 via its `gemma3v` SigLIP mmproj (4B and 12B are the tested
  pairs; fixed 896² canvas); the whole Qwen vision family via `qwen3vl_merger` — Qwen3-Omni-30B
  (mrope decoder positions), dense Qwen3-VL with DEEPSTACK (4B/8B tested: three tower taps,
  wide soft tokens, per-layer decoder adds) — and Qwen2.5-Omni/VL via `qwen2.5o` (window-attention
  ViT, non-interleaved M-RoPE decoder; 3B is the tested pair) — the family is sniffed from the
  mmproj; one image per turn. On
  Apple builds the embedder / vision towers and the image span serve on Metal by default
  (qwen image spans included — the mrope tables and deepstack slice adds ride the Metal prefill);
...
modules/dasLLAMA/
  ...
  ARCHITECTURE_GPU_VULKAN.md  # companion: the Vulkan resident driver - window chain, cm2 tiles, class pipelines, expert chain
  ...
  followup_vulkan.md          # PLANNED: grow the vulkan resident driver to Metal's contract shape
                              # (closed-arc plan/record docs — the per-family bring-up
                              #  plans among them — are archived under history/dasLLAMA/;
                              #  the archive log is history/README.md. A *_plan.md here
                              #  is an ACTIVE arc's working plan.)
  dasllama/                   # the module — require dasllama/<name>
    ...
    dasllama_vulkan_classes.das #  the Vulkan KERNEL home — every kernel a [spirv_kernel]/[vk_dispatch] class (no device state)
    dasllama_vulkan_dispatch.das # the [vk_dispatch] structure macro — generates ensure_/set_/enc_ + the census seed per class
    dasllama_vulkan_common.das #   Vulkan device state (GpuState/g_gpu) + buffer/command plumbing + hazard rail + arena + profiler
    dasllama_vulkan_seams.das #    the per-op class-rail seams (vk_add_rms / vk_rope_kv_store / vk_decode_attn)
    ...
    dasllama_arch_mistral3.das #  Mistral-Medium-3.5 arch (the llama dense shape + YaRN rope, v7-tekken chat)
    ...
    dasllama_arch_glm4moe.das #   GLM-4.5 arch (leading dense layer + sigmoid router w/ exp_probs_b selection bias + ungated shared expert)
...
respond(model, chat, params) $(piece) { ... }`); design record in `history/dasLLAMA/API_REWORK.md`.

---

## Supported models — what runs *for sure*

Legend: ✅ **verified token-for-token** vs the reference · `upstream` = the model's reference C++ engine · 🚧 in progress · ❌ not supported yet
...
| **TinyLlama-1.1B-Chat-v0.3** | F16 GGUF | Llama-2 | SPM (GGUF) | ✅ | upstream, 40/40 token-for-token (weak model — ChatML) |
| **TinyLlama-1.1B-Chat-v1.0** | Q8_0 GGUF | Llama-2 | SPM (GGUF) | ✅ | upstream, 69/69 token-for-token vs fp32; good chat (Zephyr) |
| **Llama-3.2-1B-Instruct** | Q8_0 GGUF | Llama-3 | BPE/tiktoken | ✅ | upstream (instrumented `simple_ids`, CPU greedy), 40/40 token-for-token |
| **Llama-3.1-8B-Instruct** | Q8_0 GGUF | Llama-3 | BPE/tiktoken | ✅ | upstream (instrumented `simple_ids`, CPU greedy), 40/40 token-for-token (8.5GB, needs the fmap >4GB fix) |
| **SmolLM2-1.7B-Instruct** | Q8_0 GGUF | Llama (arch `llama`) | BPE | ✅ | upstream `simple_ids`, 40/40 token-for-token; frozen fixture in `performance/model_specs.das` |
| **Mistral-7B-Instruct-v0.3** | Q8_0 GGUF | Llama (arch `llama`, no SWA) | SPM (GGUF) | ✅ | upstream `simple_ids`, 40/40 token-for-token; frozen fixture; chat via the detected `[INST]` template |
| **Mistral-Medium-3.5-128B** | Q4_K_M GGUF, 3-shard split (native K-quant planes) | Mistral3 (arch `mistral3` — the llama dense shape at 88 layers / embd 12288 / 96Q 8KV heads, NORM rope) with **YaRN** rope (factor 64 over a 4096 original context, **beta_fast 4 / beta_slow 1 from metadata** — the correction band and the 1.416 magnitude correction now key-driven; this model exposed the NORM-rope mscale drop, fixed in the same change). `attention.temperature_scale` (llama4-style Q-scaling) and `yarn_log_multiplier` ship as 0 and are fail-closed panics if nonzero | BPE (`pixtral` pre = the llama3 splitter; verified id-for-id vs the oracle) | ✅ | upstream `simple_ids`: 40/40 token-for-token on the counting continuation (frozen huge-tier fixture in `performance/model_specs.das`); chat via the detected `[SYSTEM_PROMPT]`/v7-tekken template |
| **Qwen2.5-0.5B / 1.5B-Instruct** | Q8_0 GGUF | Qwen2 (QKV bias, NEOX rope, eps 1e-6) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 1.5B 40/40 (frozen fixture); 0.5B matches to ~0.02 logits, flips only genuine near-ties (tiny model) |
| **Qwen3-0.6B / 4B-Instruct-2507** | Q8_0 GGUF | Qwen3 (QK-norm: per-head Q/K RMSNorm pre-RoPE; NEOX rope, no QKV bias) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: both 40/40 token-for-token (frozen fixtures in `performance/model_specs.das`) |
| **Qwen1.5-MoE-A2.7B-Chat** | Q8_0 GGUF | Qwen2-MoE (routed experts: softmax router → top-4 of 60, un-renormalized weights; sigmoid-gated shared expert; QKV bias, NEOX rope) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the counting AND prose prompts (frozen fixture in `performance/model_specs.das`) |
| **Qwen3-30B-A3B** | Q8_0 GGUF | Qwen3-MoE (arch `qwen3moe`: Qwen3 attention — per-head QK-norm pre-RoPE, NEOX rope, no QKV bias — with routed experts: softmax router → **renormalized** top-8 of 128, and **no shared expert**; vs qwen2moe's un-renormalized top-4 + sigmoid shared expert. 48 layers, embd 2048, 32Q/4KV heads, expert_ff 768) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the counting continuation (frozen fixture in `performance/model_specs.das`, both grouped + reference prefill paths, bit-identical). High-entropy prompts diverge at the model's OWN near-ties — a prose probe flips at the first coin-flip (oracle top-1/top-2 gap 0.04 at step 6, das picks the oracle's #2), **not** a forward bug: das tracks the oracle through margins as tight as 0.08 logits, no systematic router flips (the shared float `ffn_norm` already matches the reference's double-accumulated norm closely enough for the top-8-of-128 pick) |
| **Qwen3.5-0.8B** | Q8_0 GGUF | Qwen3.5 (arch `qwen35` hybrid: 18 Gated-DeltaNet recurrent layers + 6 gated full-attention layers — 2×-wide Q with per-head sigmoid out-gate, partial NEOX rope 64/256, QK-norm) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: token-for-token on the counting continuation, both the per-token and chunked-DeltaNet prefill paths (frozen fixture in `performance/model_specs.das`) |
| **Qwen3.8-27B** | Q4_K_M GGUF (native K-quant planes) | Qwen3.5 (arch `qwen35` dense at scale: 48 Gated-DeltaNet recurrent + 16 gated full-attention layers — 24Q/4KV heads at 256, 48 v-heads x 128 DeltaNet, partial NEOX rope 64/256, QK-norm) | BPE (`qwen35` pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the counting continuation, per-token and chunked-DeltaNet prefill (frozen large-tier fixture in `performance/model_specs.das`) |
| **Qwen3.6-35B-A3B** | UD-Q4_K_M GGUF (native K-quant planes) | Qwen3.5-MoE (arch `qwen35moe`: the qwen35 hybrid — 30 DeltaNet + 10 gated-attention layers, 16Q/2KV with 32 V-heads over 16 K-heads (modulo GQA-repeat) — plus routed experts: softmax top-8-of-256 renormalized + sigmoid-gated shared expert; F32 ssm_beta/alpha on the fp32 matmul rail; K-quant expert stacks) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: token-for-token on the counting continuation across all three prefill paths — grouped MoE, reference, and chunked-DeltaNet (frozen large-tier fixture in `performance/model_specs.das`) |
| **Qwen3-Coder-Next (80B-A3B)** | Q4_K_M GGUF (native K-quant planes) | Qwen3-Next (arch `qwen3next`: the parent of the qwen35 hybrid — 36 DeltaNet + 12 gated-attention layers, softmax top-10-of-512 + sigmoid-gated shared expert. The FILE layout differs from qwen35: fused Q4_K `ssm_ba` and HF grouped v-head order; the loader splits ba and permutes every v-indexed tensor to the kernels' tiled order at load) | BPE (qwen2 pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: token-for-token on the counting continuation, per-token and chunked-DeltaNet prefill (frozen huge-tier fixture in `performance/model_specs.das`) |
| **GLM-4.5-Air (106B-A12B)** | Q4_K_M GGUF 2-shard + Q8_0 GGUF 3-shard splits | GLM-4.5-MoE (arch `glm4moe`: **1 leading dense layer** + 45 routed-MoE layers — 128 experts top-8 over a **sigmoid router with the DeepSeek-style `exp_probs_b` selection bias** (biased pick, unbiased renormalized mixing weights) + an **ungated shared expert** (no `ffn_gate_inp_shexp`, added at weight 1); QKV bias, 96Q/8KV heads of 128, **partial NEOX rotary 64/128 on the standard attention path**, pre-FFN norm named `post_attention_norm`; the trailing NextN block ships its OWN `embed_tokens`/`shared_head_head`, so the loader disables the MTP draft head with a warning — trunk = 46 layers, exactly the upstream forward) | BPE (`glm4` pre = the llama3 splitter; verified id-for-id vs the oracle, no BOS — glm4 nulls it) | ✅ | upstream `simple_ids`: 40/40 token-for-token on the counting continuation, both quants (Q8_0's own oracle continues identically; frozen huge-tier fixtures in `performance/model_specs.das`); chat via the detected `[gMASK]<sop>` template (prelude + role markers, thinking on by default) |
| **Phi-3.5-mini-instruct** | Q8_0 GGUF | Phi3 (fused QKV + gate_up, NEOX rope, LongRoPE) | SPM | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 (frozen fixture); prose matches to ~0.06 logits, flips only genuine near-ties |
| **Gemma-2-2B-it** | Q8_0 GGUF | Gemma2 (GeGLU, dual softcap, sliding window, sandwich norms, embed ×√dim) | SPM (GGUF) | ✅ | upstream `simple_ids`, token-for-token; frozen fixture in `performance/model_specs.das`; SWA exercised on a 4k+ context |
| **Gemma-3-1B / 4B-it** | Q8_0 GGUF | Gemma3 (Gemma-2 shape minus softcaps + QK-norm; 5:1 sliding:global layer pattern, dual RoPE θ 10k/1M, linear-8 position scale on the 4B's global layers) | SPM (GGUF) | ✅ | upstream `simple_ids` / `harness/parity.sh`: both 40/40 token-for-token (frozen fixtures); sliding mask exercised on a ~900-token prompt (40/40) |
| **Gemma-4-12B-it** | Q8_0 GGUF | Gemma4 (heterogeneous geometry: sliding layers 16Q/8KV heads of 256 vs global 16Q/1KV of 512, per-layer bool SWA pattern; p-RoPE freq factors on global layers only; weightless V-norm; V-from-K on global layers; unit attention scale; per-layer output scale; final softcap; suppressed-token logit bias) | SPM-style BPE (`gemma4`) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the short counting prompt AND on a ~1490-token window-engaged prompt — the latter is the frozen fixture, encoded in-test through the gemma4 tokenizer (the short prompt's continuation drifts into the channel format on a near-tie, so it stays out of the suite); tokenizer 46/46 vs the `ggml-vocab-gemma-4` corpus |
| **Gemma-4-31B-it** | Q8_0 GGUF | Gemma4 — the same heterogeneous arch as the 12B, scaled (60 layers, embd 5376, 32Q / [16×5,4]KV heads; PLE off, no shared-KV, final softcap 30, 5-local:1-global SWA) | SPM-style BPE (`gemma4`) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the ~1490-token window-engaged counting prompt — runs the existing gemma4 forward unchanged; the frozen fixture reuses the 12B continuation (byte-identical, both count) |
| **Gemma-4-26B-A4B-it** | Q8_0 / Q4_K_M GGUF (the Q4_K_M mix bumps layer-3+ `ffn_down`/`ffn_down_exps` to Q5_0 — loaded via the exact Q5_0→Q8 transcode; the Q4_K/Q6_K rest ride native kq planes) | Gemma4 **MoE** (30 layers, embd 2816, 16Q / [8×5,2]KV heads; 128 experts top-8, expert_ff 704, softcap 30, SWA 1024, PLE off). Every layer SUMS two parallel branches: a **dense shared expert** (`ffn_norm` → GeGLU → `post_ffw_norm_1`, always on) and **routed experts** (`pre_ffw_norm_2` → softmax top-8-of-128 renormalized → `post_ffw_norm_2`), then a shared `post_ffw_norm` on the sum. Custom-normalized router — weightless RMS(attn_out) × 1/√dim × learned `ffn_gate_inp_s` (double-accumulated to match the reference's double accumulation, since it feeds a discrete selection); fused `ffn_gate_up_exps` split into the separate gate/up stacks at load; a per-expert down-output scale folded into the routed weights | SPM-style BPE (`gemma4`) | ✅ | upstream `simple_ids` / per-step logit margins: token-for-token on the confident head of the counting continuation (frozen 11-token fixture in `performance/model_specs.das`). Past that the model's OWN routing is near-tied (oracle top-1/top-2 gap ~0.5–2.2 at several counting steps) — the discrete top-8-of-128 routing lets a float residual land on the other side of the coin-flip; verified **not** a forward bug (matches the oracle at every confident step; the double router RMS removes the systematic flips). The Q4_K_M file holds a 15-token confident head the same way (its own per-file oracle; frozen fixture in `performance/model_specs.das`) |
| **Gemma-4-E4B-it** | Q8_0 GGUF | Gemma4 **edge** member (42 layers, embd 2560, 8Q/2KV heads; 5-local:1-global SWA, window 512, softcap 30). Two features beyond the dense 12B/31B: **PLE (per-layer embeddings)** — a per-(token, layer) side input (gather the token's Q8 PLE row, project the scaled embedding through `per_layer_model_proj`, RMSNorm + combine) mixed into each layer's output by `gelu(inp_gate·cur) ⊙ ple_inp[l] → proj → post_norm`, before the layer scale; and **cross-layer KV sharing** — the last 18 layers carry no `attn_k`/`attn_v`, projecting Q only and attending against an earlier layer's cached K/V (sliding → `kv_from_start-2`, global → `kv_from_start-1`; those layers' own K/V weights are dead in the file, skipped at load) | SPM-style BPE (`gemma4`) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the ~1490-token window-engaged counting prompt (the 512 window is genuinely engaged, kstart > 0) — dense, so it holds the full 40; the frozen fixture reuses the 12B continuation (byte-identical, same counting momentum + vocab) |
| **Gemma-4-E2B-it** | Q8_0 GGUF | Gemma4 edge model (35 layers, embd 1536, 8Q/1KV heads) — PLE + cross-layer KV sharing exactly as the E4B, plus **per-layer FFN width**: `feed_forward_length` is a per-layer array (6144 for layers 0-14, 12288 for 15-34 — the elastic MatFormer sub-model of the E4B), so the dense FFN weight offsets vary per layer (`ffn_w` / `w1_offs`·`w2_offs`·`w3_offs`; `hidden_dim` is the max for scratch) | SPM-style BPE (`gemma4`) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the window-engaged 1..320 counting prompt; reuses the 12B continuation. Fast-tier (~5 GB) — runs in the default test loop |
...
| **Gemma-4 E2B / E4B** (audio input) | ggml-org Q8_0 decoder + **bf16 audio mmproj** GGUFs (read directly — bf16→f32 exact, no conversion) | Gemma-4 E-series text decoder (unchanged — the G3 PLE + cross-layer-KV path) + NEW `gemma4a` **Conformer** audio encoder (`dasllama_gemma4a.das`): 128-mel HTK filterbank → Conv2D ×2 subsample (LN-over-channels + ReLU) → input proj → 12 blocks { half-step macaron FFN (SiLU) · causal sliding-window-12 self-attn with sinusoidal RPE (`matrix_bd[12−d]`) + softcap 50 + per-dim Q scale · conv module: pw1 → GLU → causal depthwise conv1d k=5 → RMSNorm → SiLU → pw2 } → out proj → weightless RMSNorm → mm input proj. 🔑 the FFN/attn/conv matmuls honor **per-op activation-quant calibration ranges** (clamp-in/clamp-out); 🔑 the conv `norm_conv`/`conv_norm` tensors are name-swapped in the GGUF. Soft tokens splice at `<\|audio>…<audio\|>` in the Gemma-4 turn template via `forward_prefill_embd`; new `AsrKind.gemma4a`, routed by the mmproj's `clip.audio.projector_type='gemma4a'` | SPM (gemma) | ✅ | `llama-mtmd-cli --temp 0 --jinja` (no-think): E2B encoder soft tokens match mtmd's audio embeddings at rel ~0.0027 (float-order noise); greedy transcription token-for-token through the 48-token confident prefix, diverging only at the Gemma-4 q8 decoder's OWN ~0.6-logit near-tie (upstream's raw logits there: 3 tokens within 0.6). Fixture `test_gemma4a_audio_oracle` in `test_whisper.das`; perf A/B on the site ASR board (`performance/records/<box>.json`) |
| **Qwen3-Omni-30B-A3B-Instruct** (audio input) | ggml-org Q8_0 thinker + **bf16 audio mmproj** GGUFs (read directly — bf16→f32 exact, no conversion; ~34 GB) | Audio-in/text-out (Talker/speech-generation out of scope). Thinker = the SHIPPED `qwen3moe` forward, registered as arch `qwen3vlmoe` (= qwen3moe + M-RoPE whose coordinates all equal the linear position for text+audio so any section layout degenerates to standard rope, — bit-identical to the plain qwen3moe forward for text+audio, while image turns rope the interleaved IMROPE sections via the same mmproj's `qwen3v` ViT (see Image-in chat above): 48 layers, embd 2048, 32Q/4KV heads, 128 experts top-8, expert_ff 768). Audio side = the SHARED `qwen3a` AuT tower VERBATIM (data-driven from the mmproj: d_model 1280, 32 blocks, 20 heads, ff 5120, proj 2048 — same conv2d-chunk frontend + 800-frame windowing as Qwen3-ASR, just bigger dims). Greedy transcription turn: soft tokens splice at `<\|audio_start\|>…<\|audio_end\|>` in a bare Qwen chatml user turn (non-thinking Instruct), decoded until `<\|im_end\|>`; new `AsrKind.qwen3omni`, routed by the `qwen3vlmoe` decoder arch under the same `qwen3a` mmproj sniff (reuses the decoder + tower fields — zero frame growth) | BPE (qwen2 pre) | ✅ | `llama-mtmd-cli --temp 0 --jinja` (same Q8 thinker + bf16 mmproj): **jfk + 2 LibriSpeech clips FULL token-for-token** incl. the trailing `<\|im_end\|>` (151645), with NO near-tie divergence. Large-tier fixture `test_qwen3omni_audio_oracle` in `test_whisper.das` (`DASLLAMA_PARITY_FULL=1`); perf A/B on the site ASR board (`performance/records/<box>.json`) |
| **gpt-oss-20b** | MXFP4 GGUF → self-Q8 | gpt-oss (MoE: 32 experts top-4, softmax over the SELECTED weights, no shared expert; router + per-expert + QKV + output-projection biases; clamped SwiGLU `swiglu_oai`; per-head attention sinks; alternating 128-token SWA; YaRN rope factor 32 over a 4096 original context) | BPE (`gpt-4o` pre) | ✅ | upstream `simple_ids` / `harness/parity.sh`: 40/40 token-for-token on the counting prompt AND on a 449-token window-engaged prompt encoded in-test through the gpt-4o pre-tokenizer (both frozen fixtures in `performance/model_specs.das`); tokenizer verified id-for-id vs upstream on counting / mixed-case-contraction / whitespace probes |

Models are **not** checked into the repo — they live in `$MODELS/`, wherever you keep
GGUFs (`export MODELS=~/models`; gitignored). Get them with
...
bin/daslang -jit examples/dasLLAMA/run.das -- $MODELS/tinyllama-1.1b-v0.3-f16.gguf
bin/daslang -jit examples/dasLLAMA/run.das -- $MODELS/Llama-3.2-1B-Instruct-Q8_0.gguf

# Interactive chat — ONE REPL for every model; the arch registry picks the chat template + stop tokens
bin/daslang -jit examples/dasLLAMA/chat.das -- $MODELS/tinyllama-1.1b-chat-v1.0.Q8_0.gguf
bin/daslang -jit examples/dasLLAMA/chat.das -- $MODELS/gemma-2-2b-it-Q8_0.gguf
...
bin/daslang -jit examples/dasLLAMA/audio_chat.das -- $MODELS/qwen2audio-7b-q8_0.gguf \
    $MODELS/qwen2audio-mmproj-f32.gguf $MODELS/jfk.wav "What is being said in this audio?"
bin/daslang -jit examples/dasLLAMA/audio_chat.das -- $MODELS/Llama-3.2-1B-Instruct-Q8_0.gguf \
    $MODELS/mmproj-ultravox-1b-f32.gguf $MODELS/jfk.wav
...
# Token-for-token gates: test_forward (stories15M vs llama2.c) + test_parity (frozen upstream
...
# Ad-hoc parity vs upstream for a new model / prompt (wires both sides)
...
| Chat | per-arch data-driven templates in the registry + one segment-accumulation renderer (`dasllama_chat`) — reproduces the reference prefills token-for-token (`test_chat.das`); the template is auto-detected by sniffing the GGUF's embedded `tokenizer.chat_template` (never executed), falling back to the arch heuristic; Gemma-4 uses its channel-based turn format (`<|turn>` / `<turn|>`; thinking is the default — the `<|think|>` gate opens the system turn and the bare opener lets the model emit its `thought` channel; the instruct opt-out prefills the closed empty channel); gpt-oss uses the Harmony-lite template (`<|start|>role<|message|>...<|end|>`, replies end at `<|return|>` / `<|call|>`, channel markers render in decoded text rather than being stripped) |
| Thinking (reasoning) | per-arch reply-side matcher (`THINKING.md`): symmetric `<think>` (thinking-capable Qwen3/3.5/3.6 and GLM), Harmony channel-switch (gpt-oss), asymmetric channels (Gemma-4) — `split_reasoning` splits a reply into reasoning/content (history stores content only), the `make_think_stream`/`think_feed`/`think_finish` splitter streams the same split, and both are vocab-gated so an inert declaration never reclassifies a reply; a Qwen Instruct template whose generation branch has no reasoning form, such as Qwen3 Instruct-2507, defaults off and leaves reply text intact; `set_thinking` toggles supported per-family defaults (`enable_thinking=false` empty-block suppression, Gemma-4's closed-channel opt-out), and `effective_stop_ids` adds the instruct-mode framing-marker stops |
| Tool calling | per-arch `ToolMode` wire codecs (`dasllama_tools.das`): hermes (Qwen), harmony (gpt-oss), gemma4 DSL, mistral control tokens, llama_json — defs render, call replay, and result framing dispatch on the mode in the chat layer (`set_tools`/`add_tool_results`/`render_assistant_calls`), the server parses each format back to OpenAI `tool_calls`; model-free codec pins in `tests/test_tool_formats.das`, live legs per family in the server tests |
| Self-speculative decode (MTP/NextN) | qwen35-family `-MTP-` GGUFs load natively (`nextn_predict_layers == 1`): the in-file NextN draft block — one trunk-shaped attention layer at index `n_layers` plus `nextn.eh_proj/enorm/hnorm/shared_head_norm` — drafts one token ahead and a 2-row batched trunk forward verifies it, so accepted steps stream the weights once for two tokens. **Output-invariant vs plain greedy** (gated by `modules/dasLLAMA/tests/test_mtp.das` + the scheduler arm in `test_scheduler.das`); 1.39× decode on Qwen3.6-27B Q8 flat-loop, ~2× through the server (zen2, list continuation). Engine gate `set_mtp_spec` + per-step `mtp_spec_eval`: prefills maintain the head's KV incrementally (chunked prefill included, seam rows via the `mtp_h` handoff), flat AND paged sessions (the pool carries the draft-block slab), every KV codec; `dasllama-server --mtp` spec-ticks every stream - a sampled one (temperature > 0, penalties included) draws each verify row with its own sampler and accepts while the draw equals the draft, so the plain sampled distribution is kept at a lower acceptance rate; the DeltaNet recurrent state snapshots and rolls back around rejected drafts; `mtp_only` sidecar files panic honestly; a draft block with its OWN `nextn.embed_tokens`/`shared_head_head` (glm4moe) disables the MTP head with a warning — the das head drafts through the trunk's embedding/classifier, so it would use the wrong weights there (trunk forward unaffected; upstream never runs the NextN layer either). **Assistant drafter on Metal (Gemma-4):** the ggml-org `mtp-<model>-Q8_0.gguf` sidecar beside a gemma-4 trunk is picked up automatically and served as a fused Metal round — the drafter's k steps in one command buffer, the k+1 verify rows on the batched decode driver, one join per round (`dasllama_metal_mtp_gemma`); depth is a per-box knob (`runtime.mtp_depth_assistant`, minted by the tuner; `--mtp-depth` overrides). gemma-4-26B-A4B on an M5 Max: 122 -> 143 tok/s at 74.6% acceptance on the SpecBench chat subset |
| Performance | KV cache, SIMD + JobQue-threaded matmul, activation-quant Q8·Q8 behind a pluggable kernel-backend registry (ARM SDOT/laneq + the generated x64 AVX2 maddubs/vpdpbusd tier — `history/dasLLAMA/x64_arch.md`), flash-attention batched prefill, per-box kernel tuning (`tune_for_this_box.md`) |
...
2. **Expected output IDs** from the instrumented reference oracle `harness/oracle/simple_ids.cpp`
   (a minimal driver over the reference library that prints greedy token ids), or `run_ids` for the
...
3. Feed the same prompt IDs through dasLLAMA's forward pass (`harness/parity.das`) and diff the
   ...
   Once verified, freeze the prompt + continuation as the model's `parity` evidence on its
   `performance/model_specs.das` row (`ParityEvidence`: pinned ids or the in-model prompt text,
   plus the arms it was frozen under) — `tests/test_parity.das`'s generic loop and the board's
   parity pregate both read that row, so it is a self-skipping regression gate AND the
   profiling entry check at once.
   Note: on tiny models a near-tie argmax can flip from sub-ULP numerical differences vs upstream's
...
4. For the tokenizer itself: the `ggml-vocab-*.gguf.inp`/`.out` fixtures in
   `$MODELS/` are ready-made round-trip test vectors.

**Adjudicating a near-tie divergence.** When a temp-0 gate diverges, walk your greedy steps
printing the top-2 logit gap at each one — that is the standard verdict method. A flip at a gap
of a fraction of a logit out of ~24 (p ≈ 0.5) is the model's own coin-flip, not a forward bug,
but the verdict needs all three of: every MATCHED step clears a wide gap; tokenizer, decoder and
encoder each verify independently; and a prompt that happens to carry no tie matches
byte-identical to the natural stop. That last one is the positive gate — without it, "it was
just a near-tie" is unfalsifiable.

**Audio towers: oracle against fp32, ship f16.** Published mmprojs are f16, and the reference's
f16 matmul accumulates in fp16 on ARM, which drifts against our fp32 tower — so an oracle run
uses a local f16→f32 re-encode of the mmproj. **The conv weights must stay f16**: the reference's
clip conv/im2col path aborts on F32 kernels (which is also why the upstream converter pins convs to
f16 even at `--outtype f32`). Our loader reads either dtype identically.

**Two audio-loader traps the oracle cannot warn you about.** The reference's stack "padding"
never materializes — its pad helper is a power-of-2 bitmask macro and the stack strides are not
powers of two, so the real semantics are `floor(n_pos / stack)` full rows with the ragged tail
positions DROPPED, and the zero-pad row never reaches the projector; implement the floor
directly (`audio_tokens_per_chunk`). And `clip.audio.projection_dim` lies on ultravox-1b (claims
4096 while `mm.a.mlp.2` projects into the 2048-wide decoder) — derive projector shapes from the
mm tensors, never from that key.
