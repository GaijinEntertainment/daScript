...
a dead exe names why and reads 0); plus the committed-records sweeps: image-chat receipts match their
...
axes, or no batched row), the two guards on `records/<box>.json` (`--jit` and `--catalog all` each
