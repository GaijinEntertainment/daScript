...
input, and the hybrid file on Qwen3.5-0.8B, where each recurrent layer keeps a device state slot
per region, so the two sessions' batched steps advance their own deltanet states through the N-row
command's recurrent form with the gated q over the rows, and a session's prefill sends home whatever
another session left dirty in its region's slot before it zeroes that slot; its scheduler cells hold that a recurrent model parks and adopts no rows and prefills a long
