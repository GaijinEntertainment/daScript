...
plain and under the router bias), `test_vkd_router_gemv_cols` (the router GEMV over three x rows against the one-row
dispatch a row at a time, bit for bit, the sentinel past the last column standing),
