...
- **Work added to, or moved within, what `run_jit` (`daslib/llvm_jit_run.das`) executes - its
  ...
  `ARCHITECTURE.md` sec.1). Option resolution before the first timer, and log lines, are not work.
...
- **A new environment or config input to the cache key folds inside `jit_env_salt`
  ...
  split-obj key (`ARCHITECTURE.md` sec.2)** - salt feeds both keys, and a config
