...
- **Work added to or moved within a phase of the `LLVM JIT time:` breakdown runs inside a
  timed phase** (the phase inventory and the code behind each phase: `ARCHITECTURE.md` §1).
  Option resolution before the first timer and log lines are not pipeline work.
...
- **Weakening `tests-cpp/small/test_jit_emitter_pin.cpp` (repo root) is a defect** — it is
  what makes every `EMITTER_FILES` edit visible: the pin fails until the hash is re-pinned,
  and its message asks the bump question.
