...
- **`emit_mul`** routes `*` by operand shape BEFORE the scalar binop path (component-wise FMul only works
  ...
  an unsupported mix (e.g. integer vectorxscalar) is a clean error.
...
A single combined textured-quad render - UBO column-major MVP + combined image sampler (2x2 checkerboard) +
...
- **6.1 leaf + arithmetic; 6.2 control flow; 6.3 resources + composites.** Ported construct-by-construct
  ...
  `spirv_dis::op_operand_is_literal`). Added the sec.7-A normalized-disassembly golden gate up front, plus sec.7-B
...
- **6.4 fail-closed.** Added the explicit-rejection half of the contract: a `preVisit*` override for every
  ...
  future AST node type with no hook). sec.7-C negative tests (`tests/spirv/_fail_closed/`, compiled via
...
- **7.2 explicit-LOD + texel fetch.** `textureLod`->`OpImageSampleExplicitLod` (Lod operand), `texelFetch`->
  ...
  known 64x64 texture into a storage image, assert every pixel equals the source.
...
- **7.4 separate image + sampler.** `texture2D` -> standalone `OpTypeImage`, `sampler` -> standalone
  ...
  Reflection kinds `sampled_image` + `sampler`. GPU gate: a 2x2 checkerboard sampled through the split
...
- **`dFdx`/`dFdy`/`fwidth` -> `OpDPdx`/`OpDPdy`/`OpFwidth`** (screen-space derivatives over the 2x2 quad).
...
1. **Generic builtins get mangled instance call-names** (`spirv_builtins`dFdx`5408...`), so the emitter's
   ...
   concrete overload for exactly this reason; the derivatives had to follow suit (4 widths x 3 functions).
...
- **Fail-closed.** A call-graph cycle check (`detect_recursion`, 3-colour DFS over {entry}  union  user_funcs)
