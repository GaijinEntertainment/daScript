...
docs: `ARCHITECTURE.md`, `ARCHITECTURE_COOPMAT.md` (cooperative matrix, emission-time
unrolling). Shared emitter rules: `modules/REVIEW_SHADER_EMITTERS.md` - apply that
...
capability after the change.** A device cell runs a kernel using
the capability on a device, in `modules/dasVulkan/tests/integration/` or
