...
4. **✅ DONE (2026-06-23) — `gl_InstanceIndex` → GL `gl_InstanceID`.** Moved into the shared module + GL rename gate + `tests/glsl` lock; only `vindex.txt` golden reordered (benign); dasVulkan smoke green. See changelog. **Remaining sub-task — `spirv_builtins` audit done (2026-06-23), candidates triaged:**
   - **Clean name-identical moves (do next — same pattern as `gl_FragCoord`/`fwidth`, no rename, no marker field):** `gl_FragDepth` (Output `float`, GL core + ES 3.0), `textureLod` + `texelFetch` (GL core + ES 3.0 functions). Each is delete-from-`spirv_builtins` → add-to-`shader_lingua_franca`; **first verify `glsl_common` doesn't already declare it** (else a delete-from-both is needed). `gl_FragDepth` is used by the `fragx` spirv fixture → expect that golden to reorder (regen + verify only it changes, as with `vindex`).
   - **Deferred until #3 (Plan-A marker decision):** `samplerCube`, `sampler2DArray`, `sampler2DShadow` — GL-portable (ES 3.0/WebGL2) but opaque-marker structs the GL rail needs a texture-handle field + bind codegen on — i.e. the exact backend-field-in-shared-marker question #3 reconsiders. Also `textureCompare`: GL spells shadow compare as `texture(sampler2DShadow, vec3)`, not a distinct function, so it needs a GL-specific lowering, not a passthrough.
