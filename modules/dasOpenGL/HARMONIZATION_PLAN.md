...
0b. **The browser loop — the 3-call lifecycle convention (B).** NOT the eval_main_loop/unwind patch, NOT dasLiveHost. In the existing wasm run path (`utils/daScript/main.cpp:310`, the same `callMain` startup the playground uses), under `__EMSCRIPTEN__`, after compile: **if the program exposes `update`** → call `init()` if present, `emscripten_set_main_loop(→ update, 0, false)`, **leak/persist the Context** (move it off the stack, don't tear down), return; `shutdown()` if present on cancel/stop. **Else** run `main()` (single-shot). That single Context-leak is the entire lifetime fix — it deletes the unwind hazard (the host now owns a persistent Context; nothing the per-frame callback needs is on a stack that unwinds). Tutorials are authored with `init`/`update`/`shutdown` + `main` (main = desktop driver: `init(); while(!exit){update()}; shutdown()`) — the shape the games already use (arcanoid `main.das`). On web the run path calls the 3 directly; desktop runs `main`; one homogeneous `.das`. Bonus (deferred): browser hot-reload via the same structure.
