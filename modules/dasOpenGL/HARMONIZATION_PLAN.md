...
**▶ NEXT:**
0. **wasm gate (b) part 2 — FULL generator (ACTIVE).** Mechanism PROVEN (above). Build the generator: parse emscripten GLES2/GLES3 headers → WebGL2 function-name list → emit `gl_wasm_wrappers.inc` (the `Wrap<decltype(&::glX)>::call` template machinery + a `{hash(name) → wrapper}` table + collision-validation), `#include <GLES3/gl3.h>`; switch `getWrapper`'s wasm branch from the prototype sig-hash table to the generated name-hash table. Then render the full `01_hello_triangle` (shader compile + VBO/VAO + draw). The main-loop lifetime question (eval_main_loop + simulate_infinite_loop unwinds `pctx`) is still open but ORTHOGONAL — the probe proves single-frame render; the loop is a separate Boris-call (keep the context alive across the browser loop). Everything else upstream is built + proven.
