...
4. **✅ DONE (2026-06-23) — `gl_InstanceIndex` → GL `gl_InstanceID`.** Moved into the shared module + GL rename gate + `tests/glsl` lock; only `vindex.txt` golden reordered (benign); dasVulkan smoke green. See changelog. **Remaining sub-task:** audit the rest of `spirv_builtins` for other modern-union members GL also speaks (note: `gl_DrawID` stays VK-only — its GLSL form needs `GL_ARB_shader_draw_parameters`, outside the portable floor).
