...
0a. **wasm gate (b) part 2 — FULL generator (ACTIVE, NEXT).** Mechanism PROVEN (blue canvas). Build the generator: parse the emscripten GLES2/GLES3 headers → WebGL2 fn-name list → emit `gl_wasm_wrappers.inc` (a `Wrap<decltype(&::glX)>::call` template that unpacks the vec4f arg-slots by the function's own param types + a `{hash_blockz64(name) → wrapper}` table + build-time collision-validation), `#include <GLES3/gl3.h>`; then swap `getWrapper`'s `__EMSCRIPTEN__` branch from the prototype sig-hash table to the generated name-hash table (hash `fun->name`, == the gl symbol; desktop-only gl funcs are absent → fail-closed at address resolution, the correct WebGL2 result). Then render the full `01_hello_triangle` (shader compile + VBO/VAO + draw).
