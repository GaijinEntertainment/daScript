...
**▶ NEXT:** grow the shared surface toward the modern union, each with its emitter lowering (no declared-but-unlowerable symbols): `fwidth` (name-safe both rails) → `gl_FragCoord` (GLSL name-compatible) → `gl_VertexIndex` WITH the GL emitter `gl_VertexIndex`→`gl_VertexID` rename (§5). Each its own preflight-green commit on this branch.
