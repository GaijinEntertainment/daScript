...
4. **✅ DONE (2026-06-23) — `gl_InstanceIndex` → GL `gl_InstanceID`** (only `vindex.txt` reordered, benign) **and the `spirv_builtins` audit tail (commit `2670b9e`).** See both changelog entries. **Audit done (2026-06-23), candidates triaged:**
   ...
   - **Unblocked by #3 (Plan-A kept) but a REAL work item, not a passthrough:** `samplerCube`, `sampler2DArray`, `sampler2DShadow` — GL-portable (ES 3.0/WebGL2) opaque markers; with Plan-A kept they carry the same GL handle field as `sampler2D`. The blocker is no longer the design question — it is that the GL bind codegen (`glsl_opengl.das generate_bind_uniform`) only handles `sampler2D`/`sampler3D`/`image2D` today; the others fall into the generic `glUniformAny` `else` branch (wrong for a sampler), so each needs a new GL bind path + GL runtime support before its move is real. `textureCompare` further needs a GL-specific lowering (GL spells shadow compare as `texture(sampler2DShadow, vec3)`, not a distinct function). The dummy-host-bind-stub pattern (commit `2670b9e`) covers their `tests/glsl` emission locks once the bind paths exist.
