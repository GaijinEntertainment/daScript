...
namespace das {
   ...
   // Top-level
        ...
        static constexpr uint32_t getVersion () {
            return 206;   // 206: the record header carries the requires the parse took (205: a vector of a handled element streams under the element's module; 204: the record header stamps the source by content hash; the policy stream carries every CodeOfPolicies field)
        }
