...
namespace das {
    ...
    //
    // Modeling choices (see ast_cfg.cpp): `finally` (ExprBlock::finalList) runs on the normal exit of
    // its block and is threaded onto the normal-path edges. `panic` is treated as program-exit (no
    // edge) - daslang panic is fatal and skips finally, so it carries no escape-relevant flow.
    //
