...
#pragma once
        ...
        #define DAGOR_NOISE_SUPRESS_UB  __attribute__((no_sanitize("undefined")))
    #endif
#endif

#ifndef DAGOR_NOISE_SUPRESS_UB
#define DAGOR_NOISE_SUPRESS_UB
#endif


// Math for Game Programmers: Noise-Based RNG
// author: Squirrel Eiserloh
__forceinline DAGOR_NOISE_SUPRESS_UB unsigned int uint32_hash(unsigned int val)
{
...
__forceinline DAGOR_NOISE_SUPRESS_UB unsigned int uint_noise1D(int position, unsigned int seed)
{
  constexpr unsigned int BIT_NOISE1 = 0x68E31DA4;
  constexpr unsigned int BIT_NOISE2 = 0xB5297A4D;
  constexpr unsigned int BIT_NOISE3 = 0x1B56C4E9;
...
__forceinline DAGOR_NOISE_SUPRESS_UB unsigned int uint_noise2D(int posX, int posY, unsigned int seed)
{
  constexpr int PRIME_NUMBER = 198491317;
  return uint_noise1D(posX + (PRIME_NUMBER*posY), seed);
}

__forceinline DAGOR_NOISE_SUPRESS_UB unsigned int uint_noise3D(int posX, int posY, int posZ, unsigned int seed)
{
