...
Do NOT use `daslib/functional` (`map`, `filter`, `each`, `to_array`) for new code. The linq operators (`select`, `where_`, `_select`, `_where`, `to_array`, …) are the supported chainable ones; the functional surface is older, less integrated with the pipe-syntax macros, and patterns built on it have hit fusion-runtime regressions.
...
```das
// Map only:
let names <- [for (x in items); string(x.name)]
...
let pairs <- [for (k, v in keys(t), values(t)); MyPair(key=k, value=v)]
...
- **Inside `_<op>` bodies, `_` is the element.** `_select("{info.dim[_]:d}")` substitutes `_` correctly inside string interpolation. Multi-statement bodies via `build_string() $(w) { ... use _ ... }` work too — the macro substitutes `_` throughout the literal AST you pass.
...
- A linq chain is a complete transformation. Don't drop a `[for (x in iter); ...]` comprehension into the middle.

```das
// Bad — comprehension handed to a linq sequence operator:
let pairs <- join_to_array(
    [for (x in customers); x],
    orders,
    @@(c) => c.id, @@(o) => o.customer_id, @@(c, o) => Pair(c, o))

// Good — pure linq:
let pairs <- _fold(customers |> join_to_array(orders, @@(c) => c.id, @@(o) => o.customer_id, @@(c, o) => Pair(c, o)))

// Bad — linq chain feeding a comprehension:
...
The rule applies to any sequence-consuming high-order primitive, including string `join`. If a single comprehension suffices and is fed only to free functions like `length`, that's still one style. The split happens when an operator that itself walks a sequence shows up — at that point the upstream should be linq.
...
Do NOT shoehorn side effects into `_select` lambdas — they're meant to be pure.
