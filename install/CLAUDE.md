...
- **`label`, `expect`, and `pass` are reserved words** (lexer keywords — `pass` is the no-op statement) — using any as a parameter/variable name is a syntax error; rename (`tag`, `want`, `cpass`)
...
| `int64(length(x))` / `uint64(capacity(x))` (also `count`, `find_index`, `fread`, `fwrite`) | `long_length(x)` / `long_capacity(x)` / … | LINT017: the inner call returns `int`, so the wrap past 2^31 already happened *before* the widening cast — the cast buys nothing. Gated on receiver type, so same-named user overloads and the fixed-array `length` generic (no `long_` twin) stay silent |
