...
**gen2 is the DEFAULT parser** — every `.das` file parses as gen2 unless it explicitly opts out with `options gen2 = false` (the only gen1 discriminator; `options gen2` markers in existing code are historical no-ops — NEVER infer gen1 from their absence). All code MUST use gen2 syntax; house style still adds `options gen2` at the top of new files. Key rules:
...
- **Named arguments:** bare `foo(pos, name = value)` — named args after positionals, no brackets needed (0.6.4+); works on method calls too (`obj.m(name = v)` / `obj->m(name = v)`). The bracketed form `foo([name = value])` still parses; there, multiple named arguments share one bracket group (`foo(positional, [first = a, second = b])`) — separate groups such as `foo([first = a], [second = b])` are a syntax error
...
- **`delete` on `array<T?>` (any container of raw pointers) FREES THE POINTEES** — it finalizes and heap-frees every element, then the buffer. On borrowed (non-owning) pointers this is heap corruption: the interpreter reports `deleting <ptr>, which is not a chunk pointer`; Release+JIT corrupts SILENTLY and crashes at some later unrelated alloc/free. For non-owning pointer containers: struct fields take `@do_not_delete` (canonical: `daslib/aot_cpp.das` `@do_not_delete stack : array<ExprBlock?>` — pointees survive, buffer still freed); locals do `arr |> clear()` before `delete arr` (`clear` does NOT finalize elements)
- **Strings:** `var s : string` is a writable local copy (no propagation). `var s : string&` propagates. **`:=` on a string clones ONLY when the source is a temp (`#`) string or `options multiple_contexts` is on** (infer promotes it to `clone_string`); otherwise it is the same pointer copy as `=`, and LINT016 flags it. The cross-context copy is explicit `clone_string(s)`.
...
- **`addr<T?>(x)`** — sugar for `reinterpret<T?>(addr(x))` with ONE unsafe gate: `unsafe(addr<T?>(x))` covers both halves. Target must be a pointer type — `addr<int>(x)` is `error[30133]`, always a typo for `addr<int?>`. The exact-type form (`addr<int?>(i)` where `addr(i)` is already `int?`) folds to plain `addr` but still demands unsafe via the addr gate. Prefer over the double-wrapped `unsafe(reinterpret<T?>(unsafe(addr(x))))` — STYLE034 flags the long form (pointer targets only)
...
- **`reinterpret<T>(expr)`** requires `unsafe` — used for const-stripping on regular pointers: `unsafe(reinterpret<Foo?>(const_ptr))`. The cast is self-delimiting: `reinterpret<float?>(b) + 1` adds on the CAST result's stride
...
- **`label`, `expect`, `pass`, `explicit`, `capture`, `deref`, `template`, `where`, and `shared` are reserved words** (lexer keywords — `pass` is the no-op statement, `explicit` the generic-parameter modifier, `where` the comprehension filter, `shared` the module modifier) — using any as a function/parameter/variable name is a syntax error; rename (`tag`, `want`, `cpass`, `declared`, `grab`, `unref`, `pattern`)
- **`range`/`urange`/`range64`/`urange64` are lexer TYPE tokens** (like `int`) — unusable as struct-field, parameter, or annotation-argument names (`@range = ...` is a syntax error); the grammar whitelists them back to a plain name only in call position, which is why `range(10)` works. Rename (`rng`, `span`)
- **Literal `{`/`}` in string literals must be escaped `\{`/`\}`** — unescaped `{...}` is interpolation. Bites when embedding shader/C source as inline strings. String literals may span multiple lines (raw newlines are legal)
...
- **`require` path resolution:** bare `require name` resolves a module-root mount (`daslib/x`, a registered module) **or** a *same-directory* `name.das`. To require a file *elsewhere in the tree* by path, use a **file-relative path with the explicit `.das` extension** — `require ../../foo/bar.das`. Dropping the `.das`, or using a non-mounted root-relative path, fails with `error[20605] missing prerequisite … file not found`. **A require path is `NAME` tokens joined by `/` `.` `%` `..` — so it CANNOT contain a hyphen:** a dir like `my-tools/` is unreachable by path (the parser stops at the `-`); keep requireable files in hyphen-free directories, or `require` a sibling by bare name from inside the dir. Upshot: a file needing a shared fixture should keep the fixture in its *own* directory (bare same-dir require) rather than reaching across the tree
...
- Class methods: `def const`, `def abstract const`, `def static`; call syntax `obj.method()` / `obj->method()` ONLY — pipe (`obj |> method()`) does NOT resolve class methods (error[30341]), and the same holds for `[class_method]` struct methods
...
- `print` is fine for application scripts. In library/tool code prefer `to_log(LOG_INFO|LOG_WARNING|LOG_ERROR)` — same stdout, but level-tagged and filterable
