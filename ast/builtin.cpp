...
namespace yzg
{
    ...
    void Program::addBuiltinOperators()
    {
        ...
        addBuiltIn ( make_shared<BuiltInFn<SimNode_VecCtor<2>,float2,float,float>>("float2") );
        // float3
        addBuiltInBasic<float3, SimPolicy_Vec<float3,7>>(*this);
        addBuiltInNumeric<float3, SimPolicy_Vec<float3,7>>(*this);
        addBuiltIn ( make_shared<BuiltInFn<SimNode_VecCtor<3>,float3,float,float,float>>("float3") );
        // float4
        addBuiltInBasic<float4, SimPolicy_Vec<float4,3>>(*this);
        addBuiltInNumeric<float4, SimPolicy_Vec<float4,3>>(*this);
        addBuiltIn ( make_shared<BuiltInFn<SimNode_VecCtor<4>,float4,float,float,float,float>>("float4") );
    }
