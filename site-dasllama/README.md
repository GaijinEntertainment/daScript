...
Caddy). The ladder paints from `/api/runs` alone - every run's cells ride its row - and
pairing is within one submission only (a ratio appears when the methodology's reference
flavor rode along in the same document). The verbatim `/api/submission/:id` document is the
receipt: fetched once per submission, the first time one of its rows opens, so receipts and
command lines still come from the submission itself. The sidecar finder lists `/api/sidecars` and fetches the
