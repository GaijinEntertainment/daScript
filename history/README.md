...
- 2026-08-21 `modules/dasLLAMA/tests/CLAUDE.md` (two incident anecdotes) -> `history/dasLLAMA/tests_claude_md_incidents.md` - the 5.75-of-6-hours full-suite session and the capture-time-filter miss behind the runner-only and log-discipline rules
- 2026-08-24 `modules/dasLLAMA/vision_plan.md` -> `history/dasLLAMA/vision_plan.md` - the vision arc's plan (images end to end; shipped via `#3738`)
- 2026-08-24 `modules/dasLLAMA/vision_gemma4v_plan.md` -> `history/dasLLAMA/vision_gemma4v_plan.md` - the gemma4v ViT tower plan (E-series vision; shipped via `#3791`)
- 2026-08-24 `modules/dasLLAMA/qwen2_audio_plan.md` -> `history/dasLLAMA/qwen2_audio_plan.md` - the qwen2audio bringup plan (audio-in; shipped, ASR suite carries the gates)
- 2026-08-24 `modules/dasLLAMA/whisper_plan.md` -> `history/dasLLAMA/whisper_plan.md` - the whisper-family port plan (shipped; parity runs recorded in-file)
- 2026-08-24 `modules/dasLLAMA/parakeet_plan.md` -> `history/dasLLAMA/parakeet_plan.md` - the parakeet ASR port spec (shipped as `dasllama_parakeet.das`)
- 2026-08-24 `modules/dasLLAMA/silero_vad_plan.md` -> `history/dasLLAMA/silero_vad_plan.md` - the Silero-VAD port spec + container layout (shipped as `dasllama_vad.das` + `models/silero_vad.bin`)
- 2026-08-24 `modules/dasLLAMA/qwen3vl_plan.md` -> `history/dasLLAMA/qwen3vl_plan.md` - the qwen3vl/omni vision arc's slice board (encode leads every cell; residuals ledgered in `PERF_LEDGER.md`)
- 2026-08-24 `modules/dasLLAMA/canary_apples_plan.md` -> `history/dasLLAMA/canary_apples_plan.md` - the Canary-Qwen bringup plan (shipped; oracle fixtures in-tree)
...
- 2026-08-27 `daslib/linq_fold.md` -> `history/linq_fold/linq_fold.md` - the PR A-G splice-pattern refactor masterplan, its phase logs and decision log (the module layout, `SourceAdapter` contract and walker contract were distilled into `daslib/ARCHITECTURE_LINQ.md` sec. 37, which the `linq_fold_*` headers cite)
...
- 2026-09-02 `modules/dasLLAMA/mtp_plan.md` -> `history/dasLLAMA/mtp_plan.md` - the MTP depth-N arc's plan and measurement log (S0-S7: the NextN round, the batched same-slab verify, the gemma assistant drafter, the k4 two-row tile and pair walk, the ruler); shipped with the arc, its open items live in `followup_general.md` #83-#99 (the plan's own text still numbers them 70-86, the numbering before the TTS arc's entries merged ahead of it)
