...
|---|---|---|---|
| **SSBO of vec/struct (std430)** | layout on `array<float4>`/`array<Particle>` | FAIL **hard-blocked** - emitter rejects any non-scalar SSBO element | #6 particles (dead today), any real compute data |
| **Composite/global swizzle** | swizzle on a *loaded* vec (`OpVectorShuffle`/`OpCompositeExtract`), not just locals | FAIL known gap (`cam.pos.xyz`) | #3 raymarch, every UBO with vec3 fields |
| **Depth-compare sampling** | `sampler2DShadow` -> `OpImageSampleDref*` + Depth image flag + compare-sampler | FAIL no Dref anywhere | #5 shadow (hardware-PCF variant) |
| **discard / alpha-test** | `OpKill` / `OpDemoteToHelperInvocation` | FAIL not dispatched | cutout, any "real" #4/scene |
| **derivatives** | `dFdx/dFdy/fwidth` -> `OpDPdx/DPdy/Fwidth` | FAIL not dispatched (enum exists) | procedural AA in #3, mip selection |
...
|---|---|---|---|
| **Shared/workgroup memory** | `Workgroup` storage class + `@shared` global | FAIL no `classify_global` branch | parallel reduction, tiled blur, prefix-sum |
| **Barriers** | `barrier()`/`memoryBarrier()` -> `OpControlBarrier/OpMemoryBarrier` | FAIL not dispatched (enum exists) | pairs with shared mem |
| **Atomics** | `atomicAdd...` -> `OpAtomic*` | FAIL not dispatched (enum exists) | histogram, counters, append buffers |
| **Texture queries** | `textureSize` -> `OpImageQuerySizeLod` | FAIL | size-aware sampling, post-fx |
| **gl_FragDepth write** | Output builtin (only `gl_Position` is written today) | FAIL | raymarch->depth, soft particles |
| **Texture/descriptor arrays (bindless)** | indexed descriptors | FAIL | multi-material scene (Phase-12 demand) |
