...
id pushes a clean `"unresolved expression {e.__rtti}"` error (this is half the fail-closed net - sec.6).
...
| `emit_value` ExprOp1 | `visitExprOp1` | `!`->LogicalNot; `-`/`~`->`unop_code`; statement-level `++`/`--` handled in sec.5 |
...
| `emit_value` `_type.flags.ref` load / ExprRef2Value | `visitExprRef2Value` | `OpLoad` through child `e2ptr` -> `e2id` (sec.3.2) |
...
| `emit_if` | `preVisitExprIfThenElse` / `...IfBlock` / `...ElseBlock` / `visitExprIfThenElse` | sec.5 |
| `emit_while` | `preVisitExprWhile` / `...WhileBody` / `visitExprWhile` | sec.5 |
| `emit_for` | `preVisitExprFor` / `...ForSource` / `...ForBody` / `visitExprFor` | sec.5 |
...
- **Runtime-bound for-loop** - `for (j in range(0, n))` with `n` a loaded/push-constant value. The
  ...
  **currently unexercised** - all `floop` ranges are constant. Direct hit on the sec.5 for-source handling.
...
### C. Fail-closed negative tests (the Phase-6 deliverable, locks in sec.6)
...
- **6.1 Leaf + arithmetic.** Full value/pointer hooks for consts, op1/op2, var, ref2value, let, copy/store,
  compound-assign, builtin loads. Add the sec.7-A golden net. Get `square`+`*arith` equal.
- **6.2 Control flow.** if/elif/else, while, for (incl. runtime bound), break/continue via the multi-phase
  hooks + terminator + loop_stack. Add sec.7-B tests. Get `*ctrl`+`*loop` equal.
...
- **6.4 Fail-closed.** Explicit rejection list + `getE`-unresolved backstop; add sec.7-C negative tests.
- **6.5 Prove + clean up.** Full suite (census both directions, spirv-val, LCOV ~100 % of the new emit
  ...
  blobs still render `i*i` + the textured quad), sec.7-A golden compare green, lint/format. **Delete the old
...
1. **Visitor at patch stage - RESOLVED yes.** `make_visitor(*v) { visit(fn.body, adapter) }` runs cleanly
...
1. **For-source `range()` interception** - the sharpest construct; covered in sec.5 + tested in sec.7-B.
2. **Id-numbering divergence vs byte-identical** - handled by normalized-dis golden (sec.7-A); byte-identical
...
3. **`canVisit*` granularity** - confirm which interface hooks exist for clean rejection vs relying on the
   `getE`-unresolved backstop; finalize the sec.6 list during 6.4 against the real `AstVisitor` surface.
