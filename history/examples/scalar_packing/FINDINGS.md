...
| across-cell (voronoi, 2x2 block)  | float4 | ~20 ops | 169 | 72 | **-57%** |
...
2. **Win scales with width x live-range** - the number of vector ops the packed value
...
4. **Transcendentals drive the big wins** - sin/cos/frac/sqrt go exactly widthx (voronoi
...
1. **Phase 1 already harvests every high-live-range opportunity.** The win scales with
   width x live-range, and the long-live-range parallelism in these shaders lives in the
