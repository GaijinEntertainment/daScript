...
- **texture** (`mean_flatness`, weight 0.15) - the anti-BEEP term: energy-weighted mean spectral
  flatness (geometric/arithmetic mean of magnitude per frame; 0 = pure tonal, 1 = noise). `squash(|Delta|,0.25)`.
...
- `overall = Sum w_i*score_i`. Squash = smooth/bounded -> gradient everywhere (good for the hill-climber).
...
ineffective), Schwar&Muller 2023 (MSS gradient pitfalls), Peeters Timbre Toolbox JASA'11 (temporal
