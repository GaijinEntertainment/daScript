...
0. **Preprocess once:** trim leading silence (first sample > -60 dB of peak = onset). STFT: Blackman
   window, N=4096-8192, hop <=256, zero-pad 2-4x (sharper parabolic interp). Keep magnitude in dB.
...
1. **Nature gate - pitched vs noise (THIS is the auto-classifier):** spectral flatness
   ...
   plateaus ~0.5-0.6, not 1.0.) Spectral crest `max|X|/Sum|X|` is the complement. **-> can auto-pick the
   metric preset AND the layer mix.**
2. **Multiple impacts (clap/flam/roll):** spectral-flux onset envelope `flux[m]=Sum max(0,|X[m,k]|-|X[m-1,k]|)`
   + 3-condition peak-pick (local-max AND above local-mean+delta AND min-spacing). >1 onset -> seed repeat:
...
3. **Pitch = strongest LOW partial** on a frame ~10-30ms AFTER the onset peak (not the transient frame).
   ...
   `p=0.5(alpha-gamma)/(alpha-2beta+gamma)`, `f=(k+p)*Fs/N`. Pick strongest peak in the drum's expected low band (cheat-
...
5. **Decay -> ring / damp_skew:** fit a line to the EDC in dB (start -5 dB below peak, fit to -25/-35 dB,
   extrapolate): `T60 = 60/|slope dB*s^-^1|`. Per-band T60 (e.g. <200/200-800/800-3k/3k-12k Hz); HF decays
   faster -> `damp_skew = T60_high/T60_low` (or fit `T60(f)=T60_ref*(f/f_ref)^-s`, s=skew). Robust form:
   nonlinear LS `E(t)=A*e^(-alphat)+n0` to not let the noise floor flatten the slope.
6. **Attack -> transient amount/rate, punch:** log-attack-time (MPEG-7 / Peeters weakest-effort): t_start
   ...
   `Sumt*E/SumE` = front-loaded check.
