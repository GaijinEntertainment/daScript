...
**The gap it chases:** fleet prefill is uniformly ~0.73-0.86x of llama.cpp on Genoa
(gpt-oss 0.61x) after the zvnni batch pin; on M1 prefill median is 0.92x (their engine
...
| `mr x nr` | e.g. 4x4, 8x4, 8x8, 16x4 (rows x tokens) | register-tile shape. Hard constraint: `mr*nr` accumulators + row loads + broadcast <= 32 vregs (16 for ymm) |
...
| `load` | `bcast_x` (broadcast activation dword-group x row-vector loads - the repack shape), `bcast_w` | M0 disasm decides which family the winners actually use - don't cargo-cult |
| `prefetch` | off, N bytes ahead (weights) | NT/streaming loads for fat w13/w2 streams is a candidate value (mm_ffn 1.42x is fat-stream territory) |
...
the 0.73-0.86x column. Hand-written kernel tiers get deleted only when the scoreboard says
...
|---|---|---|---|---|---|
| kv 2048x512 | 122.4 | **131.0** | 1.07x | 65.1 | 62.5 |
| qo 2048x2048 | 120.8 | **128.1** | 1.06x | 64.2 | 61.2 |
| w13 2048x5632 | 118.3 | **128.9** | 1.09x | 55.9 | 53.3 |
| w2 5632x2048 | 119.1 | **132.2** | 1.11x | 56.1 | 53.4 |
...
row-major sdot4x4 44 - repack tier ~ 3x row-major on both engines. **M2 bar: >= ~130 GMAC/s.**
...
|---|---|---|
| `vecreduce.add(mul(sext,sext))` -> scalar | `sdot` + `addv` yes | `vpmovsxbw`+`vpmaddwd`+hadd (sane, no VNNI) |
| `partial.reduce.add(v4i32, mul(sext,sext))` | **single `sdot`** yes | zmm widening + `vpmulld` + extracts - garbage |
| lane-splat shuffle + partial.reduce | `dup` + plain sdot (lane NOT folded) | - |
| full block loop, generic IR only | tight hand-shaped loop, 8 dup + 8 sdot, zero spills yes | - |
...
partial.reduce leaves one `dup` per sdot: +100% vector uops on the 4x4 tile's dot core); on
...
(the generator - emits the laneq 4x4 tile as IR: byte-GEP loads, phi loop with kstep=2 paired
...
- *Neighbors bit-exact + genuinely generated:* test mode = all 8 rows exact vs the hand-laneq
  ...
  proven).
- *Manifest round-trip:* tune run (2048x512x64 iso, interleaved best-of-6) - kstep2 **133.9**
...
weight vectors 2*mr per block at `kg*4*mr + qd*16`, accumulators tokenxquad, per-quad scale
...
sub-family stays future work). Grid now: kstep 1|2|4 x nrsplit 4|2 at mr4, kstep 1|2|4 at
...
**The FMF lesson (load-bearing).** First test run: mr8 x kstep1/kstep4 mismatched by last-ulp
...
- *mr8 structure:* kstep2_nrsplit2_mr8 = 192 sdot / 0 dup (2 slices x 3 blocks x 32), all 24
...
`DAS_TUNE_MODE=tune` gen_tune_probe, kv-shape iso (2048x512x64, interleaved best-of-6,
...
declined/reference ~85. The 8-row x 2-slice x 4-block tile (16 weight vectors per block,
...
- **bias128 repack for vpdpbusd:** bake `w ^ 0x80` into the grp<mr> plane at repack (schema
  ...
  per-dot VPSIGNB (~28% of the busd dot-stage uops). Exact (u8 = w+128 in [0,255], x s8);
...
1. **bias128 repack for vpdpbusd** (above) - kills the per-dot VPSIGNB, ~28% of the busd
   dot-stage uops; layout-companion field, exact. **-> DONE, slice H** (emitted + gated;
...
is pure loss: +17% dot-stage uops (339 vs 289 instructions for the same 64 dots), 2x weight
...
(u8 = w+128  in  [0,255]); the dot becomes plain `vpdpbusd(acc, w_biased, x)` - no VPABSB, no
VPMOVB2M/VPSIGNB, no masked VPSUBB. Exact: Sum(w+128)*x = Sumw*x + 128*Sumx in i32 (|acc| <=
~1.6M per block against 2^31), and it REMOVES the x  in  [-127,127] sign-trick precondition.
The correction is folded as the ACCUMULATOR INIT: acc starts at -128*Sumx(block) instead of
...
Transport (where -128*Sumx comes from):

- **tile**: a bsums plane `xbsp : int const?` ([ntok x nb] i32, -128*Sumx per (token, block))
...
- **gemv**: mm_rows is a SHARED slot typedef invoked with raw activation planes from the
  fused decode chain - no plane transport. The block's -128*Sumx is computed inline (concat
  the two 16-byte chunks, one `vpdpbusd(0, splat(128u8), x)` at kernel width,
  `vector.reduce.add`, negate, splat): ~8 uops per (block, group) against the 24*rq the
...
Grid: +6 rows - busd 256-mr8/512-mr16 x kstep2 x {plain, gkstep2} biased, a biased
...
- **Registered A/B (gemm_1core, real shapes, 1 core): x64-gen batch 190-200 GMAC/s vs
  best hand tier avx512vnni 47-49 = 4.0-4.3x on every shape** (kv/qo/w13/w2); hot-shape
  gemv (kv 2048x512) 84.4 vs 48.8 (+73%); large-d gemv ~30 vs ~28 (DRAM-bound, as
...
above has gen 4x ahead kernel-to-kernel). gemma-3-1b (0.5-0.6 across the board) is the
...
rank gate). 75-cell paired sweep - 15 models x T{8,16,24,32,48}, gate-on, biased busd512-mr16
...
- **AMX scout (single core, userland)**: enable = `arch_prctl(ARCH_REQ_XCOMP_PERM,
  XTILEDATA)` + `ldtilecfg`, no kernel help needed. TMUL reg-resident **3546 GMAC/s = 22x
...
cannot accumulate i32 across blocks. K-step = ONE block (A colsb=32, B 8x64): per block,
`tdpbssd` then fold `C_i32 -> acc_f32` with the rank-1 scale product (per token row t:
`acc[t][*] += cvt(C[t][*]) * (xs[t][b] (x) ws_vec[b])`; ws is already block-interleaved
...
Grid rows: amx 1x1 (probe shape) + 2x2, decline-by-design everywhere off-SPR; semantics +
...
pinned 1, bias=0, nrsplit =^ tiles per macro side  in  {1,2}): `emit_amx_tile` builds the TMUL
...
(stride n), nt^2 `tilezero -> tdpbssd -> tilestored`-to-scratch, then the 16-row cvt*ws*splat(q)
fold - with **yp as the k-loop accumulator** (registers can't hold 32x32 f32): the tile
...
amx stamp covers 16*nrsplit tokens x nrsplit groups per call - so the stamp gained an
...
grew to ntok=32 (one full 2x2 macro, divides every other row's step).
...
48/48 - amx 2x2 tile = 4 tdpbssd / 4 tileloadd / 4 tilezero / 4 tilestored / 64 vcvtdq2ps /
0 sign-trick ops; 1x1 = 1/2/16; **ldtilecfg hoisted to entry + tilerelease at exit (NOT
...
**Tune sweep (single-core, d=512 fixture): amx WINS kernel-iso** - 1x1 macro 191.2 GMAC/s /
2x2 157.9 vs biased busd512 156.4 (+22% for 1x1; note 191 ~ the scout's DRAM-fed ceiling
...
3259 (amx 1x1) / 2462 (2x2); 1B pp 2413 vs 979/1080. T8 discriminator (1B): 0.64x vs 0.41x
at T24 => the loss is BOTH structural (~1.6x at low T) and scaling-amplified (AMX frequency
...
**[key] TUNE-METHODOLOGY FINDING (needs a decision):** the single-core tile microbench CROWNED
amx nrsplit1 - `tune_for_this_box` on SPR would write a manifest that loses ~2x end-to-end.
...
prefill, production stamps + box_profile knobs, `_jit_fast_math` parity), interleaved F/W x2
best-of, challenger needs > x1.02, wrong-stamp or crashing child scores 0 (a SIGILL-ing
...
this same pass would have rejected the amx crown at ~0.5x emphatically.

**The arch ladder (Boris's ask, same session - full CSV in the session scratchpad):** 4 models
x 3 ISA tiers x both engines x T{8,24}, both engines genuinely tier-limited (das: explicit
...
READING: the engines climb in different places - das gains 1.9-2.4x from the VNNI step
(bias128 busd512) and nothing from amx; lcpp gains 1.1-1.3x from VNNI and 2.3-2.9x ONLY from
the AMX unit. **das-on-VNNI ~ lcpp-on-AMX within 8-15% on 3 of 4 models** (gemma = their
standout amx case, 1.49x); same-tier head-to-head das wins prefill 1.4-2.8x everywhere.
...
**lcpp AMX source read (why theirs wins where it wins; mmq.cpp):** same 2x2 macro geometry,
...
packed layout), and their GENERIC path chunks matmuls 2-D (out-rows x tokens, rows-fastest
...
**Slot scope: the batch tile.** SMMLA is a 2x2 MMA - `acc[i][j] += a_row_i * b_row_j` over 8
k-bytes, 32 MACs/instr, no horizontal reduce; the M3 scout measured 2.21x sdot4 GMAC/s
register-resident ([[dasllama-m3-air-box]]). The 2x2 shape needs two tokens per A operand, so
...
**The layout is NOT ours this time - kgroup=8.** smmla's B operand is a ROW-PAIR x 8 k-bytes
...
16 smmla + 4 uzp vs sdot's 20 loads + 32 sdot - ~1.3-1.4x ALU-side expected at the kernel
(the 2.21x is the no-marshalling ceiling; fold + A-pairing eat the rest). A-side pre-pairing
...
pipes. 1 pipe x 32 MACs < 4 pipes x 16 MACs, structurally. The scout's 2.21x was the
...
- **I2 - 2-D grid on the amx arm.** lcpp mmq.cpp:2472: MBxNB grid of 32-tok x 32-row
  ...
  ts!=4 walk (ts-token x gs-group cells); rides the same batch_grid_2d pin. Small.
...
- (noted, deferred: lcpp M in 2..15 = tile sub-blocks, M==1 = 64-row VNNI walk; our
...
- **I4 `pipe=1`** - double-buffered C spill (2xnt^2 tile regions, one alloca), peel block 0
...
- **I1 `latch=1`** - implemented as the RAW immediate-tmm intrinsics (`llvm.x86.tdpbssd`
  ...
  risk, no worker identity needed) and amortizes identically. [key] harness trap found: an
...
- **I3 `bias=128`** - the dot is **TDPBSUD** (signed A tiles x unsigned w^0x80 B plane):
  ...
  w^0x80 is w+/-128 piecewise by sign), but SU-order is Sumw*x + 128*Sumx exactly, and the
  existing xbsp bsums plane (-128*Sumx i32) lands in the FOLD stage as one embedded-broadcast
...
40/40 x3 incl. amx-cumulative + grid-on) -> Llama-1B @T24 full lattice (10 rows x grid{0,2} x
2 reps, 40 cells, zero failures, every cell stamp-verified) -> condensed lattice on
Qwen3-0.6B / gemma-3-1b / Qwen3-4B.

**Verdict: amx < vnni everywhere - 0.36-0.55x the biased-busd512 champion across all four
models; the best cell ever (pipe1_latch1 on Qwen3-0.6B, 0.55x) is below the plain AVX2
...
- **Frequency license exonerated** (P1 turbostat, bench-phase-anchored): das-amx 3745 MHz ==
  ...
  drops to 3322 MHz (-11%) and still wins 2.7x - they pay the license and win anyway.
- **Dispatch mostly exonerated** (P2, the new `DASLLAMA_TEAM_PROF` rail in prefill_perf,
  `0719968ed`): publish 528ns/op (negligible), join-tail 283us vs vnni's 29us ~ 10-15% of op
  time (coarse 32-token units -> ragged waves) - real but secondary.
- **The bill: serve 2.28x** (1125us vs 493us per op, N=512 - the chunk kernels themselves).
  Structural root = the kstep=1 Q8-scale-boundary fold: every 32-k block spills all four C
  tiles and folds on vector ports - ~256KB C-spill traffic per 32-row x 32-token unit vs
...
nrsplit1 (174 GMAC/s on the d=32 fixture), confirm child measured 0.49x e2e, REJECTED,
fallback pinned - the session-3 methodology hole closed on the box that exposed it.
**batch_grid_2d SPR verdict: no pin** - 135M @T48 champ interleaved x2: grid2 within +/-1%,
...
segments per spill halves the C traffic; honest ceiling ~ 1.6x behind the champion, so it
...
- Evidence: `history/dasLLAMA/epyc9654_measurements.md` (sessions 3b-5: why portable+pin wins today, the
  0.73-0.86x fleet column, the Genoa zmm-engine finding); `~/Downloads/M1_results.md`
  (M1 baseline: prefill 0.92x, decode 0.84x median - local, not committed).
