...
intrinsic pattern, the u8xs8 gotcha); this doc is the order of operations with acceptance
...
`+avxvnniint8` (`vpdpbssd`, natively s8xs8 - no sign trick needed). Decide the target
...
- **Never benchmark interpreted.** If a number looks 100x off, check you passed `-jit`.
