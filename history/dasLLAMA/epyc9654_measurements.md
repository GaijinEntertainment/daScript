...
| Storage | 2x3.5 TB NVMe; root was a tiny 10 GB RAID1, **claimed the free 3.83 TB into `/data`** |
...
| `test_mxfp4`/`test_groupn` internal backend sweep w/ `AVX_MATRIX=1` | matrix backends vs scalar leaf yes (this is what actually covers them end-to-end in the suite) |
| **gpt-oss-20B full MoE forward, token-for-token vs golden**, pinned per backend | portable yes * x64-avx2 yes * **x64-vnni256 yes** * **x64-avx512vnni yes** - all MATCH (40 tok, short + window-engaged) |
...
|---|--:|--:|--:|--:|:--|
| kv 2048x512 | **693** | 460 | 357 | 277 | 48 |
| qo 2048x2048 | 846 | **965** | 921 | 743 | 96 |
| w13 2048x5632 | 1000 | 1042 | **1255** | 943 | 144 |
| w2 5632x2048 | 888 | **1211** | 1199 | 977 | 96 |
| cls 2048x32000 | 1065 | 1379 | 1553 | **1620** | 192 |

### Row-major tier - `x64-avx2` (per-core at T=96 in parens)
| shape | T=48 | T=96 | T=144 | T=192 |
|---|--:|--:|--:|--:|
| kv 2048x512 | 478 | 308 (3.21/c) | 321 | 222 |
| qo 2048x2048 | - | 671 (6.99/c) | - | - |
| w13 2048x5632 | 794 | 781 (8.13/c) | 1050 | 743 |
| w2 5632x2048 | - | 783 (8.15/c) | - | - |
| cls 2048x32000 | 813 | 836 (8.71/c) | 1215 | 963 |
...
- Small `kv` (d=512) peaks at 48 and **craters 2.5x at 192** (693->277).
...
- There is a par-**threshold** (thread or not: `n*d >= g_matmul_par_threshold`) but **no
  min-chunk-*size***.
- `num_chunks` = `get_dispatch_lanes()` (~192) for GEMV, `chunks_per_job x get_dispatch_lanes()`
  for quantized/batch: **q8 batch = 4 x 192 = 768 chunks over `d` rows**, fixed, independent of `d`.
- The x4 oversplit (`g_q8_batch_chunks_per_job`, math.das:154-158) was **deliberate** (idle
...
1. **Min grain floor** (`:1388-1393`): `chunk_size = 16` rows, bumped to **64** for a GEMV
   (`nr0==1 || nr1==1`). Grid = `ceil(nr0/16) x ceil(nr1/16)`. No chunk is ever finer than 16 rows.
...
|---|---|---|
| min chunk grain | **16 rows** (64 for GEMV) | **none** - chunk = `d / (cpjxlanes)`, can be < 1 row |
| chunk count for small op | capped: `min(nth, [d/16])`, fallback <= `nth` | fixed `cpj x lanes` (q8 batch = 4x192 = **768**) |
| the `x4` factor | a **guard ceiling** (`< nth*4` => simplify) | a **multiplier floor** (`g_q8_batch_chunks_per_job`, always x4) |
...
hurts; together they produce the `kv` 2.5x cratering.
...
- `MIN_GRAIN = 16` mirrors ggml (start there; sweep 8/16/32/64 on this box). GEMV/decode may want
  the 64 bump.
- Keeps the x4 straggler-steal oversplit for *large* `d`, kills it for *small* `d`.
...
|---|--:|--:|--:|--:|--:|
| kv 2048x512 | 202 | **318** | 258 | 194 | 176 |
| qo 2048x2048 | 299 | **707** | 618 | 458 | 311 |
| w13 2048x5632 | 571 | 984 | **1007** | 796 | 668 |
| w2 5632x2048 | 627 | **962** | 914 | 647 | 370 |
| cls 2048x32000 | 1262 | **1614** | 1533 | 1593 | 1241 |

**At the 192-lane default the grain floor is a 1.3-2.4x win on every shape** (qo 299->707).
...
still won 1.6-2.4x** - the grain win is real and under-, not over-stated. What does NOT survive:
...
1. **kv (small-d) likely still short of its low-thread optimum**: row-only splitting can't feed
   ...
   kv-like shapes get 32x32 = 1024 tiles of 16x16). 2D chunking = the structural fix ->
...
3. Bench runs are sequential per config (no in-process grain interleave yet) - treat all
   cross-config ratios >~noise-floor accordingly; the 1.3-2.4x T=192 wins are far above it.
...
**Quantified** (`benchmarks/core/jobque/dispatch.das` + new team arm): fifo fork/join ~ **10 us
PER JOB** (alloc+fifo+wake+waitgroup) -> 96-chunk dispatch ~ 1 ms; x ~160 dispatches/decoded
token ~ the entire budget -> the observed Llama-1B 7.6 t/s vs llama.cpp 139 (18x). On M1 (9
lanes) this was ~90 us/dispatch - invisible. ggml pays ~us spin barriers instead (persistent
...
| Llama-1B decode @95w default | 7.6 | **43** (5.7x) |
...
their flat curve is carried by the ~us barrier alone. Our grain-64 env test: decode
43.9->**53.8** t/s at default (+13-15%). Full fix = cheaper rendezvous (barrier-cheap epoch),
NOT lane limiting - see session 2.

**Day total: Llama-1B decode 7.6 -> 97 t/s (12.7x) with knee-threads, 43-54 at defaults;
prefill-512 2x.** Fixes: min-grain clamp + phys-core cap (committed) + team-default (pending
...
### Per-op attribution (llama3_perf built-in profile, us/token, 64-tok window)
...
1. **Per-shape grain conflict:** mm_qkv wants coarse (2.6x win at 128); mm_ffn wants fine - its
...
2. **The dispatch tax is ~flat per dispatch and scales with lane count:** every bucket except
   `final` costs ~2x more at 96 lanes than at 24 at comparable chunk counts; the defaults-vs-T24
   delta ~ 86 us/dispatch x 81 dispatches/token. Chunking can't remove it (team rendezvous
...
Hypothesis: the ~86us/dispatch tax = claim-line refetch storm (spinners acquire-poll `mTeamSeq`
...
| last worker claim starts | +9.2 us | +3.3 us |
| caller-serve (its ~1 chunk of ~40) | 107 us | 68 us |
| join-tail after caller done | 24.5 us | **41.1 us (37% of op)** |
| total | 133 us | 110 us |
**The rendezvous is ALREADY barrier-cheap** - sub-us publish, 200 ns worker reaction, 0% solo
ops. The "~86 us/dispatch tax" was a confound: per-chunk COMPUTE inflates with active-lane count
(same-shape chunk 68->107 us from T24->T96 - bandwidth/frequency contention), not sync. Protocol
...
kept.** T24 92.2-96.0 (was 94.3-94.9), T96 g64 51.3 (was 49-52). Join-tail 41->31 us, but larger
...
- **Knee (T=24): pure memory-path efficiency.** Per decoded token both engines stream the same
  ...
  both against the same ~180 GB/s 24-core-DRAM ceiling. Sync us, chunking exhausted -> the 1.45x
...
Per-token us @ T=24, Llama-1B Q8 (lcpp 138.7 t/s; ours under each backend):
| bucket | lcpp | auto=avx2-repack | portable+tuned | port/lcpp |
|---|--:|--:|--:|--:|
| mm_ffn | 3884 | 5873 | 5535 | 1.42x |
| mm_qkv | 664 | 1969 | **877** | 1.32x (was 2.96x) |
| final/cls | 1243 | 1563 | 1490 | 1.20x |
| mm_wo | 348 | 739 | **736 (unmoved)** | 2.11x |
| attn | 173 | 464 | 333 | 1.93x |
| rope+kv | 181 | 82 | 99 | we win |
| TOTAL | 7083 | 10970 | 9337 | 1.32x |
...
4. Remaining vs lcpp at tuned-portable: ffn 1.42x, wo 2.11x (the one unmoved bucket - small-d
   GEMV, suspect per-op fixed cost: requant + entry), cls 1.20x, attn 1.93x (team-gated
...
### Session 3d: the M1 cross-check - the picture is coherent across uarchs
...
uarchs); batch/prefill = compute-bound GEMM -> hand/batch tiers win (laneq on M1; row-major
...
- [x] ~~Decode lane cap~~ CLOSED session 2c - the "86 us tax" was per-chunk compute inflation
...
inert as designed (66.9 vs 68.0; 2x16 < 97 lanes). **Peak 118.3 = 85% of lcpp 138.7; bracket
...
**T=8 + T=16 fleet result (acc8-pin config):** prefill uniformly 0.60-0.67x lcpp on every dense
model; emit scales with size - 0.24x (135M, FLAT across T = the 2M par threshold leaves all
non-cls matmuls single-lane) up to 0.90-0.97x (4-8B); gpt-oss outlier 0.39x/0.23x. Full tables
in the session transcript; raw logs `/data/sweep/T{8,16}_*`.

**The "we lost repack" scare - resolved, no regression anywhere:**
- Boris desktop (3990X Zen2, master tip): 1B pp 361 vs lcpp 409 at 16C (0.88x) - but **+20% WIN
...
- Genoa box, same code, same model, T=32: ours 565/571 pp vs lcpp 1072 = **0.53x**. The flip is
  the opponent, not us: lcpp's `arch/x86/repack.cpp` compiles AVX512VNNI **zmm** GEMM paths on
  Genoa (absent on Zen2) ~ 2x per-core int8 throughput. We out-kerneled AVX2-lcpp; VNNI-lcpp is
  a different engine.
...
split, exactly the hybrid design's use case. Gap after pin: 0.77x/0.71x/0.64x @T=8/16/32.
