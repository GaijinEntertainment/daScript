...
- The decoder serves Q8-NATIVE on GPU (Boris ruling 2026-08-16, after the whisper.cpp scout:
  ...
  CPU q8 decode is q8-weight x q8-ACTIVATION integer dots (`matmul_q8q8_batch`), the GPU
  lane is q8-weight x f32-activation (whisper.cpp's regime) - transcripts vs the CPU q8
  serving default are the gate, the f32 rail is the logits rung; a near-tie flip is judged
  with fixtures, not papered over.
- `whisper_cross_kv` is once per 30 s window: per layer two `[1500xd]x[dxd]` GEMMs + the kx
...
- Knockout attribution: the chain minus attention runs 1.68 ms/step - attention was ~41%
  at ~145 us/dispatch (20 threadgroups = the occupancy wall).
...
minted once per tower, ~18 MB). One command buffer per WINDOW (all chunks x three stages);
...
- **N - cross_kv on GPU.** `register_whisper_cross_kv_gpu` (tower contract: decline/false,
  counter deltas). Per window: upload enc_out once, per layer two q8-weight x f32-act GEMMs
...
- **Q - qwen3a conv2d frontend.** GPU im2col for `conv2d_s2` (stride-2 pad-1, c-fastest
  rows) x3 convs + the feature shuffle + conv_out GEMM + bias + `enc_gelu_erf` + pos-add;
