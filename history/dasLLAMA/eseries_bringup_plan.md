...
- Batched PLE on the GPU (the multi-stream Metal arm) - `followup_general.md` #40,
  long-term differentiation (upstream has no batched-PLE path either).
...
- **C. The matrix sweep - DONE**: CPU fp32/q8/q4 chat turns + gather bit-match (slice A's
  ...
  95.6 +/- 1.0 tok/s, pp512 1948 +/- 5 tok/s - ~2.3x the CPU's ~42, just above the predicted
  1.4-2.2x band. Landed along the way: the image-rail PLE tripwire moved to the after-borrows
...
- P4 (Metal perf): E2B Metal decode lands 1.4-2.2x the CPU q8 decode (memory-bound:
