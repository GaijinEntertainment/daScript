...
- **Every LLM section closes with a prefill AND decode A/B vs llama.cpp** (Boris,
  ...
  fires). Precedent: Wave Q (prefill 1.59x / decode 1.36x vs lcpp), Wave G2
  (prefill grouped 5.5x -> leads lcpp 1.44x).
...
1. **gemma-4-31B dense - DONE DONE 2026-07-07 (verify-only, zero code).** Header confirmed
   the same gemma4 arch as the 12B, scaled: 60 layers, embd 5376, 32Q / [16x5,4]KV heads;
...
2. **gemma-4-26B-A4B MoE - DONE DONE 2026-07-07. Real MoE slice; ~17 tok/s decode (M1 Max, 4B active).**
   ...
   **KEY LESSON - MoE routing is near-tie-brittle.** The router RMS feeds a DISCRETE top-8-of-128
   ...
   Header: gemma4 arch, 30 layers, embd 2816, ff 2112, 16Q / [8x5,2]KV heads, **128 experts
   ...
   - **Routed experts** - `ffn_pre_norm_2` -> MoE -> `ffn_post_norm_2`. Router is *custom-normalized*:
     `RMS_norm(attn_out) x 1/sqrtn_embd x learned ffn_gate_inp_s`, then `ffn_gate_inp` matmul -> logits.
   ...
   **vs qwen2moe** (the closest): gemma4 renormalizes top-k (qwen2moe doesn't), has a learned
   router input-scale + RMS/sqrtd normalization (qwen2moe's router is a plain matmul), fuses
   ...
   **PREFILL grouped-batched (DONE 2026-07-07).** The 26B prefill was naive per-token (each position
   looped through `gemma4_moe_ffn`) -> **35 t/s, 0.23x lcpp**. New `gemma4_moe_prefill_grouped` +
   ...
   Measured (M1 Max, prefill-512, interleaved A/B): **grouped 217 t/s vs naive 39 = 5.5x**, and **217
   vs lcpp 151.1 = 1.44x - das now LEADS lcpp on 26B prefill.** Correctness held: the counting-prompt
   1492-token prefill still produces `GEMMA4_26B_GEN` token-for-token, grouped==naive==fixture.
3. **gemma-4-E2B / E4B - the edge models. DONE TEXT DONE 2026-07-07 (2 commits: E4B then
   ...
   - **PLE** (both): new fblob regions (fp32 `per_layer_model_proj` + `per_layer_proj_norm`
     + per-layer `post_norm`) + Q8 big weights (the `[vocab x ple*layers]` token table,
     per-layer `inp_gate`/`proj`). Pre-step per token (gather xsqrtple; project x1/sqrtdim ->
     RMSNorm -> +gathered -> x1/sqrt2) + a per-layer block after the FFN residual, before the
     layer scale: `cur += post_norm(proj*(gelu(inp_gate*cur) (.) ple_inp[l]))`. [key] the token
     table is `[vocab x (ple*layers)]` - load ALL `ple*layers` per row (the initial bug was
     loading only `ple`).
   ...
   - **Per-layer FFN width (E2B ONLY)** - `feed_forward_length` is a per-layer ARRAY on E2B
     (6144 x15 then 12288 x20 - the elastic MatFormer sub-model of the E4B; E4B is uniform
     ...
     [key] `ffn_w` lives on the **Model**, NOT Config - Config must stay copyable (`let c =
   ...
   - **DONE Section-closing prefill + decode A/B vs lcpp** (M1 Max, Parsec off, box profile,
     ...
     - **E2B** - prefill das **376 t/s** (pp512) / 405 (pp256) vs lcpp **382** = **0.98x**
       (ahead at pp256); decode das **36.0 t/s** (ctx8) vs lcpp tg128 **36.9** = **0.98x**.
     - **E4B** - prefill das **178.7 t/s** (pp512) vs lcpp **192.9** = **0.93x**; decode das
       **18.7 t/s** (ctx8) vs lcpp tg128 **19.2** = **0.98x**.
     ...
     Accelerate-BLAS (AMX-backed), and the larger E4B GEMM favors AMX more (hence 0.93x vs
     E2B's 0.98x). Decode is bandwidth-bound -> tied both models. Rigs: `benchmarks/
     prefill_perf.das` (batched vs per-token), `benchmarks/decode_prof.das` (two ctx windows).
     **Ledger:** the E4B dense-prefill 0.93x is the AMX-access asymmetry (documented, boxes
     w/o AMX exposure). Potential das-side lever: a gemm-gen tuned Q8 kernel for the E-series
     dims (dim 2560 x ff 10240) - not chased mid-wave.
...
## Wave Q - Qwen3-MoE (best value/cost on the LLM side) - DONE CORRECTNESS DONE 2026-07-07

**Qwen3-30B-A3B - DONE DONE (config-only arch, zero new forward code).** VERIFY resolved GREEN
against llama.cpp `build_qwen3moe` (`src/models/qwen3moe.cpp`): standard attention with per-head
Q/K-norm before RoPE (our `c.qk_norm`), `1/sqrthead` scale, no QKV bias; `build_moe_ffn` with softmax
...
- **Prefill A/B (M1 Max, PP=512, Parsec off, box profile, interleaved best-of-5):** das grouped
  **206.8 t/s** vs das naive per-position 44.5 = **4.64x** (internal sanity - grouped batching engaged,
  like the 26B's 5.5x); vs lcpp CPU `-ngl 0 -t 8` **130.0 t/s** = **das LEADS lcpp 1.59x**. Robust:
  the pessimistic settled das rep (195) still beats lcpp's optimistic end (136) by 1.43x. Grouped is
...
- **Decode / emission (M1 Max, box profile):** das **27.5 tok/s** fresh (ctx 8) / 24.8 (ctx 512) vs
  lcpp CPU tg128 `-ngl 0 -t 8` **20.2 t/s** = **das LEADS lcpp 1.36x**. Lands in the plan's predicted
  25-30 t/s band - fully interactive; the A3B (3B-active) shape delivering as expected. So das leads
  lcpp on BOTH prefill (1.59x) and decode (1.36x) for qwen3moe-30B.
...
`gemma4_moe_prefill_grouped` slice (2026-07-07, 5.5x -> leads lcpp 1.44x).
...
### Oracle rail - DONE DONE 2026-07-07 (the wave's designated first sub-task / main risk).
...
- **[key] macOS gotcha - brew-python `_decimal` fails (mpdecimal 3->4 ABI drift).** NeMo->lhotse
...
- **Decoder = plain Qwen3-1.7B we already own** (`llm.base_model.model.model.*`): 28 layers,
  hidden 2048, ff 6144, 16 Q-heads / 8 KV-heads x 128, q_norm/k_norm [128] (qwen3 qk_norm),
  ...
  **LoRA** on `q_proj` + `v_proj` only (`.base_layer.weight` + `.lora_A`[128,2048] +
  `.lora_B`[out,128]); r=128 alpha=256 -> **merge `W += (alpha/r)*B*A = W + 2.0*B*A` at conversion**,
...
### daslang model-implementation phase - DONE DONE 2026-07-08. Token-for-token vs the NeMo SALM oracle.
...
- **(2) New module `dasllama_canary.das`** - a clean fp32 FastConformer-32 forward (NOT a
  ...
  into BN. [key] **The load-bearing delta from parakeet was NeMo's `MaskedConvSequential`** - it
...
- **(6) Section-closing perf A/B rides `benchmarks/asr/` vs the NeMo reference engine**, exactly
  ...
  `benchmarks/asr/results.md` + committed TSVs; das **leads** the NeMo reference ~1.3-1.5x even
...
## Wave A2 - Gemma-4 E-series audio input - DONE DONE 2026-07-08 (E2B; encoder token-exact)
...
- **Architecture (verified against `tools/mtmd/models/gemma4a.cpp`, NOT the pre-probe notes):**
  ...
  - Subsample: Conv2D x2 (k3 s2 p1) -> LayerNorm-over-channels (weight-only, eps 1e-6) -> ReLU,
  ...
  - **Attention is CHUNKED-LOCAL, not "full self-attn"** - it reduces to a plain causal
    ...
    `RPE_row = 12 - (i-j)`, sinusoidal RPE positions `12-p`; q_scale=(1/sqrtdh)/ln2,
  ...
  - Conv module: RMSNorm -> pw1 -> GLU (sigmoid gate on 2nd half) -> causal depthwise conv1d k=5
    (ssm_conv, left-pad 4) -> RMSNorm -> SiLU -> pw2. [key] **`norm_conv`/`conv_norm` are name-swapped
    ...
    0.74 -> 0.0027).
  - [key] **Per-op activation-quant calibration ranges** (`input_max/min`, `output_max/min`) honored
...
- **Ledger:** the E2B encode 16x gap is a gemm-gen tuned **Q8 audio-tower kernel** (dims: d_model
  1024 x ff 4096, conv pw 1024<->2048) - the obvious follow-up; and the ~500-position long-context
...
- (a) Encoder = **`gemma4a` conformer** (`clip.audio.projector_type='gemma4a'` on both
  ...
  (`tools/mtmd/models/gemma4a.cpp`): subsample Conv2D x2 (stride 2) -> per block { dual
...
## Wave A3 - Qwen3-Omni-30B-A3B, audio-in/text-out - DONE DONE 2026-07-08. FULL token-for-token.
...
shipped**:
- **Oracle DONE** - llama.cpp mtmd supports Qwen3-Omni audio (PR #19441, in the local checkout;
  `llama-mtmd-cli --temp 0 --jinja` runs it). The audio wrapper is `<|audio_start|>...<|audio_end|>`.
- **Model DONE** - `ggml-org/Qwen3-Omni-30B-A3B-Instruct-GGUF` ships a READABLE **Q8_0 thinker
  (32.5 GB)** + **bf16 audio mmproj (2.2 GB)** - no K-quant blocker, no conversion. On disk 2026-07-08.
- **Thinker DONE** - converts as arch **`qwen3vlmoe`**, NOT plain qwen3moe. But `qwen3vlmoe` =
