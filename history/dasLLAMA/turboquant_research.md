...
Companion to `kv_cache_compression.md` sec.8. Corrections to that file discovered here:
...
- **File evidence**: `ggml/src/ggml-turbo-quant.c` (36 KB, dedicated file). Contains:
  ...
  - **QJL IS implemented**: legacy `TURBO4_0` = 3-bit PolarQuant + 1-bit QJL sign-of-projected-residual (`turbo_qjl_matrix`, `TURBO_QJL_CONST = 1.2533141` = sqrt(pi/2), the QJL unbiasedness constant). But the modern default (`TURBO4_USE_4BIT`) **drops QJL** for pure 4-bit Lloyd-Max.
...
Notation: `x  in  S^{d-1}` is the vector being stored (a key or value), `y` is the query-side vector, `b` = bits per coordinate.

### 1. Pipeline structure (store time vs read time; K vs V)

**Store time** (per vector `x`, assumed unit norm; otherwise store `||x||_2` in fp and rescale at read):

- **`TurboQuant_mse`** (Algorithm 1): `y <- Prod*x` (random rotation); for each coordinate `j`: `idx_j <- argmin_k |y_j - c_k|` over a fixed `2^b`-entry scalar codebook. Store `idx` (`b*d` bits).
- **`TurboQuant_prod`** (Algorithm 2), the inner-product/unbiased variant: run `Quant_mse` at bit-width **b-1**; compute residual `r <- x - DeQuant_mse(idx)` (**in the original basis** - DeQuant includes the back-rotation `Prod^T`); compute `qjl <- sign(S*r)` (`d` sign bits, `S` is `dxd` i.i.d. N(0,1)); store `(idx, qjl, gamma=||r||_2)`. Total `b*d` bits + one fp scalar `gamma` (+ one fp scalar `||x||_2` for non-unit inputs).

**Read time**: dequantization is linear: `x = Prod^T*c[idx] + (sqrt(pi/2)/d)*gamma*S^T*qjl`. Inner product with query `y`:

```
<y,x> ~ <y, x_mse> + gamma*(sqrt(pi/2)/d)*<S*y, qjl>  =  <Prod*y, c[idx]> + gamma*(sqrt(pi/2)/d)*<S*y, qjl>
```

so per query you compute `Prod*y` and `S*y` once, then per stored vector do a codebook-LUT dot plus a +/-1-weighted sum. The estimator is **asymmetric - the query is never quantized** (stated as essential in QJL Def. 3.1: symmetric sign-quantization gives an angle estimator, and cos(*) of it is biased).

**K vs V: the paper does NOT differentiate them.** No sentence assigns different variants or bit widths to keys vs values; all KV experiments treat "KV cache" as a unit. Deliberate contrast with the prior QJL paper, which used QJL for keys and plain per-token integer quantization for values. *Inference (not in paper)*: since K feeds `q*k^T`, `Q_prod` is the natural choice for K (unbiasedness matters pre-softmax); V is consumed as a convex combination, where `Q_mse`'s bias largely averages out - an implementer could use `Q_mse` for V and save the sign-bit plane + gamma, but the paper neither says nor evaluates this.

### 2. Stage 1 rotation

- **Construction (explicit)**: "Generate Prod by applying QR decomposition on a random matrix with i.i.d Normal entries" - a Haar-random `dxd` orthogonal matrix. **One fixed Prod per quantizer instance** (Global Parameters), shared by all vectors that instance quantizes, reused at read time.
- **Granularity for KV (partially explicit)**: the LongBench setup splits the **128 channels** (= head_dim of Llama-3.1-8B and Ministral-7B) into outlier/non-outlier sets and runs "two independent instances of TurboQuant" on them - so `d` is (a partition of) the head dimension, e.g. a 32x32 and a 96x96 rotation for the 2.5-bit point. Whether Prod is shared across layers/heads, and how outlier channels are identified, is **not stated**; the sibling QJL paper identifies outlier channels "during the prompt phase" per layer - *reasonable default, flagged as inference*.
- **Seed handling: not discussed at all.** *Implementer decides*: any PRNG; Prod (and S) must be reproducible/stored between store and read.
...
- **Target distribution (explicit, Lemma 1)**: after rotation, each coordinate of a unit vector has density `f_X(x) = Gamma(d/2)/(sqrtpi*Gamma((d-1)/2))*(1-x^2)^((d-3)/2)` on [-1,1] (a scaled/shifted Beta), converging to **N(0, 1/d)** for large d. Coordinates nearly independent in high d, so scalar quantization per coordinate is near-optimal.
- **Construction (explicit)**: 1-D continuous k-means / Lloyd-Max, Eq. (4): minimize `Sum_i integral |x-c_i|^2*f_X(x) dx` over Voronoi cells (boundaries = midpoints). "We solve [it] for a range of practically relevant bit-widths b **once, and store the results**" - fixed constants, computable offline, **one shared codebook for all coordinates**, scaling as `1/sqrtd`.
- **Values the paper lists (explicit, Gaussian-limit)**: b=1: `{+/-sqrt(2/pi)/sqrtd}` (=+/-0.7979/sqrtd); b=2: `{+/-0.453/sqrtd, +/-1.51/sqrtd}`. b=3,4 not printed - *inference*: classic Max(1960) Lloyd-Max Gaussian levels scaled by 1/sqrtd (8-level: +/-0.2451, +/-0.7560, +/-1.3439, +/-2.1520; 16-level: +/-0.1284, +/-0.3881, +/-0.6568, +/-0.9424, +/-1.2562, +/-1.6181, +/-2.0690, +/-2.7326). The paper's own D_mse values 0.36/0.117/0.03/0.009 match the standard Gaussian Lloyd-Max MSEs 0.3634/0.1175/0.0345/0.0095, confirming this reading.
...
- **Projection (explicit)**: `S  in  R^{dxd}`, i.i.d. N(0,1) - QJL with **m = d** projections, 1 bit per coordinate. (In the QJL paper m is free; `m ~ eps^-^2*log n` suffices for n keys.)
- **Quantizer / dequantizer (explicit, Def. 1)**: `Q_qjl(r) = sign(S*r)  in  {-1,+1}^d`; `Q_qjl^-^1(z) = (sqrt(pi/2)/d)*S^T*z`.
- **Residual (explicit)**: `r = x - Q_mse^-^1(Q_mse(x))`, with `E[||r||^2] = D_mse at (b-1) bits`; `gamma = ||r||_2` stored exactly (fp).
- **Estimator (explicit)**: `<y, x> = <y, x_mse> + gamma*(sqrt(pi/2)/d)*<S*y, sign(S*r)>`; unbiased (Theorem 2, via QJL Lemma 3.2 applied to r/gamma).
- **Variance (explicit, Lemma 4 + Thm 2 proof)**: `Var <= (pi/2d)*||r||^2*||y||^2`, hence `D_prod <= (pi/2d)*||y||^2*D_mse(b-1) <= (sqrt3*pi^2*||y||^2/d)*4^{-b}`. At b=1..4: `D_prod ~ 1.57/d, 0.56/d, 0.18/d, 0.047/d` (x||y||^2). QJL high-probability form: `|est - <q,k>| <= eps||q||||k||` w.p. 1-delta for `m >= (4/3)(1+eps)eps^-^2log(2/delta)`; attention scores get (1+/-3eps) relative error for `m >= 2r^2eps^-^2*log n`.
- Why two stages: `Q_mse` alone is biased for inner products - at b=1 it degenerates to `sign(Prod*x)` with `E[<y,x>] = (2/pi)*<y,x>` (explicit example sec.3.2); bias shrinks with b but never vanishes. Stage 2 makes the estimator exactly unbiased while adding only the residual's variance.

### 5. Bit accounting

- **Core accounting (explicit)**: bit-width b means `B = b*d` payload bits per vector: `(b-1)*d` index bits + `d` sign bits for `Q_prod`, or `b*d` index bits for `Q_mse`. **Metadata is extra and uncounted**: `gamma` (per vector, `Q_prod` only) and `||x||_2` (per vector, non-unit inputs) in fp. No block/group scales.
- **Where 3.5 (and 2.5) comes from (explicit, sec.4.3)**: NOT fractional-bit coding - **mixed per-channel-set allocation over head_dim=128**: "in our 2.5-bit setup, 32 outlier channels are quantized at 3 bits, while the remaining 96 channels use 2 bits -> (32x3+96x2)/128 = 2.5. For 3.5-bit quantization, a different ratio of outliers and regular channels leads to a higher effective bit precision" - the 3.5 split ratio is **not given** (64/64 at 4/3 bits would produce it; *inference*). Two independent TurboQuant instances, one per channel set; outlier strategy credited to QJL [63] and RotateKV [51].
- **Other operating points**: theory covers all `b >= 1` (Q_prod) / `b >= 0` (Q_mse); experiments use 2.5 and 3.5 bits for KV, 4-bit for NNS timing, NIAH at "memory compression ratio 0.25" (~4 bits effective vs fp16; *arithmetic inference*). Claimed compression: "at least 4.5x" at 3.5 bits, "exceeding 5x" at 2.5 bits (metadata not counted).
...
- **Per query (once per rotation instance)**: two `dxd` matvecs, `u = Prod*y` and `v = S*y`. No per-key dequantization to a full fp vector is ever needed.
- **Per K vector (dot product)**: `Sum_j u_j*c[idx_j] + gamma*(sqrt(pi/2)/d)*<v, qjl>`.
...
- **Per V vector (weighted sum)**: dequantization is linear, so accumulate in quantized domain: `Sum_i w_i*v_i = Prod^T(Sum_i w_i*c[idx_i]) + (sqrt(pi/2)/d)*S^T(Sum_i w_i*gamma_i*qjl_i)` - LUT-weighted accumulations per token, then **one** `Prod^T`/`S^T` application per attention output, not per token. (*Inference; the paper never spells out the V-side read path.*)
...
- **"Within 2.7x of the bound" (explicit)**: applies to the **MSE variant**: upper `(sqrt3pi/2)*4^{-b}` vs lower `4^{-b}` (Theorem 3) - ratio `sqrt3pi/2 ~ 2.72` asymptotic, ~1.45 at b=1. The **inner-product variant's** gap is bigger: `sqrt3pi^2 ~ 17.1` vs `(||y||^2/d)*4^{-b}` - the abstract's 2.7 does *not* apply to Q_prod.

### 8. Theory-vs-practice gaps (implementer must decide)

Explicitly absent from the paper (each verified by search):
- **RoPE**: never mentioned. *Inference*: quantize keys post-RoPE (the estimator reconstructs `q*k^T` of whatever was stored); pre-RoPE storage would require dequantize->rotate->dot and breaks the LUT read path.
- **Which variant for K and V** - not stated. **Per-head vs fused** - 32/96 split of head_dim=128 implied; sharing of Prod/S across heads/layers unstated. **Outlier-channel identification** - unstated (QJL: by magnitude during prompt phase, per layer).
- **Attention sinks / recent full-precision window: none** - quantizes everything, including streamed generated tokens.
- **Seeds/PRNG, storage of Prod and S**, fp precision of gamma and norms, metadata overhead accounting (gamma + norm per key per channel-set ~ ~0.5 bit/coordinate at fp16 if per-128-dim; the paper's "b bits per channel" excludes this).
...
- **Store time per vector**: one `dxd` matvec (`Prod*x`) + nearest-centroid per coordinate; `Q_prod` adds a second matvec (`S*r`), a subtraction, a norm. All batchable - substance of the "indexing time virtually zero" claim (100k vectors in ~1-2 ms at d=1536-3072 on A100). No FLOP analysis of the rotation in the paper; no O(d log d) alternative discussed.
- **Query/read time**: query is **rotated but never quantized** - `Prod*y` and `S*y` once per query per instance (O(d^2) each; per channel-set if split), then per stored vector only LUT-dot + sign-weighted-sum (O(d), tiny constants). Back-rotation `Prod^T` only when a full vector is needed (V-side output, once per output vector - *inference*).
- KV at head_dim 128 (32/96 split): store ~ 32^2+96^2 ~ 10.2k MACs per key per head (x2 for Q_prod) - small next to the layer matmuls; query cost identical, once per decode step per head. (*Arithmetic inference.*)

**Cross-check identity for unit tests**: `D_prod(b) = (pi/2d)*D_mse(b-1)`: 1.57=pi/2*1, 0.56~pi/2*0.36, 0.18~pi/2*0.117, 0.047~pi/2*0.03 - measure empirical MSE at b-1 and empirical inner-product variance at b; their ratio must be pi/(2d).
