...
| 5015-5231 | session construction + `eval_` | b, d | 144-line `make_run_state`, paged twin. | `make_run_state`, `create_session_` x2, `eval_` |
...
| 5924-6286 | codec-generic decode attention + flash witnesses | b, d | 4x4 KxV codec dispatch matrix longhand; recompute witnesses. | `attn_head_decode(_d)`, `kv_slice_*`, `flash_decode_check_g/d` |
...
| 10332-10923 | prefill attention head kernels | b, d | Six head kernels (flash/blocked/classic x f32/`_d`). | `attn_head_*(_d)`, `prefill_attention` |
...
**Duplicated blocks:**
6. **4x hand-rolled rope cos/sin loops** L2930-2938, L2972-2980, L3013-3023, L3078-3086 - byte-identical; all bypass `build_rope_table` (5464) and `build_decode_rope_row` (847).
...
**Stale line refs from sibling docs into this file:** `kv_cache_compression.md:30` (:1429 -> actually 4853), `:150` (:2838 -> actually 5898); `qwen2_audio_plan.md:56` (:4184 -> actually 12652); `utils/dasllama-server/CONTROL_PAGE_PLAN.md:32` (:7155 wrong); `API_REWORK.md:1021` calls the file "~1940 lines" (7x drift).
...
> **SUPERSEDED - the vulkan-tier + class-kernel arcs dissolved this monolith into the role
> family** (ARCHITECTURE sec.1.5): `dasllama_math_vulkan.das` 316 (entry/facade) *
> `dasllama_vulkan_classes.das` 3,804 (**the kernel home - 41 class kernels**, every shader
> below re-landed as a `[spirv_kernel]` class; the module-global kernels file is DELETED) *
> `dasllama_vulkan_common.das` 2,513 * `dasllama_vulkan_decode.das` 1,244 *
> `dasllama_vulkan_prefill.das` 1,570 * `dasllama_vulkan_dispatch.das` 544 (the `[vk_dispatch]`
> lens) * `dasllama_vulkan_seams.das` 168 * plus the device-agnostic
> `dasllama_gpu_resident.das` 1,387 / `dasllama_gpu_tier.das` 1,202. The shader/host line map
> below describes the pre-arc file and stays as the census record of what was dissolved.
Shaders: 65-113 `q8_moe_groupn` * 115-228 `q8_moe_batch` (sdot4 32x32) * 230-873 coopmat variants (`_cmf16`/`_cmi8`/`_mm`/`_mm_a`/`_mm_m`) * 875-975 `kq_moe_batch_q40_cmf16` * 977-1102 cm2 twins * 1104-1117 `q8_moe_xf16` * 1119-1233 FFN mid-chain (`act_mul`, `q8_moe_actrq`, `q8k_moe_actrq`, `q8_moe_actf16`, `rd_f16cvt`) * 1235-1259 `moe_combine` * 1261-1834 deltanet * 1836-2064 attention (`at_prep_*`, `at_attn`) * 2066-2914 resident residual-stream (`ar_add_rms(_b/_rq)`, `qk_rms`, `rope_kv_store(_b)`, `qkn_rope_kv`, `da_attn(_b/_b_h128)`, `q8k_requant`) * 2916-3150 kq decode GEMV family * 3152-3659 kq batch tiles (k4/q40/k5/k6).
...
**`dasllama_math_gen.das` - 2,207, "arm64-gen"/"x64-gen" backends.** Required by 11 directly (apps get it via `dasllama_common` behind `?llvm`). 1-51 the eight-function-stamp doctrine * 52-97 `[tune_scope]` + layout companions * 99-521 gemv/tile `[tune]` stubs per family (each with `[tune_perm]` grids) * **526-1045 the 7 `repack_*_grp` functions** (q8q8/mx4/q51/k4/k5/k6/q40) + `_gen`/`_bake` twins + `k45_nib`/`k5_hbit`/`k6_nib` extractors (818-838) * 1050-1184 `unpack_kq_panel_grp` + `kq_grp_row_dot_b` * 1185-1892 cell walkers (generic/amx/neon-laneq x q8/mx4/kq) * 1893-2142 `_s16` twins * 2143-2207 `[init]`.
...
|---|---|---|---|
| **(a1) load-time weight repack skeleton** | `math_gen` 526-576, 593-635, 660-707, 843-891, 893-945, 947-1000, 1002-1045 | **copy-paste x7** | One skeleton: stride calc -> resize 2 scratch arrays -> byte-by-byte copy of whole tensor -> 3-deep gather loop; only the innermost gather (~10-25 ln) differs. ~340 lines where a parameterized walker + 7 gather lambdas would be ~150. Plus 14 near-identical `_gen`/`_bake` 2-line twins. |
| **(a2) stamped-vs-bake selector pairs** | `math_gen` 581-592, 639-646, 711-718, 1158-1184; `math.das` 2762-2810 | copy-paste x4 both sides | Reason real (lane contexts can't read math's globals - div0 comment 527-531) but the shape is one generic. |
...
| **(b1) Q8_0 activation quantize (CPU)** | `quant.das` 79-105, 122-156, 172-204 | copy-paste x3 | Identical two-lane float4 amax reduction verbatim (82-96 == 125-137 == 175-187); `_bs` adds per-16 sums, `_k` widens to 256. Comment admits it (178). |
| **(b2) Q8_0 requant (vulkan intra-file)** | 1134-1164 vs 1805-1834 vs inlined 2196-2224 | ~75-86% identical x3 - **RESOLVED (class-kernel arc)**: shared free fns `q8_amax3`/`q8_pack4` + the `ActRqMembers`/`rq_cls` family | Delta = `act_mul` vs plain read. |
| **(b3) Q8_K requant (vulkan intra-file)** | 1169-1205 vs 2880-2914 | ~86% identical - **RESOLVED (class-kernel arc)**: `q8k_amax5`/`q8k_pack_butterfly` free fns, 4-line store shells remain | Comment says "mirrors ... minus the act". |
...
| **(c) flash attention prefill (vulkan intra-file)** | `at_attn` 1931-2064 vs `da_attn_b` 2500-2637 | **~76% identical** - **RESOLVED (class-kernel arc)**: `AtAttnBase` carries the shared flash core, `AtAttn`/`DaAttnB`/`DaAttnBH128` are leaves | Entire flash core byte-identical; deltas = meta unpack + naming + guards. |
| **(d) kq batch tiles (vulkan intra-file)** | 3160-3277 (k4), 3281-3393 (q40), 3398-3520 (k5), 3526-3658 (k6) | **~91% identical x4** - **RESOLVED (class-kernel arc)**: `KqBatchBase` + thin `stage_w`/`stage_ws`/`blk_fma` overrides per format | Verbatim 40-line preamble + activation stage + 4xsdot4 block + 20-line store. |
| **(e) kq decode GEMVs (vulkan intra-file)** | 2985-3023, 3028-3064, 3068-3106, 3112-3150 (+ `q8_moe_groupn` 75-113, `q8n_moe_groupn` 1061-1102) | **~87-95% identical - 6 copies** - **RESOLVED (class-kernel arc)**: `KqGemvBase` + per-format `blk_contrib` overrides | Subgroup-per-row region-list GEMV shell. |
| **(f) region-list meta unpack (vulkan)** | 12 copies: 131-142, 240-252, 343-355, 411-425, 619-631, 764-777, 881-892, 1001-1011, 3163-3172, 3284-3293, 3401-3410, 3529-3538 | copy-paste x12 - **RESOLVED (class-kernel arc)**: `region_rec` base method on the batch families | 10-14 lines each; tile edge E varies. |
| **(g) subgroup partial-sum reduce (vulkan)** | 5 copies: 2092-2107, 2136-2151, 2180-2195, 2247-2261, 2338-2352 | copy-paste x5 - **RESOLVED (class-kernel arc)**: `RmsWgBase` workgroup-rms methods shared by the ar_*/qk_rms/qkn_rope family | Verbatim in the ar_* / qk_rms / qkn_rope_kv family. |
...
Biggest mirrored blocks: resident driver 5623-7040 <-> whole `metal_llama.das`; kq batch tiles 3152-3659 <-> `metal_prefill` MulMm families; attention 1836-2064+2384-2828 <-> metal SqAttn (metal carries 4 KV codecs x 5 tiers, vulkan f32 only); deltanet 1261-1834 <-> metal 7-kernel decomposition (vulkan fuses decode step into one kernel); coopmat tiles <-> simdgroup tiles; rope+KV store <-> metal x5 codecs.
...
5. ~~Vulkan file header (23-49) undersells file by half~~ - OBSOLETE: the monolith is dissolved (see the SUPERSEDED note above); each role file carries its own charter.
...
12. All 7 `repack_*_grp` copy the whole tensor with a **byte-at-a-time loop** (dominant load-time repack cost, duplicated 7x); vulkan has `par_memcpy` (7524-7542) for exactly this shape.
...
14. `asr_requant_rows_q8` (`audio.das:807`) is a near-verbatim copy of common's private `requant_rows_q8` (6749) - copied because the original is private; the copy lost the par-threshold gate + profiling. Sibling wrappers `tw_repack`/`pk_repack`/`wm_repack` (audio:885/parakeet:270/whisper:203) + their quantize drivers are the same 8-line recipe copy-adapted x3 over shared `repack_q8q8_weight`.

### Refinements (final agent pass)

**Hypothesis verdicts.** (a) repack "mostly the same across platforms" - *refined, not confirmed*: near-identical WITHIN the CPU rail (7 copy-paste `_grp` fns + 8 wrapper twins + 3 byte-identical extractors ~ clean ~300-line reduction, zero cross-platform risk); NOT the same across platforms (GPU = gather-into-device-planes, registered as `*_repack_noop`). (b) convert - *strongly confirmed*: **11 implementations of "amax -> d=amax/127 -> round -> pack"** (quant.das x3 pasted incl. the "LLVM won't auto-vectorize fmax" comment x3; math.das `quantize_q8kv_row`/`quantize_tq4kv_row`; vulkan x5 with comments admitting equivalence). Most duplicated formula in the slice.

**Additional clusters:**
- **A5. kq scale-plane decode** - the `[f16 d][f16 dmin][8 sc][8 mn]` 20B decode written 4+ times in 3 byte orders: `math_gen` (inside repack_k4/k5), `math_default` (`k4_sc_mn` 379-392 + `dequant_kq_row_grp`), `layout.das` (`compact_kq_scale_strip` 186 + gather_kq), vulkan in-shader x4. `KQ_DEV_SSB` names the stride; nothing names the *decode*.
...
- Repack scratch: 7x full-tensor alloc+free per call, fanned across jobque lanes by `repack_regions` (layout:215) -> peak transient = lanes x 2 x tensor_bytes; no `@scratch`, no reuse.
...
24-2552 **single-stream decode set, 100% lensed**: single-query attention x 4 KV codecs (F16 29, F32 201, Q8 369, Tq4 553) + split-K part twins (730-1312) + comb (1507); fused QKV+rope GEMV (1554/1676); W13+swiglu (1795); CopyRow/Argmax/EmbedQ8/EmbedK6 (1846-1945); AddRms/PreAddRms (1989/2042); rope-store F16/HF16/F32/Q8/Tq4 (2119-2417); FwhtUnsign (2525) * 2553-5847 **batched-step P4 set, 0% lensed**: fixed-B GEMV/mv/w13sw B2+B4 (2560-2955); batch GEMM + split-K + Metal-4 tensor twins (2956-3328); batch attention B/PartB/CombB/D/CombD x 4 codecs (3329-5184+); batched rope-store (5185-5688); AddRmsB (5799) * 5849-6019 PSO lifecycle (`metal_decode_init` 5873, ~135 compile_pso calls incl. 3 `metal_tensor_crowned` forks) * 6025-6223 single-stream encoders * 6224-6417 MoE routing encoders (Wave C) * 6418-6979 batched-step encoders (561 lines hand-written `kn_*`) * 6982-7161 `metal_kernels_release` (76 `release_pso` + 26 open-coded blocks) * 7164-7281 decode tensor races.

**`dasllama_metal_prefill.das` - 8,240, `shared public`.** 78 kernel classes, 17 lensed. 328 `@ssbo` / 250 `@role`.
37-368 RmsNorm + mul_mm family (Q8/Bf16 + `T` tensor twins) + PLE gather/finish * 369-876 K-quant mul_mm twins (K4/K4T/K5/K5T/K6T/K6) * 877-1924 MoE routing Wave C (Router/RouterB/Select/Gemv x q8/k4/k5/k6/mx4/q51, Combine, SwigluOai, G4RouterNorm, MoeWScale) * 1925-2703 MoE prefill CSR + gathered mul_mm (Count/Bucket/Reduce/MulMm x formats) * 2704-3025 K-quant site GEMVs * 3026-3485 kq small-batch mv twins B=2..4 * 3486-3738 kq B=5..8 single-pass twins * 3739-4540 Rope/AddBiasRows/QkNorm/AttnQK/Softmax/AV(+MmT/Mm)/Swiglu/Geglu/Add/PfCopy/PfCat2/SuppressRow/SoftcapRow * 4541-4888 DeltaNet Wave D (9 classes, 6 lensed) * 4889-5291 module state (~90 `g_pf_*` + `PF_NEEDS_OK`) + `metal_prefill_shutdown` * 5293-5550 `pf_compile_pso`, `metal_prefill_init`, pf-uniforms, `prefill_decline` (5488) * 5553-6260 prefill + MoE + deltanet encoders * 6261-6295 weights-epoch guard + PLE gate * **6296-7526 `metal_prefill_forward` - 1,231 lines in one function** (~150-line hand-unwound `pool_release` tail at 7370-7514) * 7528-7553 `[init]` registers "metal" prefill override + PLE gate * 7555-8240 prefill tensor races (7 families, `metal_tensor_race` 8214).
...
| Hazard granularity | exact byte ranges (`MemRange`, `@span`) | region BITS (3 namespaces x 6 bindings) | metal finer; vulkan coarser/cheaper |
...
| Manual-dispatch census (compile-time) | `[lint_macro]` census | `[vk_dispatch]` derives access per class + seeds the kernel census (generated `[init]` -> `vkd_census_seed`); `coverage-vk` serving arm gates it | mirrored since the class-kernel arc |
...
- `enc_rms` x4 geometries over one MSL, two PSOs (1024/1, 1024/nrows, 256/nrows, 256/1) - undocumented why.
...
|---|---|---|---|
| `dasllama_transformer.das` | 26 | pure umbrella: `require dasllama_common public` + 16 side-effect requires (layout hooks, 13 arch files, metal_shapes unconditional, 2x `?das_metal` overrides - cycle-break documented L25) | 32 |
...
| `dasllama_audio.das` | 1,565 | **the hub (20+ dependents)**: mel flavors x2, FFT plan, `AudioTower`, shared row kernels, tower Q8 rail, loader, `EncoderState`, `enc_attention`, `encoder_blocks` (1422-1491 - the ONE shared transformer loop), 3 projector tails |
...
| `dasllama_vad.das` | 478 | self-contained Silero-VAD v6 (learned-basis STFT -> conv1d x4 -> LSTM -> sigmoid). Only ASR file with no `.dlim` facade; not required by asr.das - sibling rail via umbrella |
...
- **gemma4: ~600 lines in common** (per-layer geometry 6837-6941, PLE 9205-9318, MoE decode 9319-9469, prefill router 12330+, ~10 loader sniffs) + layout PLE/dense-shexp + metal touches x12 + shapes x5.
- gpt-oss: sinks sniff L4127, biases, swiglu_oai, YaRN + metal_prefill touches x8.
- glm4moe: dense-lead, `exp_probs_b` sniff, NextN block + layout x5.
...
- **C7 log-mel/STFT x6 independent copies** - parakeet<->canary = COPY-PASTE (preemph/pad/normalize line-for-line; only DFT engine differs); audio's two divergent by design (different oracles); + 2 filterbank generators (Slaney vs HTK).
- C8 conv subsampling x3 (parakeet<->canary same topology, divergent impl - f4/threaded vs naive scalar; canary adds length masking parakeet lacks); output-length arithmetic `(l-1)/2+1` x4.
- **C9 conformer block loop x2 = COPY-PASTE with systematic bias delta** (canary adds add_bias_rows everywhere, drops Q8).
- C10 attention x5: whisper self<->cross = COPY-PASTE (54 ln each); parakeet<->canary rel-pos = COPY-PASTE.
- C11 sinusoidal pos table x2 verbatim (9 lines).
- **C12 greedy argmax decode x4 copy-pasted loops all inside asr.das** + logsumexp block x2 + prompt-assembly scaffold as a 5th near-identical block. No CTC, no beam anywhere.
- **C13 bin container readers - densest cluster:** `bytes_str` byte-for-byte x4 (whisper/parakeet/vad/canary); `*_read` x3; record structs x4.
- **C14 prepared-image loader facades x6** (~30 lines each: .dlim branch -> box profile -> load_image -> panic; env -> eager -> save -> gc), same log wording modulo one noun.
- C15 Q8 GEMM rails x3 acknowledged in-source ("tw_repack's twin", "parakeet's v1 recipe").
- C16 finalize apply-walkers x6. C17 image_post_load + meta-tripwire pair x6. C18 `*_ensure` grow helpers x5 with 4 different semantics. C19 conv2d im2col core COPY-PASTE x2 (qwen3a/gemma4a), divergent tails.

**Plumbing:** C20 `q4k_scale_min` x3 (gguf array + gguf ptr + **math_default `k4_sc_mn` byte-identical**); superblock dequants array+ptr twins (~110 ln); plane dequants re-derive same math a 4th time; Q5_1 split logic x2 * **C21 `save_image` vs `save_model_image_streaming` ~130 near-identical lines; the streaming twin OMITS the nested-weight-carrier arm - silent plane-drop if a Model ever gains a nested image_map carrier** * C22 `moe_gpu_gather_stack` vs `_q8n` ~50 verbatim lines.

### Repack/convert sightings (this slice)

**Repack:** the whole CPU repack registry (`layout.das:199-536`: `push_repack` chunking, `repack_regions` 14-pointer hoist + 4-way fmt dispatch, collectors per format, `stream_repack_one`) * kq scale-strip compactions (157-197) * GPU gathers 538-835 (un-interleave grp<mr> -> row-major; **k-nibble re-pairing + k5 qh bit-transpose + 6-bit scale decode to KQ_DEV_SSB**) * conv1d torch->im2col permute COPY-PASTE x2 (audio:959-971 == whisper:312-324) * parakeet tap-major conv repack * BN folds x2 (different formulas! see A40) * whisper cross-K 4x4 blocked transpose + cross-V fan-out * twiddle transpose * feature permutes x2.

**Convert:** the gguf codec library (above) incl. the fp32->Q5_K encoder * BF16 paths * E8M0 denormal * q8-planar->34B-blob (layout:124-151, f16 clamp +/-65504 - clamp also at layout:578/643) * whole-blob Q8 quantize x2 (audio/whisper) * parakeet direct file->Q8 * `asr_requant_rows_q8` * GGML_GELU_FP16 round-trip + 65536-entry f16 gelu LUT (audio) * canary int64-from-2xu32. **Not present:** fp16 storage in ASR pile, int4 there, canary quantization (fp32 by design).
...
- **A37 parakeet key mask provably dead** (`n_len_org == n_len` at L780 => mask never fires) at n_headxn_layerxtt^2 compares - acknowledged in-comment and left in.
...
Rest: 2 tuners' wrappers (`dasllama_tuner.das` 81, `tuner_cli.das` 28), parity oracles (`parity.das/sh`, `pk_gate.sh`, `gptoss_parity_probe`, silero/whisper/canary/qwen2audio py oracles), converters (**`convert_canary_to_ggml.py` 299**, **`convert_silero_vad.py` 82**), one-off probes (~30 files: tq4_* 1026 lines research, wh_*, trace_*.py, smmla, dispatch, flash_stage2, kv_range, mem, k4_kernel, accel_*, bnns_hgemm.c, gemm_1core, q3omni...), emission checks (`gen_*_emission_*`), ref-engine patches x4, `gen_env_doc.das` 27 (**writes ENVIRONMENT.md**), C++/LLVM oracles under `oracle/`.
...
| control.html:391-392 | Metal+Vulkan "~15,000" | **31,522 (2.1x off)** |
...
| CONTROL_PAGE_PLAN.md:88-98 | "S8 DEFERRED ... resume via revert-of-revert" | **SHIPPED** (bench button live: openai_server:348-529, control.html sec.10) |
...
| server README:41 | "--mtp ~2x decode" | no date/box/record pointer |
...
| results_metal_*.md:1 x5 | "living doc - CURRENT numbers only" | **no generator existed** - archived to history/dasLLAMA/performance/ |
...
| API_REWORK.md:1021 | "dasllama_common.das (~1940 lines)" | 13,684 - 7x drift |
...
- **"Living docs" with no generator:** results_metal_*.md x5, baseline_metal_*.tsv - archived to history/dasLLAMA/performance/ 2026-08-02.
...
| ENVIRONMENT.md | env-registry #3580 | **STAYS - GENERATED, never hand-edit** | - | RST `.. include::` (hard build dep), CLAUDE.md, skills, code xmany |
...
| API_REWORK.md | API rework + perf ledger | **SPLIT**: plan (1-338, 1005-1042) -> history; **ledger (339-1005, ~47 entries, standing "note HERE" rule) -> new root `PERF_LEDGER.md`** | Tier scope table (corrected) + arch-registry/kernel-seam prose -> ARCHITECTURE.md | PQ: tutorial RST :80 + tutorial das :86; bare: README x3, audio plans, CLAUDE.md:373 |
...
| x64_arch.md | x64 port (self-marked HISTORICAL) | **ARCHIVE** | ground rules (-jit is the ONLY tier - never interp, never AOT; the doc's "AOT correct-but-untuned" framing is RETIRED, do not re-import); 3-layer x64 safety model; u8xs8 sign-trick identity -> ARCHITECTURE.md kernel chapter (best existing draft of one) | bare sibling links only |
...
| turboquant_research.md | tq4 codec (shipped) | **ARCHIVE** - textbook case | skip-QJL verdict + "benchmark vs q4_0+Hadamard, not f16" -> one para. in ARCHITECTURE KV section or `//!` on `quantize_tq4kv_row` | zero |
