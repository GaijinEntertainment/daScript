...
| **2 - block swaps** | **MoE** (FFN -> router + experts), QK-norm, partial/pruned RoPE, per-layer attention patterns (SWA ratios, per-layer theta, NoPE layers), shared KV cache (Gemma-4: last-N layers reuse earlier KV), per-layer embeddings (PLE), attention sinks, parallel attn+FFN | **This rework.** Not flags - they change block dataflow. See the model-support wave plan below. |
...
```
Blocks are **function pointers, one per (attention|ffn) x (decode|prefill)** - decode is the fused
...
1. **Naming + `eval`** DONE - `Transformer -> Model`, `RunState -> Session`, `n_past` into
   `Session`, `eval()` / `create_session()`. Pure refactor, token-identical.
2. **Tokenizer facade** DONE - `SpmTokenizer` / `BpeTokenizer` (renamed) behind a unified
...
3. **Sampling** DONE - `SamplingParams` (defaults = greedy: temp 0, penalty 1.0 -> the greedy branch
...
4. **Arch registry + block seam** DONE - `configure_arch` switch -> `[init]` self-registering
...
5. **Minimal unified chat app** DONE - new `dasllama_chat.das` (layer 2): `Role`/`Message`/`ChatSession`
...
**Below the line (demonstrations):** DONE an OpenAI-compatible server - `utils/dasllama-server/` (the
...
- **Docs:** das2rst registers ONLY the facade module (new stdlib section `sec_ai.rst`,
  ...
  links).
- **[key] `//!` placement:** the doc extractor (`daslib/rst_comment`) attaches a docstring only when
...
Performance ledger extracted to `PERF_LEDGER.md` (2026-07-29; lives at modules/dasLLAMA/PERF_LEDGER.md).
