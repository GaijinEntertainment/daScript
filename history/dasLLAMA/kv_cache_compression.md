...
|---|---|---|
| **dasLLAMA today (FP32)** | 4.0 | 1x |
| llama.cpp default (F16) | 2.0 | 1/2 |
| llama.cpp `q8_0` | ~1.06 | ~1/4 |
| llama.cpp `q4_0` / `IQ4_NL` | ~0.56 | ~1/7 |
| TurboQuant (3.5-bit) | ~0.44 | ~1/9 |

dasLLAMA's KV is **2x larger than llama.cpp's uncompressed default** and **~9x larger than TurboQuant**.
...
2. **Share across heads** (architectural, training-time) - MQA/GQA (Google). Llama-2/3, Mistral, Qwen
   already use GQA -> dasLLAMA **inherits** a 4-8x KV shrink for free from the model.
3. **Low-rank / latent** (architectural) - **MLA** (DeepSeek, ~10-15x shrink). Needs training: no
...
- **F16 covers it decisively:** max normal ~ **65504** (a 1000-magnitude outlier is ~1.5% of ceiling ->
  overflow essentially impossible for post-norm KV). Relative precision ~ **2^-^1^1 ~ 0.049%** (10
...
3. **Portable scalar bit-twiddle** - `ggml/src/ggml-impl.h:384-431`, from Marat Dukhan's FP16
   ...
   (round-to-nearest-even via an FPU-rounding trick: `(|f|*2^1^1^2)*2^-^1^1^0`), handles subnormals (magic
...
- M1+ has native f16 **arithmetic** (`__ARM_FEATURE_FP16_VECTOR_ARITHMETIC`, `ggml-cpu.c:3768`): ggml
  does 8-lane `vfmaq_f16` (`float16x8_t`, unrolled x4 -> step 32) accumulating **in f16**
...
- `dot(qp+qoff, kcp+koff, head_size)` - Q*K^T scores - decode `dasllama_common.das:2838`
...
- register-blocked attention kernel - `gemm_f32_uk_4x16_template` (`dasllama_math.das:453-489`,
  ...
  called from `attn_head_flash` (`common.das:3774` Q*K^T, `:3820` att*V). **Flash-mode nuance:**
...
- **Therefore the KV-dtype axis is ADDITIVE in the shipped binary**, not `dtype x tune`. Native f16
...
- **The inversion:** convert the thing that's `O(head_dim)` per step (query row), not the thing that's
  `O(context x head_dim)` (the cache). Keeps the bandwidth win.
...
volume, **not** runtime. My earlier "dtype x tune blows up" concern was wrong (conflated tuning-stage
...
- Single-shot over the whole prompt: `eval_` (`:1573-1576`) calls `forward_batch(t, s, tokens, npos,
  ...
  `npos x max_kv_dim` (`:4966-4967`) -> **the entire prompt's K/V is already in f32 scratch**.
...
- **Three prefill modes** (`g_attn_prefill_mode`): flash / blocked / classic. Flash packs the cache
  ...
  dequant seam) is in the pack step, not the GEMM (sec.5).
...
- **Continuation (`start_pos>0`):** prior context lives only in the (compressed) cache -> decompress the
  current layer's prior range, one layer, **amortized against the O(npos^2) attention GEMM**. Fine.
...
- Each step reads the **whole current-layer cache** (`0..pos`). Decompress it externally to f32 every
  step -> traffic = `read compressed + write f32 + read f32` ~ **2x the f32 baseline -> strictly WORSE
...
5. **K vs V asymmetry:** K consumed via `dot` (Q*K^T), V via `axpy` (weighted sum) / per-row dequant.
...
- Two stages (Zandieh/Daliri/Hadian/Mirrokni; one DeepMind coauthor - NOT "a DeepMind paper"):
  ...
  **(2) QJL** (the authors' own prior paper, arXiv 2406.03482): 1-bit sign of a JL projection of the
  ...
  (within ~2.7x of the rate-distortion bound). NOTE: "PolarQuant" is a *separate* paper by the same
...
- **The weight matmul is memory-bound on the weight stream**, so bytes read decide the winner, not
  ...
  the weight to f32 (4x) + a materialization pass, to feed AMX/SME whose only edge is f32 throughput
...
- BLAS/AMX would only win a **compute-bound, already-f32** GEMM - which doesn't exist in the hot path.
  The lone f32 GEMM (prefill attention) is small per-head and going F16 anyway (sec.7), so that sliver
  closes too.
