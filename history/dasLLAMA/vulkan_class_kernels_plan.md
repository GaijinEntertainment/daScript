...
INVENTORY measured (meta unpack x12, subgroup reduce x5, kq tiles ~91% identical x4, GEMV shells
x6, `ensure_*` x18) follows directly.
...
7. **Multiple kernel methods per class** over one binding set + one params struct - the family
   pattern (kq GEMV x4, `ar_*` fused family). One set layout serves several pipelines; the decode
...
- Family by family, each becoming one class with N entry methods + one params struct: kq decode
  GEMVs (4->1), batch GEMM tiles (4 near-identical x~120 lines), attention (at/da pair), rope+KV
  store, quant/cvt/requant, deltanet set, MoE kernels, embed/argmax. The x12 region-list unpack and
  x5 subgroup reduce become shared methods/free functions.
...
- Decode (`dasllama_vulkan_decode.das` ladders at :147-266 and friends), prefill (`pf_bind` x19,
