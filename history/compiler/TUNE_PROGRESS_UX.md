...
**Status.** Shipped. Everything below is built and tested; the operative reference is
`skills/tune.md` (user-facing) and `skills/llvm_tune.md` (internals). This file is the record of
...
| `gen_tune_probe.das` | `--tune-fast` | has **no** parser today - add one (sec.8) |
...
round count. `tune_kernels` fast cuts its total budget 80 -> 20 (4x) and its full-grid screening
20 -> 4 (5x). The proportionate move is **`ROUNDS` 6 -> 3 under fast** (6 -> 2 if we want to match
the 3x more closely). Don't port the screening machinery: it pays when the grid is large and the
...
of sec.7 rather than a separate change.
...
| progress | - | **nothing - `run_scope_tuner` buffers the whole tune (sec.1)** |
...
**sec.1 is the direct cause of the thing the stage machinery was written to paper over.** watchdog.py
...
never what it is doing." That window *is* the buffered relay. Fix sec.1 and the window has content.
...
- A `\r`-drawn bar from the daslang side is garbage here - every redraw becomes a JSON record.
  The sec.3 rule already prevents it (the server's stdout is a pipe, so it forwards and never draws),
