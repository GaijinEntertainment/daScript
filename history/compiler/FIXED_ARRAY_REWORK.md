...
Exit: full CI green with aot_cpp.das / llvm_jit.das / macro daslib UNMODIFIED (riding compat).
  ...
  **NATURAL semantics. A generic alias binds the WHOLE matched type**: `auto(TT)` <-
  ...
  dasGlsl produce_zero, dasLLVM llvm_jitx3 dim-clear->element-walk (+ the `new Handle[N]`
  ...
  COUNTS across the chain (master parity: 32767^2 passes infer; lint reports byte-size
...
dasImgui cb_dasIMGUI.h + dasIMGUI.main.cpp, dasImguiNodeEditor x2, dasImguiImplot
x2) -> `!arg->type->isFixedArray()`. That helper doesn't exist on master, so the
...
runs every modules/<x>/.das_module descriptor (dyn_modules.cpp), which dlopens
