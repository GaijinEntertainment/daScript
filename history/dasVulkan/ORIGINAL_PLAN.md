...
The boost is a **parallel idiomatic API**: the user writes in boost types (`Device`, `Image`, `SubmitInfo` with `array<Semaphore>`) and never touches raw `Vk*` except via escape hatches. It is generated uniformly from the same `VkRegistry` the raw layer already builds, so "generate everything" is the same emission code applied to all types - not Nx the work. The long tail of irregular types is handled with logged skips + a small override table, exactly as the raw layer skips 130 structs / 13 commands.
