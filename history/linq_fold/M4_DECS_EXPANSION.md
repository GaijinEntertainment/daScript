...
|---|---:|---:|---:|---:|---:|
| sum_aggregate | 29 | 30 | **2** | 14.5x | 15.0x |
| select_where_count | 32 | 57 | **5** | 6.4x | 11.4x |
| sum_where | 32 | 43 | **4** | 8.0x | 10.8x |
| chained_where | 36 | 45 | **6** | 6.0x | 7.5x |
| aggregate_match | 34 | 49 | **5** | 6.8x | 9.8x |
| all_match | 27 | 21 | **3** | 9.0x | 7.0x |
| take_while_match | 7 | 22 | **2** | 3.5x | 11.0x |
| count_aggregate | 29 | 29 | **4** | 7.2x | 7.2x |
| select_where_sum | 36 | 60 | **7** | 5.1x | 8.6x |
| to_array_filter | 74 | 42 | **11** | 6.7x | 3.8x |

### Order/sort family (m3f dominates m3)

| benchmark | m1 | m3 | m3f | m3/m3f |
|---|---:|---:|---:|---:|
| sort_take | 38 | 763 | **27** | 28.3x |
| order_take_desc | 38 | 746 | **27** | 27.6x |
| select_where_order_take | 36 | 379 | **24** | 15.8x |
| sort_first | 37 | 742 | **756** | (regresses; sort dominates) |

### Group-by family (2-5x wins)

| benchmark | m1 | m3 | m3f | m1/m3f | m3/m3f |
|---|---:|---:|---:|---:|---:|
| groupby_sum | 173 | 102 | **37** | 4.7x | 2.8x |
| groupby_count | 143 | 68 | **36** | 4.0x | 1.9x |
| groupby_average | 171 | 106 | **52** | 3.3x | 2.0x |
| groupby_having_count | 144 | 74 | **36** | 4.0x | 2.1x |
| groupby_having_hidden_sum | 175 | 103 | **40** | 4.4x | 2.6x |
| groupby_max | 174 | 103 | **44** | 4.0x | 2.3x |
| groupby_min | 173 | 106 | **43** | 4.0x | 2.5x |
| groupby_multi_reducer | 190 | 139 | **53** | 3.6x | 2.6x |
| groupby_select_sum | - | 110 | **59** | - | 1.9x |
| groupby_where_count | 76 | 64 | **23** | 3.3x | 2.8x |
| groupby_where_sum | 86 | 79 | **23** | 3.7x | 3.4x |
| groupby_first | - | 68 | **35** | - | 1.9x |

### Anomalies / weak spots

| benchmark | m1 | m3 | m3f | note |
|---|---:|---:|---:|---|
| indexed_lookup | 1,431 | 2,029,891 | 200,399 | SQL B-tree wins 1417x over linear scan; splice helps 10x over m3 but algorithmically can't match SQL. Decs equivalent uses **eid lookup** (archetype hash lookup, fast path). |
...
| from_decs_sum | 4 (query) | 202 | **8** (within 2x of hand) |
...
**Net Cat A:** m4 beats SQL on most shapes (8 of 17 with concrete m1+m4 numbers; another 4 at 0 ns/op). m4 vs m3f shows a ~3-5x decs overhead from the 6-component multi-iter for-loop (every component's get_ro participates even when chain only reads one field). This is the splice's structural cost on a multi-field decs schema.
...
If implemented, m4 numbers on Cat A would close most of the 3-5x gap vs m3f, making decs effectively as fast as array for projection-heavy chains.
...
|---|---:|---:|---:|---:|---:|---:|---:|
| sort_first | 37 | 713 | 722 | **42** | 802 | **121** | 17x |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| take_while_match | `._take_while(_.id < 50K).count()` | 7 | 23 | 2 | 55 | **8** | 6x | 6.9x |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| groupby_count             | 142 | 67  | 37  | 115 | **50**  | 2.3x | 2.8x faster |
| groupby_sum               | 172 | 101 | 36  | 115 | **50**  | 2.3x | 3.4x faster |
| groupby_average           | 175 | 101 | 52  | 128 | **62**  | 2.1x | 2.8x faster |
| groupby_max               | 173 | 103 | 49  | 120 | **57**  | 2.1x | 3.0x faster |
| groupby_min               | 177 | 106 | 42  | 122 | **56**  | 2.2x | 3.2x faster |
| groupby_first             | -   | 70  | 35  | 112 | **46**  | 2.4x | - |
| groupby_having_count      | 142 | 73  | 36  | 114 | **49**  | 2.3x | 2.9x faster |
| groupby_having_hidden_sum | 175 | 103 | 40  | 122 | **57**  | 2.1x | 3.1x faster |
| groupby_multi_reducer     | 191 | 139 | 52  | 130 | **63**  | 2.1x | 3.0x faster |
| groupby_select_sum        | -   | 110 | 60  | 137 | **71**  | 1.9x | - |
| groupby_where_count       | 75  | 65  | 23  | 101 | **36**  | 2.8x | 2.1x faster |
| groupby_where_sum         | 94  | 80  | 23  | 105 | **36**  | 2.9x | 2.6x faster |

All 12 groupby_* rows improve 1.9-2.9x (avg ~2.3x). m4 now consistently lands 10-20 ns above m3f - the remaining gap is the Wave 4 multi-component `get_ro` overhead (every bridge component participates in the inner for-loop iteration even when the chain only reads one). m4 beats SQL on 10/10 measurable rows by 2.1-3.4x.
...
|---|---|---:|---:|---:|---:|---:|---:|
| sort_take                | `_order_by + take` | 38 | 713 | 28 | 119 | **57** | 2.1x |
| sort_first               | `_order_by + first` | 37 | 711 | 41 | 121 | **71** | 1.7x |
| order_take_desc          | `_order_by_descending + take` | 37 | 694 | 28 | 117 | **58** | 2.0x |
| select_where_order_take  | `_where + _order_by + take` | 36 | 352 | 25 | 102 | **38** | 2.7x |
| bare_order_where         | `_where + _order_by` | 274 | 359 | 118 | 196 | **130** | 1.5x |
| reverse_take             | `.reverse().take(N)` | 0 | 22 | 0 | 114 | **48** | 2.4x |

All 6 rows improve 1.5-2.7x (avg ~2.1x). m4 lands within 2x of m3f on most rows - the remaining gap is the Wave 4 multi-component `get_ro` overhead (sort dominates the wall-clock so the gap doesn't fully close even when the splice fires). `bare_order_where` is the tightest squeeze (1.5x) because sort over 100K rows is the bottleneck; once the m4 buffer build matches m3f (~117 ns), the rest is pure sort time.
...
|---|---|---:|---:|---:|---:|---:|---:|
| distinct_count | `_select(_.brand).distinct().to_array()` | 41 | 44 | 15 | 97 | **28** | 3.5x |
...
|---|---:|---:|---:|---:|---:|
| sum_aggregate | 29 | 29 | 2 | 7 | 3.5x |
| sum_where | 32 | 43 | 4 | 7 | 1.8x |
| count_aggregate | 29 | 28 | 4 | 6 | 1.5x |
| long_count_aggregate | 29 | 29 | 4 | 6 | 1.5x |
| max_aggregate | 30 | 36 | 6 | 10 | 1.7x |
| min_aggregate | 30 | 38 | 5 | 10 | 2.0x |
| average_aggregate | 29 | 32 | 5 | 11 | 2.2x |
| select_where_count | 32 | 57 | 5 | 9 | 1.8x |
| select_where_sum | 37 | 73 | 8 | 10 | 1.3x |
| select_where | 189 | 28 | 11 | 22 | 2.0x |
| chained_where | 36 | 45 | 6 | 10 | 1.7x |
| all_match | 27 | 20 | 3 | 6 | 2.0x |
| contains_match | 0 | 29 | 2 | 4 | 2.0x |
| any_match | 0 | 0 | 0 | 0 | matches |
| take_while_match | 8 | 23 | 2 | 3 | 1.5x |
| skip_while_match | 3 | 20 | 5 | 7 | 1.4x |
| to_array_filter | 69 | 42 | 11 | 15 | 1.4x |

### Group-by - m4 close to m3f, m4 beats m1 sql 10/11

| benchmark | m1 sql | m3 | m3f | m4 | m4/m3f | m1/m4 |
|---|---:|---:|---:|---:|---:|---:|
| groupby_sum | 171 | 100 | 36 | 50 | 1.4x | **3.4x m4 win** |
| groupby_count | 143 | 66 | 36 | 50 | 1.4x | **2.9x m4 win** |
| groupby_average | 173 | 102 | 51 | 62 | 1.2x | **2.8x m4 win** |
| groupby_max | 172 | 103 | 43 | 56 | 1.3x | **3.1x m4 win** |
| groupby_min | 176 | 107 | 43 | 57 | 1.3x | **3.1x m4 win** |
| groupby_multi_reducer | 191 | 138 | 55 | 63 | 1.1x | **3.0x m4 win** |
| groupby_having_count | 141 | 73 | 36 | 50 | 1.4x | **2.8x m4 win** |
| groupby_having_hidden_sum | 177 | 103 | 39 | 57 | 1.5x | **3.1x m4 win** |
| groupby_where_count | 79 | 65 | 24 | 37 | 1.5x | **2.1x m4 win** |
| groupby_where_sum | 87 | 80 | 24 | 37 | 1.5x | **2.4x m4 win** |
| groupby_select_sum | - | 117 | 61 | 64 | 1.1x | - |
| groupby_first | - | 70 | 35 | 48 | 1.4x | - |

### Order / sort / reverse - m4 within 2x of m3f

| benchmark | m1 sql | m3 | m3f | m4 | m4/m3f |
|---|---:|---:|---:|---:|---:|
| sort_take | 38 | 726 | 22 | 51 | 2.3x |
| order_take_desc | 38 | 721 | 22 | 52 | 2.4x |
| select_where_order_take | 36 | 355 | 21 | 34 | 1.6x |
| sort_first | 37 | 721 | 41 | 71 | 1.7x |
| bare_order_where | 270 | 351 | 120 | 130 | 1.1x |
...
| benchmark | m1 sql | m3 | m3f | m4 | m4/m3f |
|---|---:|---:|---:|---:|---:|
| distinct_count | 40 | 44 | 15 | 20 | 1.3x |
...
|---|---:|---:|---:|---:|---|
| indexed_lookup | 1449 | 2,014,895 | 219,259 | **481** | **3x m4 win over SQL b-tree** (Wave 4b) |
...
#### Group-by JIT - m4 beats m1 sql by 12-19x

| benchmark | m1 sql | m3 | m3f | m4 | m1/m4 |
|---|---:|---:|---:|---:|---:|
| groupby_sum | 171 | 26 | 8 | 10 | **17.1x m4 win** |
| groupby_count | 141 | 16 | 8 | 10 | **14.1x m4 win** |
| groupby_average | 170 | 25 | 9 | 11 | **15.5x m4 win** |
| groupby_max | 172 | 25 | 8 | 10 | **17.2x m4 win** |
| groupby_min | 174 | 26 | 8 | 10 | **17.4x m4 win** |
| groupby_multi_reducer | 188 | 37 | 8 | 10 | **18.8x m4 win** |
| groupby_having_count | 141 | 22 | 8 | 10 | **14.1x m4 win** |
| groupby_having_hidden_sum | 174 | 30 | 8 | 10 | **17.4x m4 win** |
| groupby_where_count | 71 | 19 | 4 | 6 | **11.8x m4 win** |
| groupby_where_sum | 82 | 23 | 4 | 6 | **13.7x m4 win** |
...
| benchmark | m1 sql | m3 | m3f | m4 | note |
|---|---:|---:|---:|---:|---|
| indexed_lookup | 1250 | 498461 | 34974 | **102** | **12.3x m4 win over SQL b-tree** |
