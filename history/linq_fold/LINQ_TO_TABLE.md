...
- m7 `groupby_*` INTERP 144-201 -> 30-50 ns/op (count 163->31, ~5x); JIT 44-73 -> 8.4-11 (count
  43.5->8.4, another ~5x - the fused emit rides #3100's inline slot walk). `join_groupby_*` stays
...
- m7: `to_table` 32.3 vs `to_table_staged` 71.5 ns/elem INTERP (~2.2x over materialize-then-convert).
...
- m7 (2026-06-11 sweep): table-lead joins leave tier-2 - `join_count` 195.0 -> 65.6 ns/elem
  ...
  internal hash is ~1.7x on identical rows.
...
- **Multiple-`from` (cross / SelectMany) over tables**: the unfused `_cross_join` arm passes the
  bare source text so the arrayxarray overload resolves without an `each` unsafe trip; a table
