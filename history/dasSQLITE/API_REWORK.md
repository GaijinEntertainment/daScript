...
1. **PR #2471 - `daslib/option` + `daslib/result`.** DONE **Merged
   ...
   types being present.
2. **Named-tuple literal - `(Name=val, Price=val)`.** DONE **Already in the
...
5. **`OuterArgZeroMacro` / `OuterArgTwoMacro` parents** -
   ...
   annotation since annotations don't inherit. Net: 12 x 3-line
   override deletions, 2 x 4-line parent-class additions.
...
- **API_REWORK sec.35 revision:** sec.35 originally said streaming was a
...
No new architecture decisions - designs locked in sec.37-json (8 locks),
sec.09-column_names, and sec.10-list_tables.
...
sqlite_master ...", type<MasterRow>)`. Decision (locked sec. 10-list_tables):
...
1. **Phase 0.1** - PR #2471 `daslib/option` + `daslib/result` DONE **merged 2026-04-24** (commit `56934f491`).
2. **Phase 0.2** - Named-tuple literal `(Name=val, ...)` DONE **already in the parser** (ds2_parser.ypp `'(' make_struct_single ')'`, verified 2026-04-24). No work needed.
...
`%stringify` containing DROP + CREATE + 8 x INSERT, calls `sqlite3_exec`,
...
2. **Zero-copy column peek.** A `peek_blob` borrow API that hands the
   ...
   for strings (see CLAUDE.md sec. "String access functions").
...
Full prerequisite list in sec. "SQL read-side prerequisites" above.
...
call-macro (`_none`). See sec. "SQL read-side prerequisites" above for
...
runtime-registration strawman (see API_MISSING.md sec. 29) hit
...
> sec.30 below is the original-design snapshot from 2026-04-24, kept as
> historical record; many details have evolved (audit table is now
> 3 columns, `success` field dropped; `pending_migrations` returns
> `array<PendingMigration>` not `array<int>`; alpha-shape concurrency
...
> rest of sec.30 stays as the original design rationale.
...
**Strawman problem (API_MISSING.md sec. 33):** every value was a
string.
...
| 31 views | 0 | 0 | `[sql_view]` + `_create_view` (linq-based DDL, raw-SQL escape) + `drop_view_if_exists` |
| 32 sql_functions | 0 | 0 | `sqlite/sqlite_boost::register_function` (SQLite-only, scalar-only) |
| 33 pragma | 0 | 0 | `sqlite/sqlite_boost::set_pragma` x 3 + `apply_recommended_pragmas` (SQLite-only) |
| **Total** | **2 fns** | **9 macros** | (emitter complexity accumulates) |
...
- **VACUUM rebuilds the file** - reclaims free pages,
  ...
  2x the DB size in free disk space.
...
- **`integrity_check` can be slow** on big DBs (full table +
  index scan). `quick_check` is 5-10x faster for page-level
...
API surface. Strawman (API_MISSING.md sec. 38) predates the tut 15
...
- **In-memory full FTS5 grammar** - NEAR, AND/OR/NOT, phrases,
  ...
  syntax parser would balloon 10x; the subset covers 90% of
  standalone use.
...
Every bullet in the API_MISSING.md sec. 40 strawman maps back to an
...
+ audit-log example from API_MISSING sec. 41.
...
chain shape x (emit-shape + runtime correctness). 3 new `failed_*.das` rejection probes:
...
`tests/dasSQLITE/test_33_join_groupby.das` - 12 new tests covering Arm B (4: emit + runtime x 2 chain shapes), Arm C (4: emit + runtime + min/max/avg + multi-aggregate), and transitive coverage via HAVING / ORDER BY / computed-key (4: emit-shape + runtime mix). 3 new `failed_*.das` rejection probes:
