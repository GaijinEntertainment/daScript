...
every cross-cutting concept from API_MISSING sec. A-E is covered in the
...
8. **`_where` - predicates** - filtering with captured-variable bind
   ...
   appendix A. Covers API_MISSING sec.5's filtering story in LINQ terms.
...
10. **`_order_by` / `_order_by_descending`** - single-column ordering
    ...
    deferred (D1/D2 in the chunk-4 plan). Covers API_MISSING sec.20.
...
11. **`take` / `skip` (LIMIT / OFFSET)** - offset/limit pagination via
    ...
    Keyset pagination is a future concept tut. Covers API_MISSING sec.21.
...
14. **`_group_by` + `_having`** - aggregate per bucket, post-aggregate
    ...
    API_REWORK sec.19 for the IGrouping-shape lock-in. **Shipped:**
...
32. **Migrations** - `[sql_migration(version=N, description=...,
    ...
    (one transaction per call, alpha-shape). Permanent NO on rollback /
...
36. **Streaming results** - iterate large result sets without
    ...
    Per API_REWORK sec.35. Mockup: **no API impact - concept tutorial**.
37. **Bulk operations** - large-batch insert, `INSERT ... SELECT`, bulk
    update. Concept + `insert_many` / transaction patterns. Per
    API_REWORK sec.38. Mockup: **no API impact - concept tutorial**.
38. **Concurrency** - threading, per-context runners, busy retry,
    connection-per-thread policy. Per API_REWORK sec.40. Mockup: **no API
...
41. **Triggers** - DB-level callbacks via raw SQL inside migrations.
    Deliberately *no* daslang-side trigger DSL. Per API_REWORK sec.41.
...
A. **The SQL function table** - which daslang / daslib calls the `_sql`
   ...
   per-provider notes. Source: API_MISSING sec. A.
B. **Cross-provider portability of annotations** - which `@sql_*`
   ...
   support). Source: API_MISSING sec. B.
...
D. **Module layout - `daslib/sql` vs. `sqlite/sqlite_boost`** - the
   ...
   Source: API_REWORK sec. Module layout.
E. **Forward-looking: `dasSQL` abstraction layer** - the roadmap beyond
   ...
   abstraction can't escape. Source: API_REWORK sec. Forward-looking.
...
| 14 | `_group_by` + `_having` | `14-group_by.das` (`19-group_by.das.mockup` superseded - IGrouping shape, see API_REWORK sec.19) | **Shipped** (chunk 4) |
