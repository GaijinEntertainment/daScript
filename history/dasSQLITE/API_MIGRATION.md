...
here graduate to API_REWORK sec.30; loose ends stay in API_MISSING sec.30.
...
Locked design preserved in [API_REWORK.md sec.30-migrations](API_REWORK.md#L3630).
...
### Q1 - Audit table semantics: log failures or only successes? DONE RESOLVED
...
### Q4 - Baseline / pre-migrations adoption DONE RESOLVED
...
### Q5 - Concurrent runners DONE RESOLVED

Closed in Scenario 8 - alpha-shape (one transaction wraps the whole call).
Coordination via SQLite's RESERVED lock + `busy_timeout`. beta
...
Status legend: [green] decided * [yellow] in discussion * [white] pending

### Scenario list (working order)

| # | Title | Status |
|---|-------|--------|
| 1 | Fresh DB - first ever app run | [green] |
| 2 | Add a column on the next release | [green] |
| 3 | Add an index + backfill data | [green] |
| 4 | Inspect, don't migrate (CLI / CI) | [green] |
| 5 | Existing pre-migrations DB ("baseline") | [green] |
| 6 | Two devs pick the same version number | [green] |
| 7 | Migration N fails midway | [green] |
| 8 | Two processes call `migrate_to_latest` at once | [green] |
| 9 | Rollback request from a non-DB user | [green] |
| 10 | DROP / RENAME column on old SQLite | [green] |
| 11 | Struct-to-struct convert + table rebuild | [green] |
| 12 | Squash / baseline old migrations (3 years in) | [green] |

---

### Scenario 1 - Fresh DB [green]
...
**Behavior (locked 2026-05-04, amended for alpha-shape concurrency 2026-05-04):**
...
**alpha vs beta.** The runner uses one transaction wrapping the whole pending
list (alpha), not per-migration transactions (beta was considered and rejected).
...
### Scenario 2 - Add a column on the next release [green]
...
- ADD/DROP CHECK constraints - too many shapes; predicate language
  doesn't map cleanly.
- 12-step ALTER for old SQLite - already deferred from API_REWORK sec.30.
...
| `migrate_22_add_column_dup_runtime.das` | Re-adding the same column at runtime panics inside the migration; alpha-shape transaction rolls back the whole call (audit table empty, table doesn't exist). |
...
### Scenario 3 - Add an index + backfill data [green]
...
**Decision (locked 2026-05-04, amended for alpha 2026-05-04) - body atomicity.**
...
> `migrate_to_latest` call, share that same transaction (alpha-shape from
...
Restating API_REWORK sec.30 lock #10 because users coming from
...
- After the **whole `migrate_to_latest` call commits** (alpha-shape from
  ...
  COMMIT boundary disappeared with alpha, so the natural unit is the call.
...
### Scenario 4 - Inspect, don't migrate [green]
...
### Scenario 5 - Existing pre-migrations DB ("baseline") [green]
...
sec. "Adopting migrations on an existing DB".
```
...
See tut 30 sec. "Adopting migrations on an existing DB".
```
...
### Scenario 6 - Two devs pick the same version number [green]
...
Amends API_REWORK sec.30 lock #5 (originally module-scoped). The migration
...
### Scenario 7 - Migration N fails midway [green]
...
**Implication of alpha-shape worth restating:** prior migrations *in this
...
| `58_failure_rolls_back_prior_migrations.das` | Pending = [v4, v5]; v4 body succeeds, v5 panics; assert v4's effects are ALSO rolled back (alpha-shape collateral). DB still at v3. Locks the alpha contract. |
...
### Scenario 8 - Two processes call `migrate_to_latest` at once [green]
...
**Decision (locked 2026-05-04) - alpha: one transaction wraps the whole call.**
...
beta (per-migration transaction + per-iteration re-check) was considered
and rejected. beta buys partial-success-on-failure (prior migrations
...
- ~30 lines of concurrency-aware loop logic vs ~10 lines for alpha.

alpha's "lose intermediate work on failure" cost is bounded - re-running
...
### Scenario 9 - Rollback request from a non-DB user [green]
...
API_REWORK sec.30 deferred-list item #1 originally left the door slightly
...
### Scenario 10 - DROP / RENAME column on old SQLite [green]
...
|---|---|---|
| `ADD COLUMN` (with constraints) | always | yes |
| `RENAME TABLE` | always | yes |
| `RENAME COLUMN` | 3.25.0 (Sep 2018) | yes |
| `DROP COLUMN` | 3.35.0 (Mar 2021) | yes |

Migration bodies can write `db |> exec("ALTER TABLE ... DROP COLUMN ...")`
or `... RENAME COLUMN ... TO ...` directly; SQLite handles them without
a recipe.

**Decision (locked 2026-05-04) - withdraw API_REWORK sec.30 deferred-list item #4.**
...
### Scenario 11 - Struct-to-struct convert + table rebuild [green]
...
`migrate_to_latest`'s big alpha-shape transaction (Scenario 8), so
...
| `93_rebuild_inside_migrate_to_latest_txn.das` | The rebuild runs inside the big alpha-shape transaction; if a later migration in the same call fails, the rebuild rolls back too; DB returns to pre-call state. |

---

### Scenario 12 - Squash / baseline old migrations (3 years in) [green]
...
- **Concurrency:** alpha-shape (one big transaction wraps the whole
