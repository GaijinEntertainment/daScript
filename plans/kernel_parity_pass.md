...
- 2026-09-03: the mirror pass on the three pp512 rows under 1.0 (Q3_K_L, IQ3_M, IQ2_XXS), on
  master after #3926 merged (866a03046, the Vulkan files byte-identical to the PR tip). Method:
  ...
  Plain pp512, llama.cpp before | ours | llama.cpp after: Q3_K_L 18813 | 20393 | 18737 (1.08);
  IQ3_M 19425 | 20734 | 19533 (1.06); IQ2_XXS 19972 | 19298 | 19483 (0.98). Verdict: gap 1 is
  closed on the board - nine of ten vehicles at or past llama.cpp, IQ2_XXS at 0.98 inside the
  box's band. The GEMM tiles still lose a little (gate/up iq2xxs 1.15, k3 1.05-1.09, iq3s 1.05;
  down 1.04-1.14 on the 2048-wide shape) and rope (2.9x ahead), act (1.4-1.7x ahead) and the
  fused residual norms pay it back. The one tile deficit worth a row of its own is the iq2xxs
  gate/up at 1.15: llama.cpp's iq2_xxs row runs at its q3_K rate (50 TF/s), so its grid decode is
  hidden where ours (41 TF/s on the 1B shape) still shows - memo delta 8 territory (the computed
  codebook and the per-workgroup prologue). The board's 0.90 rows were two-rep readings.
