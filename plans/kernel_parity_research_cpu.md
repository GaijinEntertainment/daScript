...
(`width = 256, mr = 8`) one `<32 x i8>` weight vector = 8 rows x 4 weights; for AVX-512
(`width = 512, mr = 16`) it is 16 rows x 4 weights; on NEON (`width = 128, mr = 8, rq = 2`)
it is two vectors of 4 rows x 4 weights.
...
- **We cannot put signs on the activation.** The 8 lanes of one weight vector belong to 8
  *different* rows with 8 *different* sign patterns, while the activation dword is shared
  ...
  not a choice, and it is the part of followup entry 61 that needs correcting (see sec.8).
...
- **Every grid byte is a small non-negative magnitude**: iq2xxs bytes are `{0x08,0x19,0x2b}`
  ...
  the worst-case pair sum `2 x 62 x 127 = 15748` cannot saturate `i16`; (ii) no magnitude is
...
- `keven_signs_q2xs` `int8_t[1024]` = 128 x 8 `+/-1` bytes, defined *per arch file*
...
5. Dot: `_mm256_maddubs_epi16(q2_1, q8s_1)` (x86:2699-2700), u8 x s8 -> i16 pairs.
...
4. Sign application: `vmulq_s8(q2u.val[i], q2s.val[i])` (arm:3672-3675) - **applied to the
   weights**, because `sdot` is signedxsigned and there is no `u8xs8` NEON dot. This is the
...
memory - but only for *indices*, and only in the safe direction (sec.6c): one wide store, then
narrow fully-contained loads.

Rough uop budget per 32-byte weight vector (mr = 8, AVX2):
ours ~ 8x(2 loads + 3 ALU + 1 store) + 1 reload + `abs` + `psign` ~ 51;
...
| 3 | **One `i32` load for the aux word.** Replace the four `i8` loads + 3 shifts + 3 ORs at gemm_gen.das:1766-1770 with the single aligned `i32` load IQ3_XXS already uses (gemm_gen.das:1438-1439). | no seat needed - a straight bug-shaped fix | 6 uops x 8 blocks x mr rows per superblock, for free. Do this regardless. |
| 4 | **Bake the parity-completed sign byte at repack time.** The aux word is 4x7 sign bits + a 4-bit scale, and the scale already moved to the scale row (gemm_gen.das:1746-1748) - so 4 full sign *bytes* fit in the same 4 bytes. Deletes the `ksigns` load entirely and makes the sign bits plane-resident like IQ3_S's. | repack-side; pairs with delta 2 | Removes one dependent table load per 8 weights. Zero size change for this format. |
...
1. **No parity arithmetic** - `signs64 + (q2[i] >> 9)` reads `keven_signs_q2xs` directly with
   ...
   x86 `pshufb` parity trick exists only because a 128-entry x 8-byte table read does not
...
- The `uint16` word is assembled from **two separate `i8` loads plus a shift and an OR**
  (gemm_gen.das:1711-1716). The offsets are `lp%4  in  {0,2}` within the same 4-byte column, so
...
|---|---|---|---|
| 3' | **One `i16` load for the packed word.** Replace the two `i8` loads + shift + OR (gemm_gen.das:1711-1716) with one aligned 2-byte load. | none - direct fix | 3 uops x 32 x mr per superblock, free. |
...
2. Sign expansion: `_mm256_set1_epi32(signs[0] | (signs[1] << 16))` broadcasts 4 sign bytes,
   ...
   `AND` with `k_mask2` = `{1,2,...,128}`x4 (x86:3094-3096), `cmpeq` -> 0x00/0xFF
   (x86:3133-3135).
...
- **Sign byte is read straight out of the plane** (gemm_gen.das:1541-1543) - no `ksigns`
  ...
  covers 8 rows x 4 consecutive sign bytes at stride 4** - exactly the input a
...
| 3 | Adopt the **`xor`+`sub` conditional negate** (x86:3136) in place of `or(mask,1)`+`psign` wherever the mask is already 0/-1. | folded into `psign="expand"` | 2 ops vs 2 ops on x86, but frees `VPSIGNB` port pressure and works at 512 bits where `llvm.x86.avx2.psign.b` does not exist (see sec.9). |
| 5,6,7 | `vpdpbssd` seat, `bias=128` seat, `pmaddwd` scale fold - as sec.3. | | |
...
4. Chained `ggml_vdotq_s32` x2 per 32 weights (arm:3909-3910).
...
3. **Bake the parity-completed sign byte at repack time.** Aux is 4x7 sign bits + a 4-bit
...
| 6,7 | `vpdpbssd` seat; `bias=128` seat; `pmaddwd` scale fold - as sec.3. | | |
...
- *"signs applied to the ACTIVATION"* is **not portable to our layout.** It is true of
  ...
  make the same concession for the same reason (`sdot` is signedxsigned): they apply signs to
...
- What *is* portable, and is the real content of the delta, is: **keep the grid magnitudes
  ...
  `vpdpbssd` (signedxsigned, gemm_gen.das:344-347) or by `bias=128`
...
- Removes `256 x mr / 4 = 512` scattered 4-byte stores per superblock (mr = 8) and the
...
Seats (one per existing perm row, five formats x four rows):
...
4x7 sign bits + a 4-bit scale, and the scale nibble already moved to the scale row
...
- **IQ2_XXS aux word**: four `i8` loads + 3 shifts + 3 ORs (gemm_gen.das:1766-1770) where
  ...
  (gemm_gen.das:1438-1439). 6 uops x 8 blocks x mr rows per superblock, free to reclaim.
- **IQ2_XS packed word**: two `i8` loads + shift + OR (gemm_gen.das:1711-1716) where one
  2-byte load suffices (`lp % 4  in  {0,2}`, so the pair never straddles the 4-byte column).
...
- **`bias = 128` is clamped off** for every `kq` format at gemm_gen.das:2136
  ...
  `psign` entirely and folds the `-128*Sumx` correction into the accumulator init
  (`bias_acc_init`, gemm_gen.das:400-427) - available on VNNI hosts. **Not** valid under
  `maddubs`: `(w+128) x x` pair sums reach 65280 and saturate `i16`, which is why the knob is
...
- **Per-16 split accumulators for IQ2_S/IQ2_XS** (gemm_gen.das:1817, 1893-1898) match the
  ...
  than what its IQ2_S arm does (scalar `vaddvq` x scalar scale, arm:3843-3846).
