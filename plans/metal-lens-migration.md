...
1. DONE **Lensed classes live next to where they are used** - prefill classes lens IN PLACE.
   Update prefill's require comment ("no [metal_dispatch] classes remain here") when the
   first one lands.
2. DONE **Items 13/15 via (b): extend [template_struct_instance] with axis-gated fields**
   (bindings present only when a template-constant axis is on) - cleaner than union+dummy,
   serves future single/batch twins.
3. DONE **Four per-class builders, delete enc_attn3.** Beyond simplicity: per-class DERIVED
   hazard staging is more precise than enc_attn3's one-shape-fits-all hand staging.
4. DONE **`@default = name` field annotation - bare identifier, NOT a string** (Boris's call;
...
- **P0** - drop-ins: lens the cat-d classes, delete their encoders. DONE DONE 2026-08-08.
...
- **P1** - @off/dual-view sites. DONE DONE 2026-08-08 (no lens changes, as expected). Six hand
...
- **P2** - format-axis: per-class builders + thin wrappers. DONE DONE 2026-08-08 (no lens
...
- **P3** - items 13/15 via decision #2. DONE DONE 2026-08-08. The machinery: `@template_gate = AXIS`
  ...
  (mechanics -> ARCHITECTURE.md sec.2.9-2.11), PSO rule reframed to class-file ownership with the
  ...
  bind lists on the P3 layout; all 24 depthxarm oracles green, partD/old in the historical
...
- **P4** - pf_enc_moe_route composite + the census ratchet end-state. DONE DONE 2026-08-08
...
- **P5 DONE DONE (2026-08-08, commits cfc02a925 / 2894893b8 / 9f899d793 / 67c75fa94).** The
  ...
  - **Wall-clock spot check (run - the derived-hz additions put P5 past the P4 skip
    bar):** lcpp_bench, Llama-3.2-1B Q8, --ngl 99, interleaved A/B x3 launches
