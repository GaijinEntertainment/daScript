...
- (2026-08-10) [bug]->DONE FIXED same day: temp-string reclaim + passthrough aliasing
...
- DONE FIXED (bbatkin/original-operators): a class whose NAME matches a class in a required
...
- DONE AS-DESIGNED (not a hole): init-move from a smart-pointer value (`var b <- f(p)`)
...
- DONE RESOLVED (bbatkin/original-operators): `sp..x` stays the interval (Boris: the more
...
- dasLLAMA report-only finds: tutorials/dasLLAMA/04_sessions_and_memory.das:56 hardcodes
  4 bytes/KV-entry (2x under the f16 default - its two prints disagree);
...
- (2026-08-10) Confirmed doc bugs pre-scan: 53_clargs.rst (whole-page parse_args drift +
  ...
  slipped the fan-out - DONE FIXED on bbatkin/original-operators (user report honored).
