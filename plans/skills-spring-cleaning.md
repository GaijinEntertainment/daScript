...
- `install_instructions.md` phantom `ci/check_shipped_skill_refs.awk` (x2) - dies with
  the rewrite.
...
1. DONE Charter + census + rulings (this file).
2. DONE Language migration (bundle = truth; twins shrank; daslang_internals.md born).
3. DONE Moves + splits + absorptions + retargets (opus fleet + day rulings: lint catalogs
   deleted everywhere, idiom table trimmed, gc_migration deleted, hygiene trim reverted).
4. DONE Infra: install(DIRECTORY) + .claude auto-pickup + dragon/REVIEW_COMMON ship;
   gate rewritten; skills.list dead; getting-started x3.
5. DONE Verification + PR #3755: install+smoke 19/19, gate fixture suite 12/12
...
- dasLLVM REVIEW.md: two scanner-enforced env rules deleted, version rule split (emitter
  enumeration -> ARCHITECTURE.md sec.1.2), llvm_tune_internals routing line.
...
dastest; env-knob sec.3-inventory scanner extension; computed-name env-read non-literal-arg
