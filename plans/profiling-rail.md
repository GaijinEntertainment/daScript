...
- Off-tax (pre-sweep HEAD vs post-sweep with profiling off): **zero within noise; if anything
  ...
  ~ 10-25 us against a ~10 ms 1B token - get DELETED from the off build). pp: no measurable
...
- **Metal `g_us_*` stage buckets (15 starts): untouched** - the chase/batch harnesses read
  ...
  they run ~4 pairs/token (25x sparser than the CPU family).
...
compile_check x4 harnesses, lint x12 files, kernels suite 4/4, image mechanics 22 passed /
...
Interleaved A -> B-off -> B-on x3 per bench, strictly serial, arm A via git-checkout flips.
...
- **Off-tax: 0.00% tg (82.71 -> 82.71), pp and asr wall zero within +/-0.25%.** Predicted
  ...
  deleted ~160 pairs/token (~10-25 us) sit under the +/-0.15% launch spread even on a quiet
...
6. **AMX-for-ASR audit - the M1 leg ran 2026-08-06 (structural + measured), M4 leg remains.**
   ...
   Measured (whisper-turbo + canary, jfk3, interleaved): fp32-NEON is 1.8-6x slower than q8
   ...
   is the f16-lane target - if the 2x traffic cut scales, AMX-f16 beats the q8 encoder
   ...
   **Both follow-ups BUILT and measured same-day (commit 897a7b57a), both opt-in:**
   ...
   - **Hybrid AMX+NEON row split** (`DASLLAMA_ACCEL_HYBRID=<neon %>`, `tw_mm` hook, AMX
     ...
     IDENTICAL to q8, one precision class): correct but 1.4-2x slower at every split. A
     token-blocked `dot_q8q8` loop is 6-8x off the generated tile kernel per row, and the
     ...
     (~1.7x combined throughput) is real but needs that sub-arc.
