...
- **sidecars** - the point of the whole thing: an exchange of per-box tune sidecars so a
  tool downloads one for its (version x architecture) instead of paying ~20 quiet-box
...
- v1 sidecar validity keys on (dasLLAMA version x box). Because any kernel work bumps the
...
- The three seed sidecars are FRESH mints at the current `DASLLAMA_VERSION` from current
  master on each box (m1 / m4 / zen4) - sidecar validity keys on version x box, so
...
surface) -> mint v-current sidecars on m1/m4/zen4 -> `plant` x3 -> verified on the site ->
...
- **Hardware-identity normalization (the "which boxes are the same hardware" tier)**: the
  ...
  whole uarch (every Zen 2 Threadripper/EPYC/Ryzen of that model), while darwin/linux brand
  strings are per-SKU ("Apple M1 Max" != "Apple M1 Pro"; "3970X" != "3960X") - so uarch
  siblings never match on those platforms. Curated names like "zen2" exist only as
  `DASLLAMA_BOX` records tags, not derived from hardware. Fix: a curated cpu-identity -> uarch
  map (schema- or ladder-side) adding a "same uarch" tier between same-cpu and same-arch;
  possibly a CPU-features axis (the `requires=`/`cpu_supports` vocabulary) as the honest
  compatibility floor beneath it.
- **Cross-box winner matrix**: family x architecture -> winner, aggregated server-side;
