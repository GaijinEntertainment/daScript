...
- **Metal corpus: 137 classes / 8,267 class-body LOC** (101 kernels.das, 34 prefill.das,
  ...
  {4 formats x 5 tiers}, KqMv {3 batch x 3 k-quants}, RopeStore {4 formats x 2 tiers},
  MulMm {formats x lowerings} - hold 58 classes / 3,634 LOC expressing ~14 distinct bodies.
...
- DONE `test_metal_rope_kernels` - RopeStore all 9 + FwhtUnsign (byte-exact q8/tq4 mirrors vs
  quantize_*_row; whole-arena sentinel compares on batched row tables).
- DONE `test_metal_gemv_kernels` - KqGemv K4/K5/K5C/K6, KqMv B2/B4/B8xK4/K5/K6, MoeGemv kq
  trio (dequant_k*_plane_superblock + double dot; envelope-aware bar 2e-4*Sum|w*x|).
- DONE `test_metal_misc_kernels` - CopyRow, Argmax (exact ties), EmbedQ8/K6 (bit-exact),
  AddRms/AddRmsB/PreAddRms, QkNorm (in-place + V-fusion), Geglu, Suppress/Softcap.
- DONE `test_metal_attn_kernels` - the SqAttn matrix is COMPLETE (23/23 classes direct):
  ...
  hs=128 - same kv_dim; grid (nheads, ceil(nsgs/4), B) x 128).
- DONE `test_metal_gemm_kernels` - KqMulMm K4/K5/K6 + _t tensor twins, Q8MulMmT,
...
- DONE batch GEMM q8 (7, in `test_metal_gemm_kernels`): Q8GemmB[T] dense + fused-QKV ys
...
- DONE MoE gathered mul_mm (7, same file): MoeMulMmK4/K5/K6 (k6 d-tail binding),
...
- DONE attention GEMM pair (2, `test_metal_gemm_kernels`): AttnQKMm[T]/AVMm[T] - causal +
  ...
  with garbage pads.
- DONE fused QKV rope-store + W13Sw (5, `test_metal_gemv_kernels`): Q8GemvQkvRsF16/F32 (one
...
- DONE fixed-B GEMV twins (4, same file): GemvB2/B4 (x-staged panels, BIT-exact),
  Q8MvB2/B4 (streamed skinny, 64-tg; padded xs4, nrows store gate, pad-column clamp).
- DONE MoE plumbing (8, `test_metal_prefill_kernels`): SwigluOai + Pf twin (ghost rows
...
- DONE prefill misc (5, same file): PleGatherQ8/PleFinish, AddBiasRows, PfCat2, DnBa
...
## Leg (c) - hoist the dispatch-lens micro-grammar into dasllama_kernel_access DONE DONE 2026-08-07
...
**Status 2026-08-07:**
- Grid-literal trap: DONE fixed by the leg (c) hoist (shared `mk_grid_dim` folds any literal).
- **Multi-kernel: DONE LANDED as `kernel=`** - N `[metal_dispatch]` instances on one class
...
## MoltenVK argbuf upstream report - DONE FILED 2026-08-07
...
- Subgroup lingua-franca lift: DONE DONE 2026-08-07 - the overlap was exactly the 4
