...
1. Pin tests: `tests/strings/strings_core_pins.das` - 20 `[test]` functions, 172 pins, green
   ...
   (6 markers: trim FF/VT x2, strip FF/VT x2 for the reversed direction, one over the five
...
- Length-bounded cores for slice, chop, starts_with, ends_with, find, rfind, stripx3, trimx4,
...
- Parity property harness: for every twin, `twin(d, args) == counterpart(string(d), args)`
  over generated NUL-free buffers x an edge lattice (empty view, whole view, negative and
