...
1. **Opt-in, never fatal.** `--tray` (and the `tray` key in `watchdog.json`) asks for the icon;
   ...
   without it. The default stays off: the systemd deployments never asked for one.
2. **The menu is a pure function of supervisor state.** A disabled status row, `Open <name>`
   (enabled only while the health check is green), a separator, `Shutdown`. A left click opens
   the page; the platform pops the menu on a right click. The exchange rails the Python tray
   carried (adopt a sidecar, run untuned, share) stay cut with the exchange UI.
...
6. **Pumped from `tick`.** The supervisor polls the tray once per tick, child or no child, and
   ...
   Ctrl-C. The tray never calls into the supervisor.
7. **Build.** `watchdog.das` requires `stddlg`; `bin/watchdog` links the module's static twin
   beside dasHV's, and the target needs both modules enabled.
...
- Linux, headless: the watchdog under the interpreter as a child of the test, on a private
  ...
  `watchdog_stopped`, exit 0. The bus helpers move to `tests/stddlg/_dbus_rig.das`, shared
  with the tray test.
- macOS and Windows: eyeballed on the Mac and on zen2 (a person clicks).
