...
- **Target is production daslang.io**, not a local rebuild. Half the samples arc's bugs were
  ...
  archives per-box, cached build failures. Artifacts are content-addressed (source hash x
...
- Report: one row per sample x mode (PASS/FAIL/WARN + detail + duration) appended to
...
- Wall clock: ~39 samples x 2 modes x 30-60 s ~ 40-80 min on cache-hit nights. Fine.
