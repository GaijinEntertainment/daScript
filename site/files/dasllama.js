/* dasLLAMA page — renders straight from files/dasllama/bench_records.json, the single merged
   ...
   The first paint reads bench_cells.json — the same merge minus the receipt fields, a fifth of
   the bytes — and the full store loads once, on the first receipt a visitor opens.
...
(function () {
  ...
  /* ── shared receipt: the full identity of both sides of a pair ──
     The receipt fields ride only the full store, so a receipt renders the first time its row
     opens: one fetch for the whole page, a failed one retried on the next open. */
  var fullStore = null;
  function loadFullStore() {
    if (!fullStore) {
      fullStore = fetch('files/dasllama/bench_records.json')
        .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
        .catch(function (e) { fullStore = null; throw e; });
    }
    return fullStore;
  }

  // a run's twin in the full store: same model, same (box, engine, backend, flavor, workload) —
  // the store's own run identity (upsert_bench_run); the cells row itself when the store lacks it
  function fullRun(recs, gguf, r) {
    for (var i = 0; i < recs.length; i++) {
      if (recs[i].gguf !== gguf) continue;
      var runs = recs[i].runs || [];
      for (var j = 0; j < runs.length; j++) {
        var c = runs[j];
        if (c.box === r.box && c.engine === r.engine && c.backend === r.backend &&
            c.flavor === r.flavor && (c.workload || '') === (r.workload || '')) return c;
      }
    }
    return r;
  }
  ...
  function fillReceipt(box, row, receiptFn) {
    ...
    receiptFn(row).then(function (html) { box.innerHTML = html; }, function () {
      box.dataset.loaded = '';
      box.textContent = 'the record store did not load — open the row again to retry.';
    });
  }
  ...
  function pairReceipt(p) {
    return loadFullStore().then(function (recs) {
      var lines = [];
      if (p.modelNote) lines.push('<span class="dl-receipt-note"><b>note     </b> ' + esc(p.modelNote) + '</span>');
      sideLines('das      ', fullRun(recs, p.gguf, p.das), lines);
      lines.push('');
      sideLines('reference', fullRun(recs, p.gguf, p.ref), lines);
      return lines.join('\n');
    });
  }
    ...
    recs.forEach(function (m) {
      ...
      Object.keys(boxes).forEach(function (bx) {
        LANES.forEach(function (L) {
          ...
          out.push({
            gguf: m.gguf, model: m.gguf.replace(/\.gguf$/, ''), arch: m.arch || '',
      ...
      Object.keys(boxes).forEach(function (bx) {
        ...
        runs.filter(function (r) {
          ...
          Object.keys(das.tests || {}).forEach(function (k) {
            ...
            out.push({
              gguf: m.gguf, model: m.arch || m.gguf, box: bx, lane: L.label,
  ...
  /* ── load ───────────────────────────────────────────────────── */
  fetch('files/dasllama/bench_cells.json')
    .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
    .then(function (recs) {
