...
- `site/files/news.json` (top 8 for landing sec. 05)
...
tab strip + Select example + up share + -> run on the left of a unified
...
- **Tabs**: `main.das` is the fixed entry point - non-renameable, non-deletable.
  ...
  rename (validates `.das` suffix, rejects duplicates). `x` deletes (confirms
  if non-empty).
...
- **Share** (up share): builds `location + '#z=' + LZString.compressToEncodedURIComponent(JSON.stringify(state))`.
  ...
  legacy `#code=<percent>` hash (emitted by the landing hero's up playground
