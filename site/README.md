...
`files/wasm/`, `files/news.json`, `files/profile_results_*.json`, `doc/`) are
...
The landing chart and `benchmarks.html` read them directly - no rebuild needed.
...
- CI fetches `profile_results_<platform>.json` on each publish.
