...
**A diff that adds a `*_words` fixture whose tile shapes are already mirrored by an existing
`*_words` fixture deletes the existing fixture, or re-aims it onto a shape set the engine - the
shipped kernels these fixtures mirror - still declares, in the same change.** Two fixtures over
one shape set let one keep shapes the engine no longer declares and still report green.
