...
- **The watchdog shows a tray icon** - `--tray` (the `tray` key in `watchdog.json`): a status mark badged by state, one status line as the tooltip and the menu's first row, `Open <name>` while the health check is green, `Shutdown` through the stop ladder; notifications go through the icon while it is up. `bin/watchdog` links dasStdDlg static beside dasHV.
