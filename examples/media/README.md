...
MIDI sequences of public-domain compositions for testing the MIDI player:

| File | Piece | Source |
|---|---|---|
| `midi/fur_elise.mid` | Beethoven - Fur Elise | midiworld.com |
| `midi/Bach_Air_on_G_string_BWV1068.mid` | Bach - Air on the G String (BWV 1068) | MIDI archive sequence of the public-domain composition |
| `midi/entertainer.mid` | Joplin - The Entertainer | MIDI archive sequence of the public-domain composition |
| `midi/pitch_bend_demo.mid` | first-party pitch-bend test fixture | generated in-repo |

## Audio

First-party synthesized test sounds (public domain), regenerated byte-identically
by the repo's `gen_media_assets.das` generator (repo-only, not part of the SDK) -
inharmonic modal synthesis for the gong, an additive chord pad for the music loop:

| File | Content |
|---|---|
| `audio/gong.wav` | synthesized gong strike, 2.0 s mono 44.1 kHz |
| `audio/music.wav` | synthesized ambient chord loop (Am-F-C-G), 12 s mono 44.1 kHz |
