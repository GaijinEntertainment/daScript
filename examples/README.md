...
Assets are bundled in the directory: `image.png` and `orb.obj` are first-party
procedural assets (regenerated byte-identically by the repo's
`gen_media_assets.das` generator, repo-only); `droidsansmono.ttf` is Droid Sans
Mono (Apache-2.0, see `droidsansmono.LICENSE` beside it).
