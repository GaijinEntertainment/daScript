...
set PATH=%VSPATH%\include;%WINSDK%\10.0.10586.0\ucrt;%VSPATH%\bin\x86_amd64;%VSPATH%\lib\amd64;%WINSDK%\lib\10.0.10586.0\um\x64;%WINSDK%\lib\10.0.10586.0\ucrt\x64;
...
cl /Feasjit.exe /DUSE_JIT -I %WINSINC% -I %WININC% -I %VSINC% -I jit jit\as_jit.cpp jit\virtual_asm_windows.cpp jit\virtual_asm_x64.cpp main.cpp -I sdk\angelscript\include -I sdk\add_on\ sdk\angelscript\source\*.cpp /EHsc /DAS_USE_STLNAMES=1 sdk\add_on\scriptstdstring\*.cpp sdk\add_on\scriptarray\*.cpp /Ox /Oy /DNDEBUG /DAS_NO_THREADS winmm.lib %WINSDK%\lib\10.0.10586.0\ucrt\x64\libucrt.lib as_callfunc_x64_msvc_asm.obj 
cl /Feas.exe main.cpp -I %WINSINC% -I %WININC% -I %VSINC% -I sdk\angelscript\include -I sdk\add_on\ sdk\angelscript\source\*.cpp /EHsc /DAS_USE_STLNAMES=1 sdk\add_on\scriptstdstring\*.cpp sdk\add_on\scriptarray\*.cpp /Ox /Oy /DNDEBUG /DAS_NO_THREADS winmm.lib  %WINSDK%\lib\10.0.10586.0\ucrt\x64\libucrt.lib as_callfunc_x64_msvc_asm.obj 
