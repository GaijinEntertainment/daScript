...
A `sec.6` batch - multiple requests + a notification in one message - comes back
...
- `make_batch(messages)` - wrap N pre-built wires as a sec.6 batch.
- `parse_response(line)` - server response -> `ParsedResponse {is_success,
  result_json, error_code, error_msg, error_data}`.
- `parse_response_batch(line)` - same for sec.6 batched responses.
